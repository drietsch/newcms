...
function checkDeleteEntry($id,$table){
	if($table == FILE_TABLE || (defined("OBJECT_FILES_TABLE") && $table==OBJECT_FILES_TABLE)) return true;
...
function deleteFile($id,$table,$path="",$contentType=""){
	...
	if($table==TEMPLATES_TABLE || $table==FILE_TABLE) deleteLocalFile($file);
	if($table==FILE_TABLE){
		$file = $_SERVER["DOCUMENT_ROOT"].SITE_DIR.substr($path,1);
	...
	if($table==FILE_TABLE){
		...
		if(in_array("schedule", $GLOBALS['_we_active_modules'])){	//	Delete entries from schedule as well
			$DB_WE->query('DELETE FROM ' . SCHEDULE_TABLE . ' WHERE DID="' . $id . ' " AND ClassName !="we_objectFile"');
		}
		$DB_WE->query('DELETE FROM '.NAVIGATION_TABLE.' WHERE Selection="static" AND SelectionType="docLink" AND LinkID="'.$id.'";');
...
function deleteEntry($id,$table,$delR=true){
	...
	if(defined("WORKFLOW_TABLE") && ($table==FILE_TABLE || (defined("OBJECT_FILES_TABLE") && $table==OBJECT_FILES_TABLE))){
		...
		if(weWorkflowUtility::inWorkflow($id,$table)) weWorkflowUtility::removeDocFromWorkflow($id,$table,$_SESSION["user"]["ID"],$l_workflow["doc_deleted"]);
	}
	if($id){
