...
$wfchk = defined("WORKFLOW_TABLE") && ($table==FILE_TABLE || (defined("OBJECT_FILES_TABLE") && $table==OBJECT_FILES_TABLE)) ? (isset($_REQUEST["we_cmd"][3]) ? $_REQUEST["we_cmd"][3] : 0) : 1;
$wfchk_html="";
$script = "";
...
if(!$wfchk){
	...
	if(isset($_REQUEST["sel"])){
		...
		$wfchk_html.= '<script language="JavaScript" type="text/javascript">
                     function confirmDel(){'."\n";
		if($found) $wfchk_html.= 'if(confirm("'.$l_alert["found_in_workflow"].'"))'."\n";
		$wfchk_html.= 'we_cmd("' . $_REQUEST["we_cmd"][0] . '","","'.$table.'",1);'."\n";
		if($found) $wfchk_html.= 'else top.toggleBusy(0)';
		$wfchk_html.= '}</script>';

	}else{
		$script = "top.toggleBusy(0);\n" . we_message_reporting::getShowMessageCall($l_alert["nothing_to_delete"], WE_MESSAGE_WARNING);
	}
...
} else if($_REQUEST["we_cmd"][0] == "do_delete" || $_REQUEST["we_cmd"][0] == "delete_single_document"){
    if(isset($_REQUEST["sel"]) && $_REQUEST["sel"]){
    	//	look which documents must be deleted.
		$selectedItems = explode(",",$_REQUEST["sel"]);
		$retVal   = 1;
		$idInfos  = array('IsFolder'=>0, 'Path'=>'','hasFiles'=>0);
		if (sizeof($selectedItems) > 0 && ($table == FILE_TABLE || $table == TEMPLATES_TABLE)) {
			...
			if ($idInfos['IsFolder']) {
				$idInfos['hasFiles'] = f("SELECT ID FROM $table WHERE ParentID=" . $selectedItems[0] . " AND  IsFolder = 0 AND Path LIKE '".$idInfos['Path']."%'","ID",$DB_WE) > 0 ? 1 : 0;
			}
		}
		...
		if(we_hasPerm("ADMINISTRATOR")) {
			$hasPerm = 1;
		} else if($idInfos['IsFolder']) {
			if($table == FILE_TABLE && we_hasPerm("DELETE_DOC_FOLDER") && (!$idInfos['hasFiles'] || we_hasPerm("DELETE_DOCUMENT"))) {
				$hasPerm = 1;
			} else if($table == TEMPLATES_TABLE && we_hasPerm("DELETE_TEMP_FOLDER") && (!$idInfos['hasFiles'] || we_hasPerm("DELETE_TEMPLATE"))) {
				$hasPerm = 1;
			} else if($table == OBJECT_FILES_TABLE && we_hasPerm("DELETE_OBJECTFILE")) {
				$hasPerm = 1;
			} else {
				$hasPerm = 0;
			}
			...
			if($table == FILE_TABLE && we_hasPerm("DELETE_DOCUMENT")) {
				$hasPerm = 1;
			} else if($table == TEMPLATES_TABLE && we_hasPerm("DELETE_TEMPLATE")) {
				$hasPerm = 1;
			} else if($table == OBJECT_FILES_TABLE && we_hasPerm("DELETE_OBJECTFILE")) {
				$hasPerm = 1;
			} else if($table == OBJECT_TABLE && we_hasPerm("DELETE_OBJECT")) {
				$hasPerm = 1;
			} else {
				$hasPerm = 0;
			}
		...
		if (!$hasPerm){
			$retVal = -6;
    	} else if((!we_hasPerm("ADMINISTRATOR")) && ($table == FILE_TABLE . "_cache" || $table == OBJECT_FILES_TABLE . "_cache")) { //check if mey delete cache
			$retVal = -1;
		} else {
		...
		if ($retVal == -6) {
			...
			$script .= we_message_reporting::getShowMessageCall(sprintf($l_alert['delete_workspace_object_r'],id_to_path($selectedItems[$i],$table),$objList), WE_MESSAGE_ERROR);
			...
			$script .= we_message_reporting::getShowMessageCall(sprintf($l_alert['delete_workspace_user_r'],id_to_path($selectedItems[$i],$table),$usrList), WE_MESSAGE_ERROR);
			...
			$script .= we_message_reporting::getShowMessageCall(sprintf($l_alert['delete_workspace_object'],id_to_path($selectedItems[$i],$table),$objList), WE_MESSAGE_ERROR);
			...
			$script .= we_message_reporting::getShowMessageCall(sprintf($l_alert['delete_workspace_user'],id_to_path($selectedItems[$i],$table),$usrList), WE_MESSAGE_ERROR);

		} else if($retVal == -1){	//	not allowed to delete document

			$script .= "top.toggleBusy(0);\n";
			$script .= we_message_reporting::getShowMessageCall(sprintf($l_alert["noRightsToDelete"],id_to_path($selectedItems[$i],$table)), WE_MESSAGE_ERROR);
			...
			if($table == FILE_TABLE . "_cache" || (defined("OBJECT_FILES_TABLE") && $table == OBJECT_FILES_TABLE . "_cache")) {
				...
				if (sizeof($deletedItems)) {
					...
					if ( defined("CUSTOMER_TABLE") ) { // delete the customerfilters
						...
						weDocumentCustomerFilter::deleteModel( $deletedItems, $table );
						if (defined("OBJECT_FILES_TABLE") && $table == OBJECT_TABLE) {
							if (sizeof($deleted_objects)) {
								weDocumentCustomerFilter::deleteModel( $deleted_objects, OBJECT_FILES_TABLE );
					...
					we_history::deleteFromHistory($deletedItems, $table);
					if (defined("OBJECT_FILES_TABLE") && $table == OBJECT_TABLE) {
						if (sizeof($deleted_objects)) {
							we_history::deleteFromHistory( $deleted_objects, OBJECT_FILES_TABLE );
			...
			if ($_SESSION["we_mode"] == "normal") {	//	different messages in normal or seeMode
				if(sizeof($GLOBALS["we_folder_not_del"])){
					...
					$_SESSION["delete_files_info"]=str_replace("\\n","",sprintf($GLOBALS["l_alert"]["folder_not_empty"],""));
					...
					if(		defined("OBJECT_FILES_TABLE") && $table == OBJECT_FILES_TABLE . "_cache"
						||	defined("FILE_TABLE") && $table == FILE_TABLE . "_cache") {
					...
					$script .= we_message_reporting::getShowMessageCall($delete_ok, WE_MESSAGE_NOTICE);
				}
			}
			...
			$script .= "top.toggleBusy(0);\n";
			if($table == TEMPLATES_TABLE){
				$script .= we_message_reporting::getShowMessageCall($l_alert["deleteTempl_notok_used"], WE_MESSAGE_ERROR);
			}else if($table == OBJECT_TABLE){
				$script .= we_message_reporting::getShowMessageCall($l_alert["deleteClass_notok_used"], WE_MESSAGE_ERROR);
			}else{
				$script .= we_message_reporting::getShowMessageCall($l_alert["delete_notok"], WE_MESSAGE_ERROR);
			}
		}
...
if($_SESSION["we_mode"] == "seem"){
	...
	if($retVal){	//	document deleted -> go to seeMode startPage
		$_js = we_message_reporting::getShowMessageCall($l_alert['delete_single']['return_to_start'], WE_MESSAGE_NOTICE) . "top.we_cmd('start_multi_editor');";
	} else {
...
?>
if(top.treeData.table != "<?php print ereg_replace("_cache$", "", $table); ?>"){
	top.treeData.table = "<?php print ereg_replace("_cache$", "", $table); ?>";
	we_cmd("load","<?php print ereg_replace("_cache$", "", $table); ?>");
}else{
top.drawTree();
}
...
<?php
if(!$wfchk && $_REQUEST["we_cmd"][0] != "delete"){ print $wfchk_html;exit();}
if($_REQUEST["we_cmd"][0] == "do_delete"){ print "</head><body></body></html>";exit();}
?>
<?php
...
$_buttons = $we_button->position_yes_no_cancel(	$we_button->create_button("ok", "javascript:if(confirm('".$delete_confirm."')) we_cmd('do_delete','','$table')"),
												"",
...
<h1 class="big" style="padding:0;margin:0;">'.htmlspecialchars($l_newFile["title_delete"]). '</h1>
