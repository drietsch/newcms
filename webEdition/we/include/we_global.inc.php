...
function correctUml($in) {
	$in = str_replace("ï¿½","ae",$in);
	$in = str_replace("ï¿½","oe",$in);
	$in = str_replace("ï¿½","ue",$in);
	$in = str_replace("ï¿½","Ae",$in);
	$in = str_replace("ï¿½","Oe",$in);
	$in = str_replace("ï¿½","Ue",$in);
	return str_replace("ï¿½","ss",$in);
}
...
function we_getSelectField($name,$value,$values,$attribs=array(),$addMissing=true) {
	...
	if((!$isin) && $addMissing){
	    $content .= getHtmlTag('option', array('value'=>htmlspecialchars($value),'selected'=>'selected'),htmlspecialchars($value),true) . "\n";
	}
	return getHtmlTag('select',$attribs,$content,true);
}

function we_getCatsFromDoc($doc,$tokken=",",$showpath=false,$db="",$rootdir="/", $catfield="") {
	return we_getCatsFromIDs((isset($doc->Category) ? $doc->Category : ""),$tokken,$showpath,$db,$rootdir,$catfield);
}
	...
	if($catIDs) {
		...
		for($i=0;$i<sizeof($foo);$i++){
			if (!isset($GLOBALS["WE_CATEGORY_CACHE"][$foo[$i]])) {
				$GLOBALS["WE_CATEGORY_CACHE"][$foo[$i]] = getHash("SELECT ID,Path,Category,Catfields FROM " . CATEGORY_TABLE . " WHERE ID='".$foo[$i]."'", $db);
			}
...
function initDocument($formname="we_global_form",$tid="",$doctype="",$categories="") {
	...
	if((!$session) || (!isset($_SESSION["we_document_session_$formname"]))) {
		...
		if(isset($_REQUEST["we_editDocument_ID"]) && $_REQUEST["we_editDocument_ID"]) {
			...
			$dt = f("SELECT ID FROM " . DOC_TYPES_TABLE . " WHERE DocType like '".$doctype."'","ID",$GLOBALS["we_document"][$formname]->DB_WE);
				...
				if ($_imgDataId !== false && isset($_SESSION[$_imgDataId])) {
					...
					if (isset($_REQUEST["WE_UI_DEL_CHECKBOX_" . $imgName]) && $_REQUEST["WE_UI_DEL_CHECKBOX_" . $imgName]==1) {
						...
						if ($ct == "image/*") {
							...
							$_SESSION[$_imgDataId]["serverPath"] = TMP_DIR."/".md5(uniqid(rand(),1));
							...
							$tmp_Filename = $imgName . "_" .md5(uniqid(rand(),1)) . "_" . preg_replace("/[^A-Za-z0-9._-]/", "", $_FILES["we_ui_$formname"]["name"][$imgName]);
							...
							$_SESSION[$_imgDataId]["extension"] = (strpos($tmp_Filename,".") > 0) ? eregi_replace('^.+(\..+)$',"\\1",$tmp_Filename) : "";
							...
							if ( (isset($_SESSION[$_imgDataId]["width"]) && $_SESSION[$_imgDataId]["width"])  ||
								(isset($_SESSION[$_imgDataId]["height"]) && $_SESSION[$_imgDataId]["height"])) {
									...
									$thumb->init("dummy", $_SESSION[$_imgDataId]["width"], $_SESSION[$_imgDataId]["height"], $_SESSION[$_imgDataId]["keepratio"],
										$_SESSION[$_imgDataId]["maximize"], false, '', "dummy", 0, "", "", $_SESSION[$_imgDataId]["extension"], $we_size[0],$we_size[1], $imageData,"",$_SESSION[$_imgDataId]["quality"]);
...
function checkAndCreateImage($formname, $type="we_document") {
	...
	foreach($_REQUEST as $key=>$_imgDataId) {
		if (preg_match('|^WE_UI_IMG_DATA_ID_(.*)$|', $key, $regs)) {
			...
			} else if(isset($_SESSION[$_imgDataId]['serverPath'])) {
...
function cleanTempFiles($cleanSessFiles=0) {
	...
	while($DB_WE->next_record()) {
		...
		$db2->query("
				DELETE
				FROM " . CLEAN_UP_TABLE . "
				WHERE DATE=".$DB_WE->f("Date")." AND Path='".$DB_WE->f("Path")."'");
	}
	if($cleanSessFiles) {
		$seesID = session_id();
		$DB_WE->query("
...
function getUsedTemplatesOfTemplate($id, &$arr) {
	global $DB_WE;
	$_hash = getHash("SELECT IncludedTemplates, MasterTemplateID FROM ".TEMPLATES_TABLE." WHERE ID=".abs($id),$DB_WE);
...
function makeOwnersSql($useCreatorID=true) {
	if(defined("BIG_USER_MODULE") && in_array("busers",$GLOBALS["_pro_modules"]) && (!$_SESSION["perms"]["ADMINISTRATOR"])) {
		...
		foreach($aliases as $id) we_getParentIDs(USER_TABLE,$id,$groups,$GLOBALS["DB_WE"]);
...
function in_parentID($id,$pid,$table=FILE_TABLE,$db="") {
	if(abs($pid)!=0 && abs($id)==0) return false;
...
function getHashArrayFromCSV($csv,$firstEntry="",$db) {
	...
	$out = $firstEntry ? array("0"=>$firstEntry) : array();
	...
	switch($table) {
		...
		default:
			if(defined('OBJECT_FILES_TABLE') && $table==OBJECT_FILES_TABLE){
				$type = 2;
				break;
			} else if(defined('NEWSLETTER_TABLE') && $table==NEWSLETTER_TABLE){
				$type = 4;
				break;
			}
			return "";
...
function get_def_ws($table=FILE_TABLE,$prePostKomma=false) {
	...
	if(defined("BIG_USER_MODULE") && in_array("busers",$GLOBALS["_pro_modules"])) {
		$foo = f("
			SELECT workSpaceDef
			FROM ".USER_TABLE."
			WHERE ID='".$_SESSION["user"]["ID"]."'","workSpaceDef",new DB_WE());
...
function getHrefForObject($id,$pid,$path="",$DB_WE="") {
	...
	if (!$id) {
		...
		$published = f("SELECT Published FROM " . OBJECT_FILES_TABLE . " WHERE ID='$id'","Published",$DB_WE);
		if (!$published) {
	...
	$foo = getHash("
		...
		FROM " . OBJECT_FILES_TABLE . "
		WHERE ID='".$id."'",$DB_WE);
	if(count($foo) == 0) return "";
	$showLink = false;

	if($foo["Workspaces"]) {
		if(in_workspace($pid,$foo["Workspaces"],FILE_TABLE,$DB_WE))
		$showLink = true;
		else if($foo["ExtraWorkspacesSelected"]) {
	...
	if($showLink) {
		...
		if($foo["Workspaces"]) {
			...
			$path = f("
				SELECT Path
				FROM " . FILE_TABLE . "
				WHERE Published > 0 AND ContentType='text/webedition' AND IsDynamic=1 AND Path like '$path%'","Path",$DB_WE);
			if(!$path)
			return "";
			return $path."?we_objectID=".$id."&amp;pid=$pid";
		}
	...
	return $path;
		...
		if(in_workspace($pid,$arr3[$i])) {
			$path = f("
				SELECT Path
				FROM " . FILE_TABLE . "
				WHERE Published > 0 AND ContentType='text/webedition' AND IsDynamic=1 AND Path like '$ws%'","Path",$DB_WE);
			if($path)
			return $path;
		}
		...
		for($i=0;$i<sizeof($regs);$i++) {
			if (isset($GLOBALS["we_doc"]->InWebEdition) && $GLOBALS["we_doc"]->InWebEdition) {
				...
				$foo = getHash("
					SELECT Path
					FROM " . FILE_TABLE . "
					WHERE ID='".$regs[$i][2]."' AND Published > 0",$DB_WE);
			}

			if(isset($foo["Path"])){
				$text = str_replace($regs[$i][1].'="document:'.$regs[$i][2].$regs[$i][3],$regs[$i][1].'="'.$foo["Path"] . $regs[$i][3],$text);
			}else{
	...
	if(preg_match_all('/src="thumbnail:([^" ]+)"/i',$text,$regs,PREG_SET_ORDER)){
		...
		for($i=0;$i<sizeof($regs);$i++) {
			...
			if($thumbObj->initByImageIDAndThumbID($imgID,$thumbID)){
				$text = eregi_replace('src="thumbnail:'.$regs[$i][1].'"','src="'.$thumbObj->getOutputPath().'"',$text);
			}else{
	...
	if(defined("OBJECT_TABLE")){
		if(preg_match_all('/href="object:(\d+)(\??)("|[^"]+")/i',$text,$regs,PREG_SET_ORDER)){
			for($i=0;$i<sizeof($regs);$i++) {
				...
				if($href){
					if($regs[$i][2] == "?"){
						$text = str_replace('href="object:'.$regs[$i][1]."?",'href="'.$href."&amp;",$text);
					}else{
						$text = str_replace('href="object:'.$regs[$i][1].$regs[$i][2].$regs[$i][3],'href="'.$href.$regs[$i][2].$regs[$i][3],$text);
					}
				}else{
...
function getMysqlVer($nodots=true){
	...
	if(isset($res)) {
		if($nodots) {
			...
			if(strlen($ver)<4){
				$ver = sprintf("%04d",$ver);
				if(substr($ver,0,1)=="0") $ver =(int)(substr($ver,1)."0");
			}
...
function we_check_email($email){
	...
	if (preg_match('/<(.)*>/',$email,$_email)) {
		...
		if(strpos($namePart,'"')) return false;
...
function getRequestVar($name,$default,$yescode="",$nocode=""){
	if(isset($_REQUEST[$name])){
		if($yescode!="") eval($yescode);
		return $_REQUEST[$name];
	}
	else{
		if($nocode!="") eval($nocode);
		return $default;
	}
}
...
function setUserPref($name,$value){
	if(isset($_SESSION['prefs'][$name]) &&  isset($_SESSION['prefs']['userID']) && $_SESSION['prefs']['userID']){
		...
		$_db = new DB_WE();
		$_db->query('UPDATE '.PREFS_TABLE.' SET '.$name.'="'.addslashes($value).'" WHERE userId='.$_SESSION['prefs']['userID']);
		return true;
	}
	return false;
}
	...
	foreach($patharr as $elem){
		if($elem!="" && $elem!="/"){
			...
			if(!$id){
				...
				$new->Owners=','.$owner.',';
				$new->OwnersReadOnly=serialize(array($owner=>0));
				$new->we_save();
				$id=$new->ID;
				$pathids[]=$id;
			}
...
function getHtmlTag($element, $attribs=array(), $content="", $forceEndTag=false, $onlyStartTag=false){
	...
	$attribs = removeAttribs($attribs, array("xml", "xmltype"));
...
function getNumberOfDays($month,$year){
		if(in_array($month,array(1,3,5,7,8,10,12))) $numofdays="31";
		else if(is_int($year/4) && $month==2) $numofdays="29";
 		else if($month==2) $numofdays="28";
 		else $numofdays="30";
	...
	if($ws){
		...
		foreach($b as $k=>$v){
			if((!defined("WE_DOCTYPE_WORKSPACE_BEHAVIOR")) || WE_DOCTYPE_WORKSPACE_BEHAVIOR==0){
				...
				while($db->next_record()){
					array_push($paths,"(ParentPath = '".$db->f("Path")."' || ParentPath like '".$db->f("Path")."/%')");
				}
			}else{
...
function we_loadLanguageConfig() {
	...
	if(!file_exists($file) || !is_file($file)) {
		if(WE_LANGUAGE == "Deutsch") {
			we_writeLanguageConfig('de_DE', array('de_DE', 'en_GB'));

		} else {
			we_writeLanguageConfig('en_GB', array('de_DE', 'en_GB'));
