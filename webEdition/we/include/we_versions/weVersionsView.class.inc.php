...
class weVersionsView {
	...
	function getJS() {
		...
		$js = we_htmlElement::jsElement('
			...
			function newinput() {
				...
				var searchFields = "' . str_replace("\n","\\n",addslashes(htmlSelect('searchFields[__we_new_id__]', $this->searchclass->getFields(),1,"",false,'class="defaultfont" id="searchFields[__we_new_id__]" onChange="changeit(this.value, __we_new_id__);" '))) . '";
				var locationFields = "' . str_replace("\n", "\\n", addslashes(htmlSelect('location[__we_new_id__]', $this->searchclass->getLocation(), 1, "", false, 'class="defaultfont" disabled id="location[__we_new_id__]"  '))) . '";
				var search = "' . str_replace("\n","\\n",addslashes(htmlSelect('search[__we_new_id__]', $this->searchclass->getModFields(),1,"",false,'class="defaultfont" style="width:190px;" id="search[__we_new_id__]" '))) . '";
			...
			function changeit(value, rowNr){
		        ...
		        var searchFields = "' . str_replace("\n", "\\n", addslashes(htmlSelect('searchFields[__we_new_id__]', $this->searchclass->getFields(), 1, "", false, 'class="defaultfont" id="searchFields[__we_new_id__]" onChange="changeit(this.value, __we_new_id__);" '))) . '";
		        var locationFields = "' . str_replace("\n", "\\n", addslashes(htmlSelect('location[__we_new_id__]', $this->searchclass->getLocation(), 1, "", false, 'class="defaultfont" id="location[__we_new_id__]"  '))) . '";
		        ...
		        var location = document.getElementById("location["+rowNr+"]");  
	       		...
	       		if(value=="allModsIn") {
					...
					locationFields = "' . str_replace("\n", "\\n", addslashes(htmlSelect('location[__we_new_id__]', $this->searchclass->getLocation("date"), 1, "", false, 'class="defaultfont" id="location[__we_new_id__]"  '))) . '";
					...
					search = "' . str_replace("\n", "\\n", addslashes(htmlSelect('search[__we_new_id__]', $this->searchclass->getUsers(), 1, "", false, 'class="defaultfont" style="width:190px;" id="search[__we_new_id__]" '))) . '";
					...
					search = "' . str_replace("\n", "\\n", addslashes(htmlSelect('search[__we_new_id__]', $this->searchclass->getStats(), 1, "", false, 'class="defaultfont" style="width:190px;" id="search[__we_new_id__]" '))) . '";
	...
	*/
	function getBodyTop() {
		...
		for($i=0;$i < $this->searchclass->height ;$i++){
			...
			$search = htmlSelect("search[".$i."]",$this->searchclass->getModFields(),1,(isset($this->searchclass->search) && is_array($this->searchclass->search) && isset($this->searchclass->search[$i]) ? $this->searchclass->search[$i] : "" ),false,'class="defaultfont" style="width:190px;" id="search['.$i.']" ');
			...
			if(isset($this->searchclass->searchFields[$i])) {
				...
				if($this->searchclass->searchFields[$i]=="modifierID") {
					$search = htmlSelect("search[".$i."]",$this->searchclass->getUsers(),1,(isset($this->searchclass->search) && is_array($this->searchclass->search) && isset($this->searchclass->search[$i]) ? $this->searchclass->search[$i] : "" ),false,'class="defaultfont" style="width:190px;" id="search['.$i.']" ');
				}
				if($this->searchclass->searchFields[$i]=="status") {
					$search = htmlSelect("search[".$i."]",$this->searchclass->getStats(),1,(isset($this->searchclass->search) && is_array($this->searchclass->search) && isset($this->searchclass->search[$i]) ? $this->searchclass->search[$i] : "" ),false,'class="defaultfont" style="width:190px;" id="search['.$i.']" ');
				}
				if($this->searchclass->searchFields[$i]=="timestamp") {
					$locationDisabled = "";
					$handle = "date";
					$search = $this->searchclass->getDateSelector("","search[".$i."]","_from".$i,$this->searchclass->search[$i]);
				}
			...
			$out .= '
				<tr id="filterRow_'.$i.'">
					<td>'.htmlSelect("searchFields[".$i."]",$this->searchclass->getFields(),1,(isset($this->searchclass->searchFields) && is_array($this->searchclass->searchFields) && isset($this->searchclass->searchFields[$i]) ? $this->searchclass->searchFields[$i] : "" ),false,'class="defaultfont" id="searchFields['.$i.']" onChange="changeit(this.value, '.$i.');" ').'</td>
					<td id="td_location[' . $i . ']">' . htmlSelect("location[" . $i . "]", $this->searchclass->getLocation($handle), 1, (isset($this->searchclass->location) && is_array($this->searchclass->location) && isset($this->searchclass->location[$i]) ? $this->searchclass->location[$i] : ""), false, 'class="defaultfont" ' . $locationDisabled . ' id="location[' . $i . ']" ') . '</td>
		...
		$out .=	'<table border="0" cellpadding="0" cellspacing="0">
				...
				<tr id="beschreibung_print" class="defaultfont">
				 ...
				 <tr>
					...
					<td class="defaultgray" style="width:70px;">'.htmlSelect("anzahl",$anzahl_all,1,$_anzahl,"",'id="anzahl" onChange=\'this.form.elements["searchstart"].value=0;search(false);\'').'
					</td>
					<td class="defaultfont" id="eintraege">'.$GLOBALS['l_versions']["eintraege"].'</td>
					<td>'.self::getNextPrev($foundItems).'</td>
					<td id="print" class="defaultfont">'.getPixel(10,12).'<a href="javascript:window.print()">'.$GLOBALS['l_versions']["printPage"].'</a></td>
				</tr>
				<tr>
	...
	*/
	...
	function getNextPrev($we_search_anzahl){
		...
		$select = htmlSelect("page", $pages, 1, $page, false, "id='pageselect' onChange=\"this.form.elements['searchstart'].value = this.value; search(false);\"");
	...
	*/	
	function getVersionsOfDoc() {
		...
		if($resultCount>0) {
			...
			if ($sortierung[0] == "modifierID") {
				if (isset($sortierung[1])) {
					usort($_Result, array($thisObj, 'sortResultListUserDESC'));
				} else {
					usort($_Result, array($thisObj, 'sortResultListUserASC'));
			}
		...
		for ($f=0;$f<$resultCount;$f++) {
			...
			$content[$f][2]["dat"] = '<nobr>'.($_versions[$f]["timestamp"] ? date("d.m.y - H:i:s",$_versions[$f]["timestamp"]) : "-").' </nobr>';
			$content[$f][3]["dat"] = $fromScheduler.$fromImport.$resetFromVersion.$modificationText;
			$content[$f][4]["dat"] = (we_hasPerm("ADMINISTRATOR")) ? we_forms::checkbox($_versions[$f]["ID"], 0, "deleteVersion", "", false, "defaultfont", "") : "";
			$content[$f][5]["dat"] = getPixel(1,1);
			$content[$f][6]["dat"] = "<span class='printShow'>".$we_button->create_button("reset", "javascript:resetVersion('".$_versions[$f]["ID"]."','".$_versions[$f]["documentID"]."','".$_versions[$f]["version"]."','".$_versions[$f]["documentTable"]."');",true,100,22,"","",$disabledReset)."</span>";
			$content[$f][7]["dat"] = "<span class='printShow'>".$we_button->create_button("preview", "javascript:previewVersion('".$_versions[$f]["ID"]."');")."</span>";	
			$content[$f][8]["dat"] = ($_versions[$f]["ContentType"]=="text/webedition" || $_versions[$f]["ContentType"]=="text/html" || $_versions[$f]["ContentType"]=="objectFile") ? we_forms::checkbox($_versions[$f]["ID"], 0, "publishVersion_".$_versions[$f]["ID"], $GLOBALS['l_versions']['publishIfReset'], false, "middlefont", "") : "";
		}
	...
	*/
	function makeHeadLines() {
	 	...
	 	$headline[0]["dat"] = '<a href="javascript:setOrder(\'version\');">'.$GLOBALS['l_versions']['version'].'</a> <span id="version" >' . $this->getSortImage('version').'</span>';
	 	$headline[1]["dat"] = '<a href="javascript:setOrder(\'modifierID\');">'.$GLOBALS['l_versions']['user'].'</a> <span id="modifierID" >' . $this->getSortImage('modifierID').'</span>';
		$headline[2]["dat"] = '<a href="javascript:setOrder(\'timestamp\');">'.$GLOBALS['l_versions']['modTime'].'</a> <span id="timestamp" >' . $this->getSortImage('timestamp').'</span>';
		$headline[3]["dat"] = $GLOBALS['l_versions']['modifications'];
		$headline[4]["dat"] = (we_hasPerm("ADMINISTRATOR")) ? '<div style="margin:0px 0px 10px 3px;" id="deleteButton">'.$we_button->create_button("image:btn_function_trash", "javascript:deleteVers();").'</div>'.we_forms::checkbox("1", 0, "deleteAllVersions", $markText, false, "middlefont", "checkAll();") : getPixel(1,1);
	...
	*/
	function getTextForMod($modString, $status) {
		...
		if($modString=="") return "<br/>&nbsp;<br/>".$statusTxt."<br/>";
	...
	*/
	...
	function tblListRow($content) {
		...
		for ($f=0; $f<$anz; $f++) {
			if($f==6) {
				$out .= '<td colspan="2" style="width: '.$widths[$f].'px;border-bottom:1px solid #D1D1D1;" class="middlefont" ><span id="print">' . (( isset($content[8]["dat"]) &&  $content[8]["dat"])?$content[8]["dat"]:"&nbsp;").'</span><br/></td>';
			}
			elseif($f!=6 && $f!=7) {
				$out .= '<td style="width: '.$widths[$f].'px;border-bottom:1px solid #D1D1D1;" class="middlefont">'.getPixel(1,1).'</td>';
			}
		}
		
		$out .= '<td style="width:15px;border-bottom:1px solid #D1D1D1;border-bottom:1px solid #D1D1D1;">'.getPixel(15,1).'</td>';
