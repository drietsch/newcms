...
function moveItem($targetDirectoryID,$id,$table,&$notMovedItems){
	...
	// move documents
		...
		$row = getHash("SELECT Text,Path,Published,IsFolder,Icon,ContentType FROM ".$table." WHERE ID=".abs($id),$DB_WE);
		$fileName = $row['Text'];
		$oldPath = $row['Path'];
	...
	// move Objects
		...
		$row = getHash("SELECT TableID,Path,Text,IsFolder,Icon,ContentType FROM ".$table." WHERE ID=".abs($id),$DB_WE);
		$tableID = $row['TableID'];
		$oldPath = $row['Path'];
		$fileName = $row['Text'];
		$isFolder = ($row["IsFolder"] == 1 ? true : false);
		...
		$q = "UPDATE ".$table." SET ParentID=".$parentID.", Path='".$newPath."/".$fileName."' WHERE ID=".abs($id);
		$DB_WE->query($q);
		$q = "UPDATE ".OBJECT_X_TABLE.$tableID." SET OF_ParentID=".$parentID.", OF_Path='".$newPath."/".$fileName."' WHERE OF_ID=".abs($id);
		$DB_WE->query($q);
