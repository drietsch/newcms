...
class we_import_files{
	...
	function _getJS($fileinput){
		...
		$js .= 'var we_fileinput = \'<form name="we_upload_form_WEFORMNUM" method="post" action="'.WEBEDITION_DIR.'we_cmd.php" enctype="multipart/form-data" target="imgimportbuttons">'.ereg_replace("[\n\r]"," ",$this->_getHiddens("buttons",$this->step+1)).$fileinput.'</form>\';
				...
				function uploadFinished() {
					refreshTree();
					' . we_message_reporting::getShowMessageCall($GLOBALS["l_import_files"]["finished"], WE_MESSAGE_NOTICE) . '
				}
	...
	function getStep1() {
		...
		$button = $we_button->create_button("select", "javascript:we_cmd('openDirselector',document.we_startform.importToID.value,'".FILE_TABLE."','document.we_startform.importToID.value','document.we_startform.egal.value','','','0')");
		$content = hidden('we_cmd[0]', 'import_files') . hidden('cmd', 'content') . hidden('step', '2'); // fix for categories require reload!
		$content .= we_htmlElement::htmlHidden(array('name'=>'categories','value'=>''));
		...
		$content .= we_multiIconBox::getHTML("weimportfiles","99%",$parts,30,"",$foldAt,$GLOBALS["l_import_files"]["image_options_open"],$GLOBALS["l_import_files"]["image_options_close"],($wepos=="down"),$GLOBALS["l_import_files"]["step1"]);
		$startsrceen = we_htmlElement::htmlDiv(array("id"=>"start"),we_htmlElement::htmlForm(array(
	...
	function getStep2() {
		...
		$but = we_htmlElement::htmlImg(array("src"=>IMAGE_DIR."pixel.gif","width"=>"10","height"=>"22","border"=>"0","align"=>"absmiddle"));
		$but .= we_htmlElement::htmlImg(array("src"=>IMAGE_DIR.'/button/btn_function_trash.gif',"width"=>"27","height"=>"22","border"=>"0","align"=>"absmiddle", "onMouseDown"=>"we_trashButDown(this)", "onMouseUp"=>"we_trashButUp(this)", "onMouseOut"=>"we_trashButUp(this)","unselectable"=>"on","style"=>"-moz-user-select: none;display: none;cursor:pointer;", "id"=>"trash_WEFORMNUM", "onclick"=>"wedelRow(WEFORMNUM + 1,this)"));
		$but = ereg_replace("[\n\r]"," ",$but);
		...
		$content = 	hidden('we_cmd[0]', 'import_files') . hidden('cmd', 'content') . hidden('step', '2') .
					we_htmlElement::htmlDiv(array('id'=>'desc'),htmlAlertAttentionBox(sprintf($GLOBALS["l_import_files"]["import_expl"],$maxsize),2,"520",false)) .
					we_htmlElement::htmlDiv(array('id'=>'descJupload','style'=>'display:none;'),htmlAlertAttentionBox(sprintf($GLOBALS["l_import_files"]["import_expl_jupload"],$maxsize) ,2,"520",false));
		...
		$fileinput = we_htmlElement::htmlInput(array( "name"=>"we_File","type"=>"file","size"=>"40", "onclick"=>"checkFileinput();", "onchange"=>"checkFileinput();")).$but;

		$fileinput = '<table><tr><td valign="top" class="weMultiIconBoxHeadline">' . $GLOBALS["l_import_files"]["file"] . '&nbsp;<span id="uploadFiles_headline_WEFORMNUM">WE_FORM_NUM</span></td><td>'.getPixel(35,5).'</td><td>'.$fileinput.'</td></tr></table>';
		...
		$formhtml = we_htmlElement::htmlForm(array(
														"action"=>WEBEDITION_DIR."we_cmd.php",
		...
		$content = we_htmlElement::htmlDiv(
									...
									(getPref('use_jupload') ? we_htmlElement::htmlForm(array("name"=>"JUploadForm"),"") : "") .
									we_htmlElement::htmlForm(array(
															...
															$this->_getHiddens()) .
									we_multiIconBox::getHTML("uploadFiles","100%",$parts,30,"",-1,"","","",$GLOBALS["l_import_files"]["step2"])
															);




		$body = we_htmlElement::htmlBody(array("class"=>"weDialogBody","onMouseMove"=>"if(typeof(document.JUpload)=='undefined') checkFileinput(); else setApplet();","onload"=>"if(typeof(document.JUpload)!='undefined') setApplet();"),
										$content);
	...
	function getStep3() {
		...
		if(isset($_SESSION["WE_IMPORT_FILES_ERRORs"])){
			...
			$parts[] = array(
				'html' => htmlAlertAttentionBox(sprintf(str_replace('\n','<br>',$GLOBALS["l_import_files"]["error"]),$filelist),1,"520",false)
			);
		...
		$content = we_htmlElement::htmlForm(array(	"action"=>WEBEDITION_DIR."we_cmd.php",
													"name"=>"we_startform",
													"method"=>"post"
											),
											we_htmlElement::htmlHidden(array('name'=>'step','value'=>'3')).
											we_multiIconBox::getHTML("uploadFiles","100%",$parts,30,"",-1,"","","",$GLOBALS["l_import_files"]["step3"]) // bugfix 1001
		);
...
';
		...
		if(isset($_SESSION["WE_IMPORT_FILES_ERRORs"]) && $formnum == $formcount && $formnum != 0){
			...
			unset($_SESSION["WE_IMPORT_FILES_ERRORs"]);
			$js .=	"			" . we_message_reporting::getShowMessageCall( sprintf($GLOBALS["l_import_files"]["error"],$filelist), WE_MESSAGE_ERROR ) . "\n";
		}else{
			$js .=	"			" . we_message_reporting::getShowMessageCall($GLOBALS["l_import_files"]["finished"], WE_MESSAGE_NOTICE) . "\n";
		}
		...
		$table->setCol(0, 1, array("align"=>"right"),
			we_htmlElement::htmlDiv(array('id'=>'normButton'),$we_button->position_yes_no_cancel($prevNextButtons,null,$cancelButton,10,'',array(),10)) .
			we_htmlElement::htmlDiv(array('id'=>'juButton','style'=>'display:none;'),$we_button->position_yes_no_cancel($prevButton2,null,$closeButton,10,'',array(),10))
		);

		if($this->step == 3){
			$table->setCol(0, 0, null, '');
			$table->setCol(0, 1, array("align"=>"right"),
				we_htmlElement::htmlDiv(array('id'=>'normButton'),$we_button->position_yes_no_cancel($prevButton2,null,$closeButton,10,'',array(),10))
			);
		}
	...
	function importFile(){
		if (isset($_FILES['we_File']) && strlen($_FILES['we_File']["tmp_name"])){
			...
			if(!we_hasPerm($GLOBALS["WE_CONTENT_TYPES"][$ct]["Permission"])){
				return array("filename"=>$_FILES['we_File']["name"],"error"=>"no_perms");
			}
			...
			if(!@move_uploaded_file($_FILES['we_File']["tmp_name"],$tempName)){
				return array("filename"=>$_FILES['we_File']["name"],"error"=>"move_file_error");
			}
		    ...
		    if($file_id = f("SELECT ID FROM ".FILE_TABLE." WHERE Path='".$we_doc->Path."'","ID",$GLOBALS["DB_WE"])){
				if($this->sameName=="overwrite"){
					eval('$we_doc=new '.$we_doc->ClassName.'();');
					$we_doc->initByID($file_id,FILE_TABLE);
				}else if($this->sameName=="rename"){
					$z=0;
					$footext = $we_doc->Filename."_".$z.$we_doc->Extension;
					while(f("SELECT ID FROM ".FILE_TABLE." WHERE Text='$footext' AND ParentID='".$this->importToID."'","ID",$GLOBALS["DB_WE"])){
						$z++;
						$footext = $we_doc->Filename."_".$z.$we_doc->Extension;
					}
					$we_doc->Text = $footext;
			...
			if($we_ContentType == "image/*"){
				...
				if(
					($newWidth && ($newWidth != $we_doc->getElement("origwidth"))) ||
					($newHeight && ($newHeight != $we_doc->getElement("origheight")))
					){
				...
				if($this->degrees){
						$we_doc->rotateImage(($this->degrees % 180 == 0) ? $we_doc->getElement("origwidth") : $we_doc->getElement("origheight"), ($this->degrees % 180 == 0) ? $we_doc->getElement("origheight") : $we_doc->getElement("origwidth"), $this->degrees, $this->quality);
				}
	...
	function _getHiddens(){
		$content = we_htmlElement::htmlHidden(array("name"=>"we_cmd[0]","value"=>"import_files"));
		$content .= we_htmlElement::htmlHidden(array("name"=>"cmd","value"=>"buttons"));
		$content .= we_htmlElement::htmlHidden(array("name"=>"step","value"=>"1"));
		$content .= we_htmlElement::htmlHidden(array("name"=>"weFormNum","value"=>"0"));
		$content .= we_htmlElement::htmlHidden(array("name"=>"weFormCount","value"=>"0"));
		$content .= we_htmlElement::htmlHidden(array("name"=>"importToID","value"=>$this->importToID));
		$content .= we_htmlElement::htmlHidden(array("name"=>"sameName","value"=>$this->sameName));
		$content .= we_htmlElement::htmlHidden(array("name"=>"thumbs","value"=>$this->thumbs));
		$content .= we_htmlElement::htmlHidden(array("name"=>"width","value"=>$this->width));
		$content .= we_htmlElement::htmlHidden(array("name"=>"height","value"=>$this->height));
		$content .= we_htmlElement::htmlHidden(array("name"=>"widthSelect","value"=>$this->widthSelect));
		$content .= we_htmlElement::htmlHidden(array("name"=>"heightSelect","value"=>$this->heightSelect));
		$content .= we_htmlElement::htmlHidden(array("name"=>"keepRatio","value"=>$this->keepRatio));
		$content .= we_htmlElement::htmlHidden(array("name"=>"degrees","value"=>$this->degrees));
		$content .= we_htmlElement::htmlHidden(array("name"=>"quality","value"=>$this->quality));
		$content .= we_htmlElement::htmlHidden(array("name"=>"categories","value"=>$this->categories));
	...
	function _getFrameset(){
		...
		$frameset->addFrame(array("src"=>WEBEDITION_DIR."we_cmd.php?we_cmd[0]=import_files&cmd=content" . ($_step>-1 ? '&step=' . $_step : ''),"name"=>"imgimportcontent","scrolling"=>"auto","noresize"=>null));
		$frameset->addFrame(array("src"=>WEBEDITION_DIR."we_cmd.php?we_cmd[0]=import_files&cmd=buttons". ($_step>-1 ? '&step=' . $_step : ''),"name"=>"imgimportbuttons","scrolling"=>"no"));
	...
	function getHTMLCategory() {
		...
		$table = new we_htmlTable(array('id'=>'CategoriesBlock','style'=>'display: block;','cellpadding' => 0,'cellspacing' => 0,'border'=>0),4,1);

		$table->setColContent(0,0,getPixel(5,5));
		$table->setColContent(1,0,we_htmlElement::htmlDiv(array('id'=>'categoriesDiv','class'=>'blockWrapper','style'=>'width: '.($_width_size).'px; height: 60px; border: #AAAAAA solid 1px;')));
		$table->setColContent(2,0,getPixel(5,5));
		$table->setCol(3,0,array('colspan'=>'2','align'=>'right'),
