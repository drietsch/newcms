...
if(!isset($_SESSION["user"])) {
	$_SESSION["user"] = array("ID" => "","Username" => "","workSpace" => "", "isWeSession" => false);
}

if((isset($_POST["username"]) && isset($_POST["md5password"])) && isset($_SESSION["we_secret"])) {

	$_userdata = getHash("SELECT passwd, username, LoginDenied, ID FROM " . USER_TABLE . " WHERE username='".addslashes($_POST["username"])."'", $DB_WE);

	$hash = (isset($_userdata["passwd"]) && isset($_SESSION["we_secret"])) ? md5($_userdata["passwd"] . $_SESSION["we_secret"]) : "";
	...
	if ($hash !== "" && $hash === $_POST["md5password"]) {
		if ( $_userdata["LoginDenied"] ) { // userlogin is denied 
			...
			$DB_WE->query((isset($GLOBALS["BIG_USER_MODULE"]) && $GLOBALS["BIG_USER_MODULE"] && in_array("busers",$GLOBALS["_pro_modules"])) ?
								("SELECT ParentID,workSpace,workSpaceTmp,workSpaceNav,workSpaceObj,workSpaceNwl,ParentWs,ParentWst,ParentWsn,ParentWso,ParentWsnl FROM ".USER_TABLE." WHERE ID=".$_SESSION["user"]["ID"]." OR Alias=".$_SESSION["user"]["ID"]) :
								 "SELECT workSpace FROM ".USER_TABLE." WHERE ID=".$_SESSION["user"]["ID"]);
			while($DB_WE->next_record()) {
				// get workspaces
				$a=makeArrayFromCSV($DB_WE->f("workSpace"));
				foreach($a as $k=>$v) if(!in_array($v,$f)) array_push($f,$v);
				if(isset($GLOBALS["BIG_USER_MODULE"]) && $GLOBALS["BIG_USER_MODULE"] && in_array("busers",$GLOBALS["_pro_modules"])) {
					...
					while($pid) {	//	For each group
						...
						$db_tmp->query("SELECT ParentID,workSpace,workSpaceTmp,workSpaceNav,workSpaceObj,workSpaceNwl,ParentWs,ParentWst,ParentWsn,ParentWso,ParentWsnl FROM ".USER_TABLE." WHERE ID=".$pid);
						if($db_tmp->next_record()) {
							...
							if($get_wst) {
								...
								foreach($a as $k=>$v) if(!in_array($v,$t)) array_push($t,$v);
							}
							if($get_wso) {
								$a=makeArrayFromCSV($db_tmp->f("workSpaceObj"));
								foreach($a as $k=>$v) if(!in_array($v,$o)) array_push($o,$v);
							}
							if($get_wsn) {
								$a=makeArrayFromCSV($db_tmp->f("workSpaceNav"));
								foreach($a as $k=>$v) if(!in_array($v,$n)) array_push($n,$v);
							}
							if($get_wsnl) {
								$a=makeArrayFromCSV($db_tmp->f("workSpaceNwl"));
								foreach($a as $k=>$v) if(!in_array($v,$nl)) array_push($nl,$v);
							}
