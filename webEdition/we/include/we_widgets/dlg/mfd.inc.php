...
function save(){
	if(isNoError()) {
		...
		" . we_message_reporting::getShowMessageCall($l_cockpit['no_type_selected'], WE_MESSAGE_ERROR) . "
	}
}
...
if(sizeof($users)){
	...
	for($i=0;$i<sizeof($users);$i++){
		$foo = getHash("SELECT ID,Path,Icon from ".USER_TABLE." WHERE ID='".$users[$i]."'",$db);
		$content .= '<tr><td><img src="'.ICON_DIR.$foo["Icon"].'" width="16" height="18"></td><td class="defaultfont">'.$foo["Path"].'</td>'.
		'<td>'.$we_button->create_button("image:btn_function_trash","javascript:delUser('".$users[$i]."');").'</td></tr>'."\n";
	}
}else{
...
$sUsrContent = '<table border="0" cellpadding="0" cellspacing="0" width="300"><tr><td>'.
	we_htmlElement::htmlDiv(array("class"=>"multichooser"),$content).we_htmlElement::htmlHidden(array("name"=>"UserNameTmp","value"=>"")).we_htmlElement::htmlHidden(array("name"=>"UserIDTmp","value"=>"")).
	'</td></tr><tr><td align="right">'.getPixel(2,8).we_htmlElement::htmlBr().$we_button->create_button_table(array($delallbut,$addbut)).'</td></tr></table>';
...
if(we_hasPerm('CAN_SEE_DOCUMENTS')){
	$oChbxDocs = we_forms::checkbox($value=0,$checked=$sType{0},$name="chbx_type",$text=$l_cockpit['documents'],$uniqid=true,$class="defaultfont",$onClick="",$disabled=!(defined('FILE_TABLE') && we_hasPerm("CAN_SEE_DOCUMENTS")),$description="",$type=0,$width=0);
} else {
	$oChbxDocs = "";
}
if(we_hasPerm('CAN_SEE_TEMPLATES') && $_SESSION["we_mode"] != "seem"){
	$oChbxTmpl = we_forms::checkbox($value=0,$checked=$sType{1},$name="chbx_type",$text=$l_cockpit['templates'],$uniqid=true, $class="defaultfont",$onClick="",$disabled=!(defined("TEMPLATES_TABLE") && we_hasPerm('CAN_SEE_TEMPLATES')),$description="",$type=0,$width=0);
} else {
	$oChbxTmpl = "";
}
if(we_hasPerm('CAN_SEE_OBJECTS')){
	$oChbxObjs = we_forms::checkbox($value=0,$checked=$sType{2},$name="chbx_type",$text=$l_cockpit['objects'],$uniqid=true,$class="defaultfont",$onClick="",$disabled=!(defined("OBJECT_FILES_TABLE") && we_hasPerm('CAN_SEE_OBJECTFILES')),$description="",$type=0,$width=0);
} else {
	$oChbxObjs = "";
}
if(we_hasPerm('CAN_SEE_CLASSES') && $_SESSION["we_mode"] != "seem"){
	$oChbxCls = we_forms::checkbox($value=0,$checked=$sType{3},$name="chbx_type",$text=$l_cockpit['classes'],$uniqid=true,$class="defaultfont",$onClick="",$disabled=!(defined("OBJECT_TABLE") && we_hasPerm('CAN_SEE_OBJECTS')),$description="",$type=0,$width=0);
} else {
	$oChbxCls = "";
}

$oDbTableType = new we_htmlTable(array("border"=>"0","cellpadding" =>"0","cellspacing"=>"0"),1,3);
...
$oSctDate = new we_htmlSelect(array("name"=>"sct_date","size"=>"1","class"=>"defaultfont","onChange"=>""));
$aLangDate = array($l_cockpit['all'],$l_cockpit['today'],$l_cockpit['last_week'],$l_cockpit['last_month'],$l_cockpit['last_year']);
foreach ($aLangDate as $k=>$v) {
	$oSctDate->insertOption($k,$k,$v);
}
$oSctDate->selectOption($iDate);

$oChbxShowMfdBy = we_forms::checkbox($value=0,$checked=$sDisplayOpt{0},$name="chbx_display_opt",$text=$l_cockpit['modified_by'],$uniqid=true,
	$class="defaultfont",$onClick="",$disabled=false,$description="",$type=0,$width=0);
$oChbxShowDate = we_forms::checkbox($value=0,$checked=$sDisplayOpt{1},$name="chbx_display_opt",$text=$l_cockpit['date_last_modification'],$uniqid=true,
	$class="defaultfont",$onClick="",$disabled=false,$description="",$type=0,$width=0);
$oSctNumEntries = new we_htmlSelect(array("name"=>"sct_amount_entries","size"=>"1","class"=>"defaultfont"));
...
$oSelMaxEntries = new we_htmlTable(array("height"=>"100%","border"=>"0","cellpadding" =>"0","cellspacing"=>"0"),1,3);
$oSelMaxEntries->setCol(0,0,array("valign"=>"middle","class"=>"defaultfont"),$l_cockpit['max_amount_entries']);
$oSelMaxEntries->setCol(0,1,null,getPixel(5,1));
$oSelMaxEntries->setCol(0,2,array("valign"=>"middle"),$oSctNumEntries->getHTMLCode());

$show = $oSelMaxEntries->getHTMLCode().getPixel(1,5).$oChbxShowMfdBy.$oChbxShowDate.getPixel(1,5).we_htmlElement::htmlBr().$oShowUser;

array_push($parts,array("headline"=>$l_cockpit['type'],"html"=>$oDbTableType->getHTMLCode(),"space"=>80));
array_push($parts,array("headline"=>$l_cockpit['date'],"html"=>$oSctDate->getHtmLCode(),"space"=>80));
array_push($parts,array("headline"=>$l_cockpit['display'],"html"=>$show,"space"=>80));
...
$sTblWidget = we_multiIconBox::getHTML("mfdProps","100%",$parts,30,$buttons,-1,"","","",$l_cockpit['last_modified'],"",390);

print we_htmlElement::htmlHtml(
	we_htmlElement::htmlHead(
		we_htmlElement::htmlTitle($l_cockpit['last_modified']).
		STYLESHEET.
