...
if ($bTypeDoc && $bTypeObj){
	if (defined("FILE_TABLE")) $tbls[] = FILE_TABLE;
	if (defined("OBJECT_FILES_TABLE") && we_hasPerm("CAN_SEE_OBJECTFILES")) $tbls[] = OBJECT_FILES_TABLE;
} else {
...
foreach ($tbls as $table) {
	...
	while($DB_WE->next_record()) {
		$row = array();
		$_cont[$DB_WE->f("ModDate")] = $path = '<tr><td width="20" height="20" valign="middle" nowrap><img src="'.ICON_DIR.$DB_WE->f("Icon").'" width="16" height="18">'.getpixel(4,1).'</td>'.
