...
class we_browserDetect{
	...
	function we_browserDetect($ua=""){
		...
		if(eregi('^([^ ]+) ([^\(]*)(\([^\)]+\))(.*)$',$this->ua,$regs)){
			...
			if(strtolower($bez) == "lynx"){
				$this->br="lynx";
			}else if(strtolower($bez) == "mozilla"){
				if(eregi('msie (.*)$',trim($brArr[1]),$regs) && (trim($post) == "" || eregi('.net',$post))){
					$this->br = "ie";
					$this->v = $regs[1];
					$this->_getSys($bracket);
				}else if(eregi('konqueror/(.*)$',trim($brArr[1]),$regs)){
					$this->br = "konqueror";
					$this->v = $regs[1];
					$this->_getSys($bracket);
				}else if(eregi('galeon/(.*)$',trim($brArr[1]),$regs)){
					$this->br = "unknown";
					$this->v = $regs[1];
					$this->_getSys($bracket);
				}else{
					if(eregi('netscape6',$post)){
						...
						$this->_getSys($bracket);
					}else if(eregi(' AppleWebKit/([0-9\.]+)',$post,$regs)){
						$this->v = $regs[1];
						$this->br="appleWebKit";
						$this->_getSys($bracket);
					}else if(eregi('safari',$post)){
						...
						$this->_getSys($bracket);
					}else if(eregi(' firefox/([0-9\.]+)',$post,$regs)){
						$this->v = $regs[1];
						$this->br="firefox";
						$this->_getSys($bracket);
					}else if(eregi('gecko',$post)){
						$this->br="mozilla";
						if(eregi('rv:([0-9\.]*)',$bracket,$regs)){
							$this->v = $regs[1];
						}
						$this->_getSys($bracket);
					}else if(eregi('opera ([^ ]+)',$post,$regs)){
						$this->br="opera";
						$this->v = $regs[1];
						$this->_getSys($bracket);
					}else if($brArr[0] == "compatible"){
						$this->br="unknown";
					...
					}else if(!eregi('msie',$bracket)){
						$this->br="nn";
						$this->v = ereg_replace('[^0-9\.]','',$prever);
						$this->_getSys($bracket);
					}
				}
			}else if(strtolower($bez) == "opera"){
				$this->br="opera";
				$this->v = $prever;
				$this->_getSys($bracket);
			}else if(strtolower($bez) == "googlebot"){
				$this->br="unknown";
				#$this->v=$prever;
			}else if(strtolower($bez) == "nokia-communicator-www-Browser"){
				$this->br="unknown";
			}
			if($this->sys=="unknown"){
		...
		}else if(eregi('^lynx([^a-zA-Z]+)[a-zA-Z].*',$ua,$regs)){
			$this->br="lynx";
	...
	function _getSys($bracket){
		if(eregi('mac',$bracket)){
			$this->sys = "mac";
		}else if(eregi('win',$bracket)){
			$this->sys = "win";
		}else if(eregi('linux',$bracket) || eregi('x11',$bracket) || eregi('sun',$bracket)){
			$this->sys = "unix";
		}
	}
