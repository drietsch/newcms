...
class weNavigation extends weModelBase{
	...
	*/
	...
	function _getFilterOfDocument() {
		...
		if ($this->IsFolder) {
			if ($this->FolderSelection == "objLink") {
				...
				$_id = $this->LinkID;
			} else if ($this->FolderSelection == "docLink") {
				$_table = FILE_TABLE;
				$_id = $this->LinkID;
			}
		} else {
			if ($this->SelectionType == "objLink") {
				$_table = OBJECT_FILES_TABLE;
				$_id = $this->LinkID;
			} else if ($this->SelectionType == "docLink") {
				$_table = FILE_TABLE;
				$_id = $this->LinkID;
			}
		}
	...
	function save($order=true) {
		...
		if($order) {
			$_ord_count = f('SELECT COUNT(ID) as OrdCount FROM '.NAVIGATION_TABLE.' WHERE ParentID='.$this->ParentID.';','OrdCount',$this->db);
			if($this->ID==0){
				...
				$_oldPid = f('SELECT ParentID FROM '.NAVIGATION_TABLE.' WHERE ID='.$this->ID,'ParentID',$this->db);
			}
		}
		...
		if($this->isnew && $GLOBALS['weNavigationCacheDeleteAfterAdd']) {
			$ClearCache = true;
		} else if(!$this->isnew && $GLOBALS['weNavigationCacheDeleteAfterEdit']) {
			$ClearCache = true;
		}
	...
	function alnumNotValid($text){
		if(ctype_alnum($text)) {return false;} else {return true;}
	}
	...
	function isSelf(){
		if($this->ID) {
			...
			while($_parentid!=0) {
				...
				$_parentid = f('SELECT ParentID FROM ' . NAVIGATION_TABLE .' WHERE ID=' . $_parentid,'ParentID',$this->db);
				$_count++;
				if($_count==9999) {
					return false;
				}
	...
	function getDynamicEntries() {
		if($this->Selection=='dynamic'){
			...
			} else if($this->SelectionType=='category') {
				return weDynList::getCatgories($this->FolderID,$this->ShowCount);
			} else {
				
				
				return $this->ClassID > 0 ? weDynList::getObjects($this->ClassID,$this->FolderID,$this->Categories,$this->Sort,$this->ShowCount,$this->TitleField) : array();
			}
		}
	}
	...
	function getChilds() {
		...
		$this->db->query('SELECT ID,Path,Text,Ordn FROM '.NAVIGATION_TABLE.' WHERE ParentID='.$this->ID.' ORDER BY Ordn;');
	...
	function getDynamicChilds() {
		...
		$this->db->query('SELECT ID,Ordn FROM '.NAVIGATION_TABLE.' WHERE ParentID='.$this->ID.' AND IsFolder=0 AND Depended=1 ORDER BY Ordn;');
	...
	function hasDynChilds() {
		if($this->ID) {
			$_dep = f('SELECT COUNT(ID) as Navi FROM ' . NAVIGATION_TABLE . ' WHERE ParentID='.$this->ID.' AND Depended=1;','Navi',$this->db);
			...
			if($item['ParentID'] == $this->ID) {
				...
				if($_nav->IsFolder==0 && $_nav->Selection=='dynamic') {
					...
					foreach ($_dyn_items as $_dyn) {
						...
						if($rules) {
							$_items[(sizeof($_items)-1)]['currentRule'] =
									weNavigationRule::getWeNavigationRule(
	...
	function reorderUp() {
		if($this->ID) {
			if($this->Ordn>0){
				$_db = new DB_WE();
				$_parentid = f('SELECT ParentID FROM '.NAVIGATION_TABLE.' WHERE ID='.$this->ID.';','ParentID',$_db);
				$_db->query('UPDATE '.NAVIGATION_TABLE.' SET Ordn='.$this->Ordn.' WHERE ParentID='.$_parentid.' AND Ordn='.($this->Ordn-1).';');
				$this->Ordn--;
				$this->saveField('Ordn');
				$this->reorder($this->ParentID);
				return true;
	...
	function reorderDown() {
		if($this->ID) {
			$_parentid = f('SELECT ParentID FROM '.NAVIGATION_TABLE.' WHERE ID='.$this->ID.';','ParentID',$this->db);
			$_num = f('SELECT COUNT(ID) as OrdCount FROM '.NAVIGATION_TABLE.' WHERE ParentID='.$_parentid.';','OrdCount',$this->db);
			if($this->Ordn<($_num-1)){
				$_db = new DB_WE();
				$_db->query('UPDATE '.NAVIGATION_TABLE.' SET Ordn='.$this->Ordn.' WHERE ParentID='.$this->ParentID.' AND Ordn='.($this->Ordn+1).';');
				$this->Ordn++;
				$this->saveField('Ordn');
				$this->reorder($this->ParentID);
				return true;
	...
	function getHref(&$storage,$id=0){
		...
		if($this->IsFolder){
			...
			if($this->SelectionType=='urlLink'){
				$_path = $this->Url;
			} else if($this->SelectionType=='category' || $this->SelectionType=='catLink') {
		...
		$_path = $_path . ($_param!='' ? ((strpos($_path,'?')===false ? '?' : '&amp;') . $_param) : '') . ((isset($this->Attributes['anchor']) && !empty($this->Attributes['anchor'])) ? ('#'.$this->Attributes['anchor']) : '');
	...
	function setOrdn($num) {
		$_db = new DB_WE();
		if($this->ID) {
			$_db->query('SELECT ID FROM '.NAVIGATION_TABLE.' WHERE ParentID='.$this->ParentID.' AND Ordn>='.$num.' ORDER BY Ordn;');
			while($_db->next_record()) {
				$this->db->query('UPDATE '.NAVIGATION_TABLE.' SET Ordn='.($_db->f('Ordn')+1).' WHERE ID='.$_db->f('ID').';');
			}
			$this->Ordn=$num;
			$this->saveField('Ordn');
		}
	...
	function getNavCondition($id, $table) {
		...
		return ' ((IsFolder=1 AND FolderSelection="' . $_linkType . '") OR (IsFolder=0 AND SelectionType="' . $_linkType . '")) AND LinkID=' . abs($id) . ' ';
	}
}
