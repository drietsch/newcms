	...
	*/
	class weNavigationItem {
	    ...
	    function weNavigationItem($id, $docid, $table, $text, $display, $href, $type, $icon, $attributes, $limitaccess, $customers = "") {
			...
			if($this->table==FILE_TABLE) {
				...
				if ($__id) {
					$_v = f('SELECT ID FROM ' . FILE_TABLE . ' WHERE ID=' . $__id . ' AND Published>0','ID',new DB_WE());
	    ...
	    function isCurrent($weNavigationItems) {
			...
			} else if (isset($GLOBALS["WE_MAIN_DOC"]) && (!isset($GLOBALS["WE_MAIN_DOC"]->TableID)) && $this->table == FILE_TABLE) {
    ...
    function getNavigationField($attribs) {
	    	...
	    	if(isset($attribs['name'])) {
	    		...
	    		} else if (isset($this->attributes[$fieldname]) && $this->attributes[$fieldname]!='') {
	    			if($fieldname=='title') {
	    	...
	    	if(isset($attribs['complete'])) {
	    		...
	    		if(
	    				(($_compl=='link' && isset($this->text)) ||
	    				($_compl=='image' && isset($this->icon) && $this->icon!='/'))
	    		) {
	    				unset($attribs['complete']);
	    ...
	    function getNavigationFieldAttributes($attribs) {
	    	if(isset($attribs['attributes'])) {
	    		...
	    		foreach ($_fields as $_field) {
		    		switch ($_field) {
			    		case 'link':
			    			$useFields = array('href' , 'title', 'target', 'lang', 'hreflang', 'accesskey', 'tabindex', 'rel', 'rev');
							foreach ($useFields as $field) {
								if (isset($this->$field) && $this->$field!='') {
									...
									//$attribs[$field] = $this->$field;
								} else if(isset($this->attributes[$field]) && $this->attributes[$field]!='') {
			    		...
			    		default:
							...
							} else if(isset($this->attributes[$_field]) && $this->attributes[$_field]!='') {
		...
		function getPopupJs(&$attributes) {
			...
			}else if($this->attributes['popup_xposition']!='' || $this->attributes['popup_yposition']!=''){
			...
			$attributes = removeAttribs($attributes, array('name','target','href','onClick','onclick'));
	...
	*/
	class weNavigationItems {
			...
			if(is_array($_dyn_items)) {
				foreach ($_dyn_items as $_dyn) {
					...
					$_items[] = array(
						...
						'name'=>!empty($_dyn['field']) ? $_dyn['field'] : (isset($_dyn['name']) && !empty($_dyn['name']) ? $_dyn['name'] : $_dyn['text']),
						'docid'=>$_dyn['id'],
		...
		function initById($parentid=0, $depth=false, $showRoot=true) {
			...
			$_navigation->initByRawData(
				$_item ? $_item : array('ID'=>0,'Path'=>'/')
			);
		...
		function getTemplate($item) {
			...
			} else if(isset($useTemplate['defaultCurrent'])){
...
//			$rootTemplate = '<we:navigationEntries />';
		...
		function readItemsFromDb($id) {
			...
			while ($_db->next_record()) {
				...
				} else  if($_db->Record['Selection']=='static' && $_db->Record['SelectionType']=='docLink') {
					$_ids[] = $_db->Record['LinkID'];
				} else if(($_db->Record['SelectionType']=='category' || $_db->Record['SelectionType']=='catLink') && $_db->Record['LinkSelection']!='extern') {
