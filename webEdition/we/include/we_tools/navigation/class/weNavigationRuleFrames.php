	...
	function getHTMLContent() {
		...
		array_push($parts, array(
							...
							'html' => $yuiSuggest->getYuiJsFiles().'<table border="0" cellpadding="0" cellspacing="0">
										<tr><td>' . htmlSelect('navigationRules', $_rules, 8, '', false, ' style="width: 275px;" onclick="we_cmd(\'edit_navigation_rule\', this.value)"') .'</td>
											<td>' . getPixel(10,1) . '</td>
											<td valign="top">
												' . $we_button->create_button('new_entry', 'javascript:we_cmd("new_navigation_rule")') . '<div style="height:10px;"></div>
												' . $we_button->create_button('delete', 'javascript:we_cmd("delete_navigation_rule")') . '
											</td>
										</tr>
										</table>'
							)
		...
		$yuiSuggest->setSelectButton($we_button->create_button('select', "javascript:we_cmd('openSelector', document.we_form.elements['NavigationID'].value, '" . NAVIGATION_TABLE . "', 'document.we_form.elements[\\'NavigationID\\'].value', 'document.we_form.elements[\\'NavigationIDPath\\'].value')"),10);
		...
		array_push($parts, array(
							...
							'space' => 200,
							'html' => htmlSelect('SelectionType', $selectionTypes, 1, 0, false, "style=\"width: 275px;\" onchange=\"switchType(this.value);\"")
							)
		);

		// getDoctypes
		$docTypes = array(0=>$GLOBALS['l_navigation']['no_entry']);
		...
		$yuiSuggest->setWidth(275);
		$yuiSuggest->setSelectButton($we_button->create_button('select', "javascript:we_cmd('openDirselector', document.we_form.elements['FolderID'].value, '" . FILE_TABLE . "', 'document.we_form.elements[\\'FolderID\\'].value', 'document.we_form.elements[\\'FolderIDPath\\'].value')"),10);
		$yuiSuggest->setTrashButton($we_button->create_button("image:function_trash", "javascript:document.we_form.elements['FolderID'].value = '';document.we_form.elements['FolderIDPath'].value = '';"),10);
...
<tr id="trDoctypeID">
	...
	<td>' . htmlSelect('DoctypeID', $docTypes, 1, 0, false, "style=\"width: 275px;\"") . '</td>
</tr>';
		...
		if (defined('OBJECT_TABLE')) {
			...
			$yuiSuggest->setWidth(275);
			$yuiSuggest->setSelectButton($we_button->create_button('select', "javascript:we_cmd('openDocselector', document.we_form.elements['ClassID'].value, '" . OBJECT_TABLE . "', 'document.we_form.elements[\\'ClassID\\'].value', 'document.we_form.elements[\\'ClassIDPath\\'].value', 'top.opener.we_cmd(\"get_workspaces\");')"),10);
...
<tr id="trWorkspaceID">
	...
	<td>' . htmlSelect('WorkspaceID', array(), 1, '', false, "style=\"width: 275px;\"") . '</td>
</tr>';
		}
		$formTable .= '
<tr id="trCategories">
...
<body onload="switchType(document.we_form[\'SelectionType\'].value)" class="weDialogBody">
	...
	' . hidden('cmd', '') . '
	' . hidden('ID', '0') . '
	' . we_multiIconBox::getHTML('navigationRules', "100%", $parts,30, $we_button->position_yes_no_cancel($saveButton, null, $closeButton), -1, '', '', false, $GLOBALS['l_navigation']['rules']['navigation_rules'] ) . '
	</form>
	...
	function getHTMLCategory() {
		...
		$addbut = $we_button->create_button("add", "javascript:we_cmd('openCatselector','','" . CATEGORY_TABLE . "','','','fillIDs();opener.addCat(top.allPaths, top.allIDs);')");
		...
		$js = we_htmlElement::jsElement('',array('src'=>JS_DIR.'utils/multi_edit.js?'));
		...
		$table = new we_htmlTable(array('id'=>'CategoriesBlock','style'=>'display: block;','cellpadding' => 0,'cellspacing' => 0,'border'=>0),3,1);

		$table->setColContent(0,0,we_htmlElement::htmlDiv(array('id'=>'categories','class'=>'blockWrapper','style'=>'width: 380px; height: 80px; border: #AAAAAA solid 1px;')));
		...
		return 	$table->getHtmlCode() .
				hidden('CategoriesControl', 0) .
				hidden('CategoriesCount', 0) .
				$js .
