...
class searchtoolView extends weToolView {
  ...
  ';
	...
	function processCommands() {
        ...
        ' . $this->editorHeaderFrame . '.location="' . $this->frameset . '?pnt=edheader' . (isset ( $_REQUEST ['tabnr'] ) ? '&tab=' . $_REQUEST ['tabnr'] : '') . '&text=' . urlencode ( $this->Model->Text ) . '";
     ...
     ' );
				...
				case 'tool_weSearch_edit' :
					...
					if (! $this->Model->isAllowedForUser ()) {
						print we_htmlElement::jsElement ( we_message_reporting::getShowMessageCall ( $GLOBALS ["l_tools"] ["no_perms"], WE_MESSAGE_ERROR ) );
						$this->Model = new searchtool ( );
						$_REQUEST ['home'] = true;
						break;
					}
					print we_htmlElement::jsElement ( '
        ' . $this->editorHeaderFrame . '.location="' . $this->frameset . '?pnt=edheader' . (isset ( $_REQUEST ['cmdid'] ) ? '&cmdid=' . $_REQUEST ['cmdid'] : '') . '&text=' . urlencode ( $this->Model->Text ) . '";
        ' . $this->topFrame . '.resize.right.editor.edfooter.location="' . $this->frameset . '?pnt=edfooter";       
     ...
     ' );
				...
				case 'tool_weSearch_save' :
					...
					if (strlen ( $this->Model->Text ) > 30) {
						print we_htmlElement::jsElement ( we_message_reporting::getShowMessageCall ( $GLOBALS ['l_weSearch'] ["nameTooLong"], WE_MESSAGE_ERROR ) );
						break;
					}
					if (stristr ( $this->Model->Text, "'" ) || stristr ( $this->Model->Text, '"' )) {
						print we_htmlElement::jsElement ( we_message_reporting::getShowMessageCall ( $GLOBALS ['l_weSearch'] ["no_hochkomma"], WE_MESSAGE_ERROR ) );
						break;
					}
					
					if ($this->Model->filenameNotValid ( $this->Model->Text )) {
						print we_htmlElement::jsElement ( we_message_reporting::getShowMessageCall ( $GLOBALS ["l_tools"] ["wrongtext"], WE_MESSAGE_ERROR ) );
						break;
					}
					...
					if (trim ( $this->Model->Text ) == '') {
						print we_htmlElement::jsElement ( we_message_reporting::getShowMessageCall ( $GLOBALS ["l_tools"] ["name_empty"], WE_MESSAGE_ERROR ) );
						break;
					}
					...
					if ($this->Model->pathExists ( $this->Model->Path )) {
						print we_htmlElement::jsElement ( we_message_reporting::getShowMessageCall ( $GLOBALS ["l_tools"] ["name_exists"], WE_MESSAGE_ERROR ) );
						break;
					}
					if ($this->Model->isSelf ()) {
						print we_htmlElement::jsElement ( we_message_reporting::getShowMessageCall ( $GLOBALS ["l_tools"] ["path_nok"], WE_MESSAGE_ERROR ) );
						break;
					}
       ...
       ' . we_message_reporting::getShowMessageCall ( ($this->Model->IsFolder == 1 ? $GLOBALS ['l_weSearch'] ["save_group_ok"] : $GLOBALS ['l_weSearch'] ["save_ok"]), WE_MESSAGE_NOTICE ) . '
       ' . $this->topFrame . '.hot=0;
      ' );
						
						if (isset ( $_REQUEST ['delayCmd'] ) && ! empty ( $_REQUEST ['delayCmd'] )) {
							$js .= we_htmlElement::jsElement ( $this->topFrame . '.we_cmd("' . $_REQUEST ['delayCmd'] . '"' . ((isset ( $_REQUEST ['delayParam'] ) && ! empty ( $_REQUEST ['delayParam'] )) ? ',"' . $_REQUEST ['delayParam'] . '"' : '') . ');
        ' );
							$_REQUEST ['delayCmd'] = '';
						...
						$js = we_htmlElement::jsElement ( $js . '
       ' . $this->editorHeaderFrame . '.location.reload();
       ' . we_message_reporting::getShowMessageCall ( ($this->Model->IsFolder == 1 ? $GLOBALS ['l_weSearch'] ["save_group_failed"] : $GLOBALS ['l_weSearch'] ["save_failed"]), WE_MESSAGE_ERROR ) . '
       ' . $this->topFrame . '.hot=0;
      ' );
					}
					print $js;
   ...
   case "tool_weSearch_save":
    if(' . $this->editorBodyFrame . '.document.we_form.predefined.value==1) {
     ' . we_message_reporting::getShowMessageCall ( $GLOBALS ['l_weSearch'] ['predefinedSearchmodify'], WE_MESSAGE_ERROR ) . '
     break;
    }
   ...
   case "tool_weSearch_delete":
    if(' . $this->editorBodyFrame . '.document.we_form.predefined.value==1) {
     ' . we_message_reporting::getShowMessageCall ( $GLOBALS ['l_weSearch'] ['predefinedSearchdelete'], WE_MESSAGE_ERROR ) . '
     return;
    }   
    ...
    ' . (! we_hasPerm ( "DELETE_" . strtoupper ( $this->toolName ) ) ? (we_message_reporting::getShowMessageCall ( $GLOBALS ["l_tools"] ["no_perms"], WE_MESSAGE_ERROR )) : ('
   ...
   function search(newSearch) {
   		...
   		if('.searchtoolsearch::checkRightTempTable().' && '.searchtoolsearch::checkRightDropTable().') {
   			'.we_message_reporting::getShowMessageCall ( $GLOBALS ["l_weSearch"] ["noTempTableRightsSearch"], WE_MESSAGE_NOTICE ).'
      	}
   		else {
		    ...
		    if("' . $whichSearch . '"=="AdvSearch") {
		      ...
		      if(Checks.length==0) {
		     ' . we_message_reporting::getShowMessageCall ( $GLOBALS ["l_weSearch"] ["nothingCheckedAdv"], WE_MESSAGE_ERROR ) . '      
		    }  
		     }
		     if("' . $whichSearch . '"=="DocSearch") {
		      var m = 0;
		       ...
		       if(Checks.length==0) {
		     ' . we_message_reporting::getShowMessageCall ( $GLOBALS ["l_weSearch"] ["nothingCheckedTmplDoc"], WE_MESSAGE_ERROR ) . '      
		    }     
		     }
   ...
   function newinputAdvSearch() {
    ...
    var searchFields = "' . str_replace ( "\n", "\\n", addslashes ( htmlSelect ( 'searchFieldsAdvSearch[__we_new_id__]', $this->searchclass->getFields ( "__we_new_id__", "" ), 1, "", false, 'class="defaultfont" id="searchFieldsAdvSearch[__we_new_id__]" onChange="changeit(this.value, __we_new_id__);" ' ) ) ) . '";
    var locationFields = "' . str_replace ( "\n", "\\n", addslashes ( htmlSelect ( 'locationAdvSearch[__we_new_id__]', $this->searchclass->getLocation (), 1, "", false, 'class="defaultfont" id="locationAdvSearch[__we_new_id__]"  ' ) ) ) . '";
    var search = "' . addslashes ( htmlTextInput ( 'searchAdvSearch[__we_new_id__]', 24, "", "", " class=\"wetextinput\" id=\"searchAdvSearch[__we_new_id__]\" ", "text", 170 ) ) . '";
   ...
   function changeit(value, rowNr){
    ...
    if(value=="Content") {
     ...
     search = "' . str_replace ( "\n", "\\n", addslashes ( htmlSelect ( 'searchAdvSearch[__we_new_id__]', $this->searchclass->getDoctypes (), 1, "", false, 'class="defaultfont" style="width:170px;" id="searchAdvSearch[__we_new_id__]" ' ) ) ) . '";
     ...
     search = "' . str_replace ( "\n", "\\n", addslashes ( htmlSelect ( 'searchAdvSearch[__we_new_id__]', $this->searchclass->getFieldsStatus (), 1, "", false, 'class="defaultfont" style="width:170px;" id="searchAdvSearch[__we_new_id__]" ' ) ) ) . '";
     ...
     search = "' . str_replace ( "\n", "\\n", addslashes ( htmlSelect ( 'searchAdvSearch[__we_new_id__]', $this->searchclass->getFieldsSpeicherart (), 1, "", false, 'class="defaultfont" style="width:170px;" id="searchAdvSearch[__we_new_id__]" ' ) ) ) . '";
     ...
     locationFields = "' . str_replace ( "\n", "\\n", addslashes ( htmlSelect ( 'locationAdvSearch[__we_new_id__]', $this->searchclass->getLocation ( "date" ), 1, "", false, 'class="defaultfont" id="locationAdvSearch[__we_new_id__]"  ' ) ) ) . '";
     ...
     search = "' . str_replace("\n", "\\n", addslashes(htmlSelect('searchAdvSearch[__we_new_id__]', $this->searchclass->getModFields(), 1, "", false, 'class="defaultfont" style="width:170px;" id="searchAdvSearch[__we_new_id__]" '))) . '";
     ...
     search = "' . str_replace("\n", "\\n", addslashes(htmlSelect('searchAdvSearch[__we_new_id__]', $this->searchclass->getUsers(), 1, "", false, 'class="defaultfont" style="width:170px;" id="searchAdvSearch[__we_new_id__]" '))) . '";
   ...
   var ajaxCallbackResetVersion = {
		success: function(o) {
			...
			' . we_message_reporting::getShowMessageCall($GLOBALS['l_versions']['resetAllVersionsOK'], WE_MESSAGE_NOTICE) . '
	...
	function resetVersions() {
		...
		if(check==false) {
			' . we_message_reporting::getShowMessageCall($GLOBALS['l_versions']['notChecked'], WE_MESSAGE_NOTICE) . '
		}
		else {
  ...
  ' );
	...
	function getNextPrev($we_search_anzahl, $whichSearch) {
		...
		$select = htmlSelect ( "page", $pages, 1, $page, false, "onChange=\"this.form.elements['searchstart" . $whichSearch . "'].value = this.value;search(false);\"" );
	...
	function getSearchDialogCheckboxes($whichSearch) {
		...
		$_table = new we_htmlTable ( array ('border' => '0', 'cellpadding' => '2', 'cellspacing' => '0', 'width' => '500', 'height' => '50' ), 4, 2 );
		
		switch ( $whichSearch) {
			case "DocSearch" :
				$_table->setCol ( 0, 0, array ( ), we_forms::checkboxWithHidden ( $this->Model->searchForTextDocSearch ? true : false, "searchForTextDocSearch", $GLOBALS ['l_weSearch'] ['onlyFilename'], false, 'defaultfont', '' ) );
				
				$_table->setCol ( 1, 0, array ( ), we_forms::checkboxWithHidden ( $this->Model->searchForTitleDocSearch ? true : false, "searchForTitleDocSearch", $GLOBALS ['l_weSearch'] ['onlyTitle'], false, 'defaultfont', '' ) );
				
				$_table->setCol ( 2, 0, array ( ), we_forms::checkboxWithHidden ( $this->Model->searchForContentDocSearch ? true : false, "searchForContentDocSearch", $GLOBALS ['l_weSearch'] ['Content'], false, 'defaultfont', '' ) );
			
			break;
			case "TmplSearch" :
				$_table->setCol ( 0, 0, array ( ), we_forms::checkboxWithHidden ( $this->Model->searchForTextTmplSearch ? true : false, "searchForTextTmplSearch", $GLOBALS ['l_weSearch'] ['onlyFilename'], false, 'defaultfont', '' ) );
				
				$_table->setCol ( 1, 0, array ( ), we_forms::checkboxWithHidden ( $this->Model->searchForContentTmplSearch ? true : false, "searchForContentTmplSearch", $GLOBALS ['l_weSearch'] ['Content'], false, 'defaultfont', '' ) );
			
			break;
		}
		$_table->setCol ( 2, 1, array ('align' => 'right' ), $we_button->create_button ( "search", "javascript:search(true);" ) );
	...
	function getSearchDialogCheckboxesAdvSearch() {
		...
		$_table = new we_htmlTable ( array ('border' => '0', 'cellpadding' => '2', 'cellspacing' => '0', 'width' => '550', 'height' => '50' ), 4, 3 );
		
		if (we_hasPerm ( 'CAN_SEE_DOCUMENTS' )) {
			$_table->setCol ( 0, 0, array ( ), we_forms::checkboxWithHidden ( $this->Model->search_tables_advSearch [FILE_TABLE] ? true : false, 'search_tables_advSearch[' . FILE_TABLE . ']', $GLOBALS ['l_weSearch'] ['documents'], false, 'defaultfont', '' ) );
		}
		
		if (we_hasPerm ( 'CAN_SEE_TEMPLATES' ) && $_SESSION ["we_mode"] != "seem") {
			$_table->setCol ( 1, 0, array ( ), we_forms::checkboxWithHidden ( $this->Model->search_tables_advSearch [TEMPLATES_TABLE] ? true : false, 'search_tables_advSearch[' . TEMPLATES_TABLE . ']', $GLOBALS ['l_weSearch'] ['templates'], false, 'defaultfont', '' ) );
		}
		
		if (defined ( 'OBJECT_TABLE' )) {
			if (we_hasPerm ( 'CAN_SEE_OBJECTFILES' )) {
				$_table->setCol ( 0, 1, array ( ), we_forms::checkboxWithHidden ( $this->Model->search_tables_advSearch [OBJECT_FILES_TABLE] ? true : false, 'search_tables_advSearch[' . OBJECT_FILES_TABLE . ']', $GLOBALS ['l_weSearch'] ['objects'], false, 'defaultfont', '' ) );
			}
			if (we_hasPerm ( 'CAN_SEE_OBJECTS' ) && $_SESSION ["we_mode"] != "seem") {
				$_table->setCol ( 1, 1, array ( ), we_forms::checkboxWithHidden ( $this->Model->search_tables_advSearch [OBJECT_TABLE] ? true : false, 'search_tables_advSearch[' . OBJECT_TABLE . ']', $GLOBALS ['l_weSearch'] ['classes'], false, 'defaultfont', '' ) );
			}
		}
		
		if (we_hasPerm('SEE_VERSIONS')) {
            $_table->setCol(0, 2, array(), we_forms::checkboxWithHidden($this->Model->search_tables_advSearch[VERSIONS_TABLE] ? true : false, 'search_tables_advSearch[' . VERSIONS_TABLE . ']', $GLOBALS['l_versions']['versions'], false, 'defaultfont', ''));
        }
		
		$_table->setCol ( 1, 2, array ('align' => 'right' ), $we_button->create_button ( "search", "javascript:search(true);" ) );
	...
	function getSearchDialog($whichSearch) {
		...
		switch ( $whichSearch) {
			case "DocSearch" :
				...
				if ((isset ( $_SESSION ["weSearch"] ["keyword"] ) && $_SESSION ["weSearch"] ["keyword"] != "") && (isset ( $_REQUEST ["tab"] ) && $_REQUEST ["tab"] == 1)) {
				...
				$searchInput = htmlTextInput ( $searchTextName, 30, (isset ( $this->Model->searchDocSearch ) && is_array ( $this->Model->searchDocSearch ) && isset ( $this->Model->searchDocSearch [0] ) ? $this->Model->searchDocSearch [0] : ''), "", "", "text", 380 );
			...
			case "TmplSearch" :
				...
				$searchInput = htmlTextInput ( $searchTextName, 30, (isset ( $this->Model->searchTmplSearch ) && is_array ( $this->Model->searchTmplSearch ) && isset ( $this->Model->searchTmplSearch [0] ) ? $this->Model->searchTmplSearch [0] : ''), "", "", "text", 380 );
	...
	function searchProperties($whichSearch) {
		...
		} elseif (($obj->IsFolder != 1 && (($whichSearch == "DocSearch" && $tab == 1) || ($whichSearch == "TmplSearch" && $tab == 2) || ($whichSearch == "AdvSearch" && $tab == 3))) || (isset ( $_REQUEST ['cmdid'] ) && $_REQUEST ['cmdid'] != "") || (isset ( $_REQUEST ['view'] ) && ($_REQUEST ['view'] == "GetSearchResult" || $_REQUEST ['view'] == "GetMouseOverDivs"))) {
			...
			if(searchtoolsearch::checkRightTempTable()=="1" && searchtoolsearch::checkRightDropTable()=="1") {
				...
				foreach($versionsFound as $k => $v) {
					foreach($_result as $key => $val) {		
						if(isset($_result[$key]['docID']) && $_result[$key]['docID']==$v[1] && isset($_result[$key]['ContentType']) && $_result[$key]['ContentType']==$v[0]) {
					...
					while ( $thisObj->searchclass->next_record () ) {
	                 if (! isset ( $saveArrayIds [$thisObj->searchclass->Record ['ContentType']] [$thisObj->searchclass->Record ['docID']] )) {
	                    $saveArrayIds [$thisObj->searchclass->Record ['ContentType']] [$thisObj->searchclass->Record ['docID']] = $thisObj->searchclass->Record ['docID'];
	                        $_result [] = array_merge ( array ('Table' => $_table ), $thisObj->searchclass->Record );
	                    }
	                 }
				}
		...
		if ($_SESSION ['weSearch'] ['foundItems' . $whichSearch . ''] > 0) {
			...
			foreach ( $_result as $k => $v ) {
				...
				if ($_result [$k] ["docTable"] == FILE_TABLE && $_result [$k] ['Published'] >= $_result [$k] ['ModDate'] && $_result [$k] ['Published'] != 0) {
					$DB_WE->query ( "SELECT a.ID, c.Dat FROM (" . FILE_TABLE . " a LEFT JOIN " . LINK_TABLE . " b ON (a.ID=b.DID)) LEFT JOIN " . CONTENT_TABLE . " c ON (b.CID=c.ID) WHERE a.ID='" . $_result [$k] ["docID"] . "' AND b.Name='Description' AND b.DocumentTable='" . FILE_TABLE . "'" );
	...
	function makeHeadLines($whichSearch) {
		$headline [0] ["dat"] = '<a href="javascript:setOrder(\'Text\',\'' . $whichSearch . '\');">' . $GLOBALS ['l_weSearch'] ['dateiname'] . '</a> <span id="Text_' . $whichSearch . '" >' . $this->getSortImage ( 'Text', $whichSearch ) . '</span>';
		$headline [1] ["dat"] = '<a href="javascript:setOrder(\'SiteTitle\',\'' . $whichSearch . '\');">' . $GLOBALS ['l_weSearch'] ['seitentitel'] . '</a> <span id="SiteTitle_' . $whichSearch . '" >' . $this->getSortImage ( 'SiteTitle', $whichSearch ) . '</span>';
		$headline [2] ["dat"] = '<a href="javascript:setOrder(\'CreationDate\',\'' . $whichSearch . '\');">' . $GLOBALS ['l_weSearch'] ['created'] . '</a> <span id="CreationDate_' . $whichSearch . '" >' . $this->getSortImage ( 'CreationDate', $whichSearch ) . '</span>';
		$headline [3] ["dat"] = '<a href="javascript:setOrder(\'ModDate\',\'' . $whichSearch . '\');">' . $GLOBALS ['l_weSearch'] ['modified'] . '</a> <span id="ModDate_' . $whichSearch . '" >' . $this->getSortImage ( 'ModDate', $whichSearch ) . '</span>';
			...
			if ($view == 0) {
				 ...
				 if(is_array($foundInVersions) && !empty($foundInVersions)) {
	                  	...
	                  	foreach($foundInVersions as $k)  {
					        ...
					        $fileExists = f("SELECT ID FROM " . $_result[$f]["docTable"] . " WHERE ID= '".$_result[$f]["docID"]."' ","ID",$DB_WE);						
							...
							if($_result[$f]['ContentType'] == "objectFile") {
								...
								$classExists = f("SELECT ID FROM " . OBJECT_TABLE . " WHERE ID= '".$_result[$f]["TableID"]."' ","ID",$DB_WE);						
								if($classExists=="") {
									$resetDisabled = true;
	                  		...
	                  		$content[$f][2]["version"][$k] = "<div style='margin-bottom:5px;float:left;'>".we_forms::radiobutton($ID, 0, "resetVersion[".$_result[$f]["ID"]."_".$_result[$f]["Table"]."]","", false, "defaultfont", "",$resetDisabled)."</div><div style='float:left;margin-left:30px;'>".$previewButton."</div>";
	                  		$content[$f][3]["version"][$k] = date("d.m.Y",$timestamp);
	                  		$content[$f][4]["version"][$k] = "";
	                  		$content[$f][5]["version"][$k] = ($_result[$f]["ContentType"]=="text/webedition" || $_result[$f]["ContentType"]=="text/html" || $_result[$f]["ContentType"]=="objectFile") ? we_forms::checkbox($ID, 0, "publishVersion_".$ID, $GLOBALS['l_versions']['publishIfReset'], false, "middlefont", "") : "";
	                  	
	                  	}
	            }
				$docExists = f("SELECT ID FROM " . $_result [$f] ["docTable"] . " WHERE ID= '".$_result [$f] ["docID"]."' ","ID",$DB_WE);						
								
				$publishCheckbox = (($_result[$f]["ContentType"]=="text/webedition" || $_result[$f]["ContentType"]=="text/html" || $_result[$f]["ContentType"]=="objectFile") && we_hasPerm('PUBLISH') && $docExists!="") ? we_forms::checkbox($_result[$f]["docID"]."_".$_result[$f]["docTable"], 0, "publish_docs_".$whichSearch, "", false, "middlefont", "") : getPixel(20,10);
	            
				$content [$f] [0] ["dat"] = $publishCheckbox;
				$content [$f] [1] ["dat"] = '<img src="' . ICON_DIR . $Icon . '" border="0" width="16" height="18" />';
				$content [$f] [2] ["dat"] = '<a href="javascript:openToEdit(\'' . $_result [$f] ["docTable"] . '\',\'' . $_result [$f] ["docID"] . '\',\'' . $_result [$f] ["ContentType"] . '\')" style="text-decoration:none;color:' . $fontColor . ';"  title="' . $_result [$f] ["Text"] . '"><u>' . shortenPath ( $_result [$f] ["Text"], 17 );
				...
				$content [$f] [4] ["dat"] = '<nobr>' . $checkTable = isset($_result [$f] ["VersionID"]) && $_result [$f] ["VersionID"] ? "-" : ($_result [$f] ["CreationDate"] ? date ( $GLOBALS ['l_weSearch'] ["date_format"], $_result [$f] ["CreationDate"] ) : "-");
   ...
   ' . htmlSelect ( $anzahl, $values, 1, $_anzahl, "", 'onChange=\'this.form.elements["' . $searchstart . '"].value=0;search(false);\'' );
		
		$out .= '</td>
   <td style="width:400px;">' . $thisObj->getNextPrev ( $foundItems, $whichSearch ) . '</td>
   <td style="width:35px;">
   ' . $we_button->create_button ( "image:iconview", "javascript:setView(1);", true, "", "", "", "", false ) . '
   </td>
   <td>
   ' . $we_button->create_button ( "image:listview", "javascript:setView(0);", true, "", "", "", "", false ) . '
   </td>
   </tr>
   <tr>
	...
	function getSearchDialogAdvSearch() {
		...
		if ((isset ( $_SESSION ["weSearch"] ["keyword"] ) && $_SESSION ["weSearch"] ["keyword"] != "") && (isset ( $_REQUEST ["tab"] ) && $_REQUEST ["tab"] == 3)) {
		...
		if (! isset ( $_REQUEST ["searchFieldsAdvSearch"] ) && ! isset ( $_REQUEST ["cmdid"] ) && ! isset ( $this->Model->searchFieldsAdvSearch [0] )) {
			$this->searchclass->height = 1;
		}
		if (! isset ( $_REQUEST ["searchFieldsAdvSearch"] ) && ! isset ( $_REQUEST ["cmdid"] ) && isset ( $this->Model->searchFieldsAdvSearch [0] )) {
			$this->searchclass->height = count ( $this->Model->searchFieldsAdvSearch );
    ...
    <tr>
     ...
     <td></td>
		...
		for($i = 0; $i < $this->searchclass->height; $i ++) {
			...
			$searchInput = htmlTextInput ( "searchAdvSearch[" . $i . "]", 30, (isset ( $this->Model->searchAdvSearch ) && is_array ( $this->Model->searchAdvSearch ) && isset ( $this->Model->searchAdvSearch [$i] ) ? $this->Model->searchAdvSearch [$i] : ''), "", " class=\"wetextinput\"  id=\"searchAdvSearch['.$i.']\" ", "text", 170 );
			...
			if (isset ( $this->Model->searchFieldsAdvSearch [$i] )) {
				...
				if ($this->Model->searchFieldsAdvSearch [$i] == "Status") {
					$searchInput = htmlSelect ( "searchAdvSearch[" . $i . "]", $this->searchclass->getFieldsStatus (), 1, (isset ( $this->Model->searchAdvSearch ) && is_array ( $this->Model->searchAdvSearch ) && isset ( $this->Model->searchAdvSearch [$i] ) ? $this->Model->searchAdvSearch [$i] : ""), false, 'class="defaultfont" style="width:170px;" id="searchAdvSearch[' . $i . ']" ' );
				}
				
				if ($this->Model->searchFieldsAdvSearch [$i] == "Speicherart") {
					$searchInput = htmlSelect ( "searchAdvSearch[" . $i . "]", $this->searchclass->getFieldsSpeicherart (), 1, (isset ( $this->Model->searchAdvSearch ) && is_array ( $this->Model->searchAdvSearch ) && isset ( $this->Model->searchAdvSearch [$i] ) ? $this->Model->searchAdvSearch [$i] : ""), false, 'class="defaultfont" style="width:170px;" id="searchAdvSearch[' . $i . ']" ' );
				}
				...
				if ($this->Model->searchFieldsAdvSearch [$i] == "Published" || $this->Model->searchFieldsAdvSearch [$i] == "CreationDate" || $this->Model->searchFieldsAdvSearch [$i] == "ModDate") {
					$handle = "date";
					$searchInput = $this->getDateSelector ( "", "searchAdvSearch[" . $i . "]", "_from" . $i, $this->Model->searchAdvSearch [$i] );
				}
				
				if ($this->Model->searchFieldsAdvSearch [$i] == "temp_doc_type") {
					$searchInput = htmlSelect ( "searchAdvSearch[" . $i . "]", $this->searchclass->getDocTypes (), 1, (isset ( $this->Model->searchAdvSearch ) && is_array ( $this->Model->searchAdvSearch ) && isset ( $this->Model->searchAdvSearch [$i] ) ? $this->Model->searchAdvSearch [$i] : ""), false, 'class="defaultfont" style="width:170px;" id="searchAdvSearch[' . $i . ']" ' );
				}
				...
				if ($this->Model->searchFieldsAdvSearch [$i] == "ParentIDDoc" || $this->Model->searchFieldsAdvSearch [$i] == "ParentIDObj" || $this->Model->searchFieldsAdvSearch [$i] == "ParentIDTmpl") {
					...
					$selector = htmlFormElementTable ( htmlTextInput ( 'searchAdvSearch[' . $i . ']', 58, $_linkPath, '', 'readonly', 'text', 170, 0 ), '', 'left', 'defaultfont', we_htmlElement::htmlHidden ( array ('name' => 'searchAdvSearchParentID[' . $i . ']', "value" => "" ) ), getPixel ( 5, 4 ), $_button );
    ...
    <tr id="filterRow_' . $i . '">
     <td>'.hidden ( "hidden_searchFieldsAdvSearch[" . $i . "]", isset($this->Model->searchFieldsAdvSearch[$i]) ? $this->Model->searchFieldsAdvSearch[$i] : "" ).'' . htmlSelect ( "searchFieldsAdvSearch[" . $i . "]", $this->searchclass->getFields ( $i, "" ), 1, (isset ( $this->Model->searchFieldsAdvSearch ) && is_array ( $this->Model->searchFieldsAdvSearch ) && isset ( $this->Model->searchFieldsAdvSearch [$i] ) ? $this->Model->searchFieldsAdvSearch [$i] : ""), false, 'class="defaultfont" id="searchFieldsAdvSearch[' . $i . ']" onChange="changeit(this.value, ' . $i . ');" ' ) . '</td>
     <td id="td_locationAdvSearch[' . $i . ']">' . htmlSelect ( "locationAdvSearch[" . $i . "]", $this->searchclass->getLocation ( $handle ), 1, (isset ( $this->Model->locationAdvSearch ) && is_array ( $this->Model->locationAdvSearch ) && isset ( $this->Model->locationAdvSearch [$i] ) ? $this->Model->locationAdvSearch [$i] : ""), false, 'class="defaultfont" ' . $locationDisabled . ' id="locationAdvSearch[' . $i . ']" ' ) . '</td>
    ...
    ';
		...
		$out .= '<table>
     <tr>
      <td>' . $we_button->create_button ( "add", "javascript:newinputAdvSearch();" ) . '</td>
	...
	function tblList($content, $headline, $whichSearch) {
     ...
     <td style="border-bottom:1px solid #D1D1D1;">' . getPixel ( 1, 10 ) . '</td>';
		...
		$columnWidths = array (31, 36, 14, 19 );
		for($f = 0; $f < $anz; $f ++) {
	...
	function tabListContent($view = "", $content = "", $class = "", $whichSearch = "") {
		...
		if ($view == 0) {
			...
			$out .= "<script type='text/javascript'>document.getElementById('mouseOverDivs_" . $whichSearch . "').innerHTML = '" . addslashes ( $allDivs ) . "';</script>";
		}
		return $out;
	}
	...
	function makeMouseOverDivs( $x, $content, $whichSearch) {
		...
		for($n = 0; $n < $x; $n ++) {
			...
			if ($content [$n] [11] ["dat"] != "") {
				$outDivs .= '<table cellpadding="0" cellspacing="0" border="0" style="font-size:10px;"><tr><td valign="top">' . $GLOBALS ['l_weSearch'] ["beschreibung"] . ':</td><td>' . getPixel ( 15, 5 ) . '</td><td>';
	 ...
	 function tblListRow($content, $class = "middlefont", $bgColor = "")
          {
              ...
              $out = '';
              $columnWidths = array(0, 2, 28, 36, 15, 19);
              for ($f = 0; $f < $anz; $f++) {
                  if ($f == 0) {
                      $width = "width:30px;";
                      $plcholder = "";
                  ...
                  $out .= '<td align="'.$align.'" style="font-weight:bold;height:30px;font-size:11px;' . $width . '">' .$plcholder. ((isset($content[$f]["dat"]) && $content[$f]["dat"]) ? $content[$f]["dat"] : "&nbsp;") . '</td>';
	...
	function tblListRowIconView($content, $class = "defaultfont", $i, $whichSearch) {
		...
		$out .= '<td width="75" valign="top" align="center" onmouseover="showImageDetails(\'ImgDetails_' . $i . '_' . $whichSearch . '\',1)" onmouseout="hideImageDetails(\'ImgDetails_' . $i . '_' . $whichSearch . '\')">' . ((isset ( $content [0] ["dat"] ) && $content [0] ["dat"]) ? $content [0] ["dat"] : "&nbsp;") . '</td>';
	...
	function getDirSelector($whichSearch) {
		...
		$yuiSuggest->setSelectButton ( $we_button->create_button ( "select", "javascript:we_cmd('openDirselector',document.we_form.elements['$folderID'].value,'" . $table . "','document.we_form.elements[\\'$folderID\\'].value','document.we_form.elements[\\'$folderPath\\'].value')" ) );
