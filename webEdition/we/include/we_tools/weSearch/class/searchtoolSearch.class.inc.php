...
class searchtoolsearch extends we_search {
	...
	function initSearchData() {
		if (isset ( $GLOBALS ['we_doc'] )) {
			...
			if (isset ( $_REQUEST ["setView"] )) {
				$this->query ( "UPDATE " . FILE_TABLE . " SET listview=" . $_REQUEST ['setView'] . " WHERE ID=" . $obj->ID . "" );
				$obj->searchclassFolder->setView = ($_REQUEST ["setView"]);
			} else {
				$obj->searchclassFolder->setView = f ( "SELECT listview FROM " . FILE_TABLE . " WHERE ID='" . $obj->ID . "'", "listview", $GLOBALS ["DB_WE"] );
			}
			...
			if (isset ( $_REQUEST ["we_cmd"] ["setView"] ) && isset ( $_REQUEST ["id"] )) {
				$this->query ( "UPDATE " . FILE_TABLE . " SET listview=" . $_REQUEST ["we_cmd"] ["setView"] . " WHERE ID=" . $_REQUEST ["id"] . "" );
			}
		}
	}
	...
	function getFields($row, $whichSearch = "") {
		...
		$tableFields = array(
			'ID' => $GLOBALS['l_weSearch']['ID'], 'Text' => $GLOBALS['l_weSearch']['Text'], 'Path' => $GLOBALS['l_weSearch']['Path'], 'ParentIDDoc' => $GLOBALS['l_weSearch']['ParentIDDoc'], 'ParentIDObj' => $GLOBALS['l_weSearch']['ParentIDObj'], 'ParentIDTmpl' => $GLOBALS['l_weSearch']['ParentIDTmpl'], 'temp_template_id' => $GLOBALS['l_weSearch']['temp_template_id'], 'MasterTemplateID' => $GLOBALS['l_weSearch']['MasterTemplateID'], 'ContentType' => $GLOBALS['l_weSearch']['ContentType'], 'temp_doc_type' => $GLOBALS['l_weSearch']['temp_doc_type'], 'temp_category' => $GLOBALS['l_weSearch']['temp_category'], 'CreatorID' => $GLOBALS['l_weSearch']['CreatorID'], 'CreatorName' => $GLOBALS['l_weSearch']['CreatorName'], 'WebUserID' => $GLOBALS['l_weSearch']['WebUserID'], 'WebUserName' => $GLOBALS['l_weSearch']['WebUserName'], 'Content' => $GLOBALS['l_weSearch']['Content'], 'Status' => $GLOBALS['l_weSearch']['Status'], 'Speicherart' => $GLOBALS['l_weSearch']['Speicherart'], 'Published' => $GLOBALS['l_weSearch']['Published'], 'CreationDate' => $GLOBALS['l_weSearch']['CreationDate'], 
	...
	function getFieldsStatus() {
        ...
        $fields = array('jeder' => $GLOBALS['l_weSearch']['jeder'], 'geparkt' => $GLOBALS['l_weSearch']['geparkt'], 'veroeffentlicht' => $GLOBALS['l_weSearch']['veroeffentlicht'], 'geaendert' => $GLOBALS['l_weSearch']['geaendert'], 'veroeff_geaendert' => $GLOBALS['l_weSearch']['veroeff_geaendert'], 'geparkt_geaendert' => $GLOBALS['l_weSearch']['geparkt_geaendert'], 'deleted' => $GLOBALS['l_weSearch']['deleted']);
          
        return $fields;
    }
	
	function getFieldsSpeicherart() {
		$fields = array ('jeder' => $GLOBALS ['l_weSearch'] ['jeder'], 'dynamisch' => $GLOBALS ['l_weSearch'] ['dynamisch'], 'statisch' => $GLOBALS ['l_weSearch'] ['statisch'] );
	...
	function searchInTitle($keyword, $table) {
		...
		$query = "SELECT a.Name, b.Dat, a.DID FROM " . LINK_TABLE . " a LEFT JOIN " . CONTENT_TABLE . " b on (a.CID = b.ID) WHERE a.Name='Title' AND b.Dat LIKE '%" . addslashes ( trim ( $keyword ) ) . "%' AND NOT a.DocumentTable='".TEMPLATES_TABLE."'";
		$_db2->query ( $query );
	...
	function searchContent($keyword, $table) {
		...
		if ($table == FILE_TABLE || $table == TEMPLATES_TABLE) {
			...
			$query = "SELECT a.Name, b.Dat, a.DID FROM " . LINK_TABLE . " a LEFT JOIN " . CONTENT_TABLE . " b on (a.CID = b.ID) WHERE b.Dat LIKE '%" . addslashes ( trim ( $keyword ) ) . "%' AND a.DocumentTable='".$table."'";
	        $_db2->query ( $query );
	        while ( $_db2->next_record () ) {
	            $contents [] = $_db2->f ( 'DID' );  
	        }
			...
			if ($table == FILE_TABLE) {
		        $query2 = "SELECT DocumentID, DocumentObject  FROM " . TEMPORARY_DOC_TABLE . " WHERE DocumentObject LIKE '%" . addslashes ( trim ( $keyword ) ) . "%' AND DocTable = '" . $table . "' AND Active = '1'";
		        $_db2->query ( $query2 );
				while ( $_db2->next_record () ) {
		...
		elseif ($table == VERSIONS_TABLE) {
			...
			$_db2->query($query2);
			while ($_db2->next_record()) {
			  $tempDoc[0]['elements'] = unserialize(html_entity_decode(urldecode($_db2->f('documentElements')), ENT_QUOTES));
			  
			  if (isset($tempDoc[0]['elements'])) {
			      foreach ($tempDoc[0]['elements'] as $k => $v) {
			          if ($k != "Title" && $k != "Charset" && isset($tempDoc[0]['elements'][$k]['dat']) && stristr($tempDoc[0]['elements'][$k]['dat'], $keyword)) {
			              $contents [] = $_db2->f('ID');
			          }
			      }
			  }
	        ...
	        if (!empty($contents)) {
		            ...
		            for($c=0;$c<count($tableInfo);$c++){
		                if(ereg('^(.+)_(.+)$',$tableInfo[$c]["name"],$regs)){
		                    if($regs[1]!="OF" && $regs[1]!="variant"){
		                        $fields[] = array("name"=>$tableInfo[$c]["name"],"type"=>$regs[1],"length"=>$tableInfo[$c]["len"]);
		                    }
		                }
		            }
		            $field = array();
				...
				//unpublished objects
		        $query2 = "SELECT DocumentID, DocumentObject  FROM " . TEMPORARY_DOC_TABLE . " WHERE DocumentObject LIKE '%" . addslashes ( trim ( $keyword ) ) . "%' AND DocTable = '" . OBJECT_FILES_TABLE . "' AND Active = '1'";
		        $_db2->query ( $query2 );
		        while ( $_db2->next_record () ) {
	...
	function getChildsOfParentId($folderID, $table) {
		...
		$_SESSION["weSearch"]["countChilds"] = array_values ( array_unique ( $_SESSION["weSearch"]["countChilds"] ) );
