...
class ImportOptions extends leWizardStepBase {
	...
	function execute(&$Template) {
		...
		$MastertemplateInput = htmlTextInput($textname,30,$path,255,' readonly="readonly" id="' . $textname . '"','text',388);
		$MastertemplateHidden = "<input type=\"hidden\" id=\"" . $idname . "\" name=\"" . $idname . "\" value=\"" . $myid . "\">";
		$MastertemplateButton = $we_button->create_button("select", "javascript:we_cmd('openDocselector',document.getElementById('$idname').value,'$table','document.getElementById(\\'$idname\\').value','document.getElementById(\\'$textname\\').value','','".session_id()."','','text/weTmpl',1)");
		$MastertemplateTrash = $we_button->create_button("image:btn_function_trash", "javascript:document.getElementById('$idname').value='$myid';document.getElementById('$textname').value='$path';", true, 27, 22);

		$MastertemplateChooserText = $this->Language['choose_mastertemplate'];
		$MastertemplateChooser = htmlFormElementTable($MastertemplateInput, "", "left", "defaultfont", $MastertemplateHidden . getPixel(20,1), $MastertemplateButton, getPixel(10,1), $MastertemplateTrash);
		...
		$DocumentInput = htmlTextInput($textname,30,$path,255,' readonly="readonly" id="' . $textname . '"','text',388);
		$DocumentHidden = "<input type=\"hidden\" id=\"" . $idname . "\" name=\"" . $idname . "\" value=\"" . $myid . "\">";
		$DocumentButton = $we_button->create_button("select", "javascript:we_cmd('openDirselector',document.getElementById('$idname').value,'$table','document.getElementById(\\'$idname\\').value','document.getElementById(\\'$textname\\').value','','".session_id()."','','text/weFolder',1)");
		$DocumentTrash = $we_button->create_button("image:btn_function_trash", "javascript:document.getElementById('$idname').value='$myid';document.getElementById('$textname').value='$path';", true, 27, 22);

		$DocumentChooserText = $this->Language['choose_document_path'];
		$DocumentChooser = htmlFormElementTable($DocumentInput, "", "left", "defaultfont", $DocumentHidden . getPixel(20,1), $DocumentButton, getPixel(10,1), $DocumentTrash);
		...
		if(!we_hasPerm('ADMINISTRATOR')) {
			$_wrkNavi = makeArrayFromCSV(f('SELECT workSpaceNav FROM ' . USER_TABLE .' WHERE ID='.$_SESSION['user']['ID'],'workSpaceNav',new DB_WE()));
			$_condition = array();
