...
class copyFolderFrag extends taskFragment
{
	...
	function init()
	{
		...
		if ($fromID && $toID) {
			...
			$db->query(
					"SELECT ID,ParentID,Text,Path,IsFolder,ClassName,ContentType,Category FROM " . FILE_TABLE . " WHERE (Path like'$fromPath/%') AND ContentType != 'text/webedition' ORDER BY IsFolder DESC,Path");
			...
			for ($num = 0; $num < 2; $num++) {
				$db->query(
						"SELECT ID,ParentID,Text,TemplateID,Path,IsFolder,ClassName,ContentType,Category FROM " . FILE_TABLE . " WHERE (Path like'$fromPath/%') AND ContentType = 'text/webedition' ORDER BY IsFolder DESC,Path");
				while ($db->next_record()) {
				...
				case "text/webedition" :
					...
					if ($we_doc->DocType && $this->data["CreateDoctypes"]) {
						...
						if (!(isset($_SESSION["WE_CREATE_DOCTYPE"]) && isset(
							...
							if ($file_id = f(
									"SELECT ID FROM " . DOC_TYPES_TABLE . " WHERE DocType='" . $dt->DocType . "'", 
									"ID", 
									$GLOBALS["DB_WE"])) {
								$z = 0;
								$footext = $dt->DocType . "_" . $z;
								while (f(
										"SELECT ID FROM " . DOC_TYPES_TABLE . " WHERE DocType='$footext'", 
										"ID", 
										$GLOBALS["DB_WE"])) {
									$z++;
									$footext = $dt->DocType . "_" . $z;
	...
	function copyTemplate($templateID, $parentID, $CreateMasterTemplate = false, $CreateIncludedTemplate = false, $counter = 0)
	{
		...
		if (!isset($_SESSION["WE_CREATE_TEMPLATE"][$templateID])) {
			...
			// if file exists we need  to create a new one!
			if ($file_id = f(
					"SELECT ID FROM " . TEMPLATES_TABLE . " WHERE Path='" . $templ->Path . "'", 
					"ID", 
					$GLOBALS["DB_WE"])) {
				$z = 0;
				$footext = $templ->Filename . "_" . $z . $templ->Extension;
				while (f(
						"SELECT ID FROM " . TEMPLATES_TABLE . " WHERE Text='$footext' AND ParentID='" . $templ->ParentID . "'", 
						"ID", 
						$GLOBALS["DB_WE"])) {
					$z++;
					$footext = $templ->Filename . "_" . $z . $templ->Extension;
