		...
		for($i=0;$i<sizeof($tags);$i++){
			if($ll==0 && $l==0 && $b==0){
				if(eregi('name ?= ?"([^"]+)"',$tags[$i],$regs)){
					if(!in_array($regs[1],$names)) array_push($names,$regs[1]);
				}
			}
			if(eregi('< ?we:linklist',$tags[$i])){
				$ll++;
			}else if(eregi('< ?we:list',$tags[$i])){
				$l++;
			}else if(eregi('< ?we:block',$tags[$i])){
				$b++;
			}else if(eregi('< ?/ ?we:linklist',$tags[$i])){
				$ll--;
			}else if(eregi('< ?/ ?we:list',$tags[$i])){
				$l--;
			}else if(eregi('< ?/ ?we:block',$tags[$i])){
	...
	* @desc Searches for all meta-tags in a given template (title, keyword, description, charset)
	...
	function getMetaTags($code){
		...
		for($_i=0;$_i<sizeof($_tmpTags);$_i++){
			...
			if(strpos($_tmpTags[$_i], "we:title") || strpos($_tmpTags[$_i], "we:description") || strpos($_tmpTags[$_i], "we:keywords") || strpos($_tmpTags[$_i], "we:charset")){
		...
		for($i=0;$i<sizeof($_tags);){
		    ...
		    array_push($_rettags, array(array($_starttag, $_endtag), $_endtag ? substr($code, $_start,$_end) : ""));
		}
		return $_rettags;
	}
	...
	function searchEndtag($code,$tagPos){
		...
		if($tagname != "back" && $tagname != "next" && $tagname != "printVersion" && $tagname != "listviewOrder"){
			...
			for($i=$this->ipos+1;$i<sizeof($this->tags);$i++){
				...
				}else if(eregi("< ?we ?: ?$tagname",$this->tags[$i])){
					$tagcount++;
				}
	...
	function parseTag(&$code,$postName="")
	{
		...
		if(!$endTag){
			...
			default:
				...
				if(		$tagname == "ifHasEntries"
					||	$tagname == "ifNotHasEntries"
					||	$tagname == "ifHasCurrentEntry"
					||	$tagname == "ifNotHasCurrentEntry"
					) {
					$code = str_replace($tag,'<?php if(we_tag("'.$tagname.'", '.$attribs.')): ?>',$code);
					$this->ipos++;
					$this->lastpos=0;

				} else if($tagname == "ifshopexists"){
					$code = str_replace($tag,'<?php if(defined("SHOP_TABLE")): ?>',$code);
					$this->ipos++;
					$this->lastpos=0;
				}else if($tagname == "ifobjektexists"){
					$code = str_replace($tag,'<?php if(defined("OBJECT_TABLE")): ?>',$code);
					$this->ipos++;
					$this->lastpos=0;
				}else if($tagname == "ifnewsletterexists"){
					$code = str_replace($tag,'<?php if(defined("NEWSLETTER_TABLE")): ?>',$code);
					$this->ipos++;
					$this->lastpos=0;
				}else if($tagname == "ifcustomerexists"){
					$code = str_replace($tag,'<?php if(defined("CUSTOMER_TABLE")): ?>',$code);
					$this->ipos++;
					$this->lastpos=0;
				}else if($tagname == "ifbannerexists"){
					$code = str_replace($tag,'<?php if(defined("BANNER_TABLE")): ?>',$code);
					$this->ipos++;
					$this->lastpos=0;
				}else if($tagname == "ifvotingexists"){
					$code = str_replace($tag,'<?php if(defined("VOTING_TABLE")): ?>',$code);
					$this->ipos++;
					$this->lastpos=0;
				}else if(substr($tagname,0,2) == "if" && $tagname != "ifNoJavaScript"){
					/*$code = str_replace($tag,'<?php echo \'<?php if(we_tag("'.$tagname.'", '.$attribs.')): ?>\'; ?>',$code);*/
					$code = str_replace($tag,$this->parseCacheIfTag('<?php if(we_tag("'.$tagname.'", '.$attribs.')): ?>'),$code);
					$this->ipos++;
					$this->lastpos=0;
				}else if($tagname == "condition"){
					$code = str_replace($tag,'<?php we_tag("'.$tagname.'", '.$attribs.'); ?>',$code);
					$this->ipos++;
					$this->lastpos=0;
				}else{
					...
					if($endTagPos > -1){
						...
						if($endTagPos > $endeStartTag){
							...
							if ($tagname == "block") {
								$content = str_replace("\n","\\n",$content);
								$content = trim(str_replace("\r","\\r",$content));
								$content = str_replace('"','\"',$content);
							} else if($tagname!="noCache") {
								$content = str_replace("\n","",$content);
						...
						if($tagname=="noCache") {
							// Tag besitzt Endtag
							$code = substr($code,0,$tagPos) . $this->parseNoCacheTag($content) . substr($code,$endeEndTagPos);
							//neu
						} else {
							// Tag besitzt Endtag
							$we_tag = 'we_tag("'.$tagname.'", '.$attribs.', "'.$content.'")';
							$code = substr($code,0,$tagPos).'<?php printElement( '.$we_tag.'); ?>'.substr($code,$endeEndTagPos);
							//neu
						...
						$code = substr($code,0,$tagPos).$this->parseCacheIfTag('<?php else: ?>').substr($code,$endeStartTag);


					} else if(isset($GLOBALS["calculate"]) && $GLOBALS["calculate"] ==1){                                         //neu
						$we_tag = 'we_tag("'.$tagname.'", '.$attribs.')';
						eval('$code = str_replace($tag,std_numberformat('.$we_tag.'),$code);');
																								//neu
					}else{
					$we_tag = 'we_tag("'.$tagname.'", '.$attribs.', "")';
						$code = substr($code,0,$tagPos).'<?php printElement( '.$we_tag.'); ?>'.substr($code,$endeStartTag);
					}
					$this->lastpos=0;
				}
				if($postName){

					$code = preg_replace('/("name"=>")('. ( isset($arr["name"]) ? $arr["name"] : "" ) .')(")/i','\1\2'.$postName.'\3',$code);
					if($tagname == 'setVar') {
						...
						if(isset($arr['to']) && $arr['to'] == "block"){
							$code = preg_replace('/("nameto"=>")('. ( isset($arr["nameto"]) ? $arr["nameto"] : "" ) .')(")/i','\1\2'.$postName.'\3',$code);
						}
					} else {
						$code = preg_replace('/("namefrom"=>")('. ( isset($arr["namefrom"]) ? $arr["namefrom"] : "" ) .')(")/i','\1\2'.$postName.'\3',$code);
					}
					//$code = preg_replace('/("namefrom"=>")('. ( isset($arr["namefrom"]) ? $arr["namefrom"] : "" ) .')(")/i','\1\2'.$postName.'\3',$code);
					if (!in_array($tagname, array('ifVar', 'ifNotVar'))) { // ifVar and ifNotVar contains a value, NO fieldname herefore don't change match!
						$code = preg_replace('/("match"=>")('. ( isset($arr["match"]) ? $arr["match"] : "" ) .')(")/i','\1\2'.$postName.'\3',$code);
					}
				}
			}
		}else{
			...
			if(		$tagname == "ifHasEntries"
				...
				||	$tagname == "ifHasCurrentEntry"
				||	$tagname == "ifNotHasCurrentEntry"
				||	$tagname == "ifshopexists"
				||	$tagname == "ifobjektexists"
				||	$tagname == "ifnewsletterexists"
				||	$tagname == "ifcustomerexists"
				||	$tagname == "ifbannerexists"
				||	$tagname == "ifvotingexists"
				) {
			...
			}else if($tagname == "printVersion"){
				$code = str_replace($tag,'<?php if(isset($GLOBALS["we_tag_start_printVersion"]) && $GLOBALS["we_tag_start_printVersion"]){ $GLOBALS["we_tag_start_printVersion"]=0; ?></a><?php } ?>',$code);
			}else if($tagname == "next"){
				if(isset($GLOBALS["_we_voting_list_active"])) $code = str_replace($tag,'<?php if($GLOBALS["_we_voting_list"]->hasNextPage() ): ?></a><?php endif ?>',$code);
				else $code = str_replace($tag,'<?php if($GLOBALS["lv"]->hasNextPage() && $GLOBALS["lv"]->close_a() ): ?></a><?php endif ?>',$code);
			}else if($tagname == "back"){
				if(isset($GLOBALS["_we_voting_list_active"])) $code = str_replace($tag,'<?php if($GLOBALS["_we_voting_list"]->hasPrevPage() ): ?></a><?php endif ?>',$code);
				else $code = str_replace($tag,'<?php if($GLOBALS["lv"]->hasPrevPage()  && $GLOBALS["lv"]->close_a() ): ?></a><?php endif ?>',$code);
			}else if($tagname == "form"){
				$code = str_replace($tag,'<?php if(!isset($GLOBALS["we_editmode"]) || !$GLOBALS["we_editmode"]): ?></form><?php endif ?><?php $GLOBALS["WE_FORM"] = ""; if (isset($GLOBALS["we_form_action"])) {unset($GLOBALS["we_form_action"]);} ?>',$code);
			}else if($tagname == "repeat"){
				if(isset($GLOBALS['_we_voting_list_active'])){
					$code = str_replace($tag,'<?php } ?>',$code);
				} else {
					$code = str_replace($tag,'<?php } unset($GLOBALS["_we_listview_object_flag"]); ?>',$code);
				}

			}else if($tagname == "listview"){
				$code = str_replace($tag,'<?php
if ( isset( $GLOBALS["we_lv_array"] ) ) {
	...
	if (count($GLOBALS["we_lv_array"])) {
		...
		unset($GLOBALS["lv"]);unset($GLOBALS["we_lv_array"]);
			...
			}else if($tagname == "object" || $tagname == "customer" || $tagname == "metadata"){
				$code = str_replace($tag,'<?php endif ?><?php
if ( isset( $GLOBALS["we_lv_array"] ) ) {
	array_pop($GLOBALS["we_lv_array"]);
	if (count($GLOBALS["we_lv_array"])) {
		$GLOBALS["lv"] = clone($GLOBALS["we_lv_array"][count($GLOBALS["we_lv_array"])-1]);
		...
		unset($GLOBALS["lv"]);unset($GLOBALS["we_lv_array"]);
            ...
            }else if($tagname == "listviewOrder"){
                $code = str_replace($tag,'</a>',$code);
			}else if($tagname == "condition"){
				$code = str_replace($tag,'<?php $GLOBALS["we_lv_conditionCount"]--;$GLOBALS[$GLOBALS["we_lv_conditionName"]] .= ")"; ?>',$code);
			}else if($tagname == "tr"){
				$code = str_replace($tag,'<?php if($GLOBALS["lv"]->shouldPrintEndTR()): ?></tr><?php endif ?>',$code);
			}else if($tagname == "repeatShopItem"){
				$code = str_replace($tag,'<?php } unset($GLOBALS["lv"]); ?>',$code);
			}else if($tagname == "xmlnode"){
				$code = str_replace($tag,'<?php }} array_pop($GLOBALS["xstack"]); ?>',$code);
			}else if($tagname == "voting"){
				$code = str_replace($tag,'<?php if(isset($GLOBALS[\'_we_voting\'])) unset($GLOBALS[\'_we_voting\']); ?>',$code);
			}else if($tagname == "votingList"){
				unset($GLOBALS['_we_voting_list_active']);
				$code = str_replace($tag,'<?php unset($GLOBALS[\'_we_voting_list\']); ?>',$code);
			}else if($tagname == "answers"){
				$code = str_replace($tag,'<?php } if(isset($GLOBALS[\'_we_voting\'])) $GLOBALS[\'_we_voting\']->resetSets();?>',$code);
...
weTagListviewCache::init($weTagListviewCache, "' . $tagname . '", unserialize(\'' . serialize($attribs) . '\'), \'\', $GLOBALS[\'we_doc\']->CacheType == \'document\' ? $GLOBALS[\'we_doc\']->CacheLifeTime : '.$lifeTime.');
...
function getEndCacheCode($tag) {
	$temp = isset($GLOBALS['weListviewCacheStarted']) && is_array($GLOBALS['weListviewCacheStarted']) ? array_pop($GLOBALS['weListviewCacheStarted']) : false;
	$code = "";
	if(!$temp) {
		return $code;
...
';
			...
			if(defined("OBJECT_TABLE")){
				$foo = attributFehltError($arr,"classid","listview");if($foo) return str_replace($tag,$foo,$code);
...
';
			...
			if(defined("OBJECT_TABLE")){
			$foo = attributFehltError($arr,"name","listview");if($foo) return str_replace($tag,$foo,$code);
...
';
		    ...
		    $foo = attributFehltError($arr,"path","listview");if($foo) return $foo;
...
function parseFormTag($tag,$code,$attribs=""){
		...
		$formAttribs   = removeAttribs($arr, array('onsubmit','onSubmit','name','method','xml','charset','id','action','order','required','onsuccess','onerror','type','recipient','mimetype','subject','onmailerror','preconfirm','postconfirm','from','confirmmail','classid','doctype','remove','onrecipienterror','tid','forcefrom','categories'));
		...
		switch($type){
			case "shopliste":
				...
				$php .= '<?php if(!isset($GLOBALS["we_editmode"]) || !$GLOBALS["we_editmode"]) : ?>' .
				    ...
				    getHtmlTag('input', array('xml' => $xml, 'type'=>'hidden', 'name'=>'type', 'value'=>'<?php if( isset($GLOBALS["lv"]->classID) ){ echo "o"; }else if( isset($GLOBALS["lv"]->ID) ){ echo "w"; }else if( (isset($GLOBALS["we_doc"]->ClassID) || isset($GLOBALS["we_doc"]->ObjectID) )){echo "o";}else if($GLOBALS["we_doc"]->ID){ echo "w"; } ?>')) .
				    getHtmlTag('input', array('xml' => $xml, 'type'=>'hidden', 'name'=>'shop_artikelid', 'value'=>'<?php if(isset($GLOBALS["lv"]->classID) || isset($GLOBALS["we_doc"]->ClassID) || isset($GLOBALS["we_doc"]->ObjectID)){ echo (isset($GLOBALS["lv"]) && $GLOBALS["lv"]->DB_WE->Record["OF_ID"]!="") ? $GLOBALS["lv"]->DB_WE->Record["OF_ID"] : (isset($we_doc->DB_WE->Record["OF_ID"]) ? $we_doc->DB_WE->Record["OF_ID"] : (isset($we_doc->OF_ID) ? $we_doc->OF_ID : $we_doc->ID)); }else { echo (isset($GLOBALS["lv"]) && isset($GLOBALS["lv"]->IDs[$GLOBALS["lv"]->count-1]) && $GLOBALS["lv"]->IDs[$GLOBALS["lv"]->count-1]!="") ? $GLOBALS["lv"]->IDs[$GLOBALS["lv"]->count-1] : $we_doc->ID; } ?>')) .
				    getHtmlTag('input', array('xml' => $xml, 'type'=>'hidden', 'name'=>'we_variant', 'value'=>'<?php print (isset($GLOBALS["we_doc"]->Variant) ? $GLOBALS["we_doc"]->Variant : ""); ?>')) .
				    getHtmlTag('input', array('xml' => $xml, 'type'=>'hidden', 'name'=>'t','value' => '<?php echo time(); ?>') ) .
				    '<?php endif ?>';
				break;
			case "object":
			case "document":
...
';
					...
					$php.='<?php if(!isset($GLOBALS["we_editmode"]) || !$GLOBALS["we_editmode"]): ?>' .
					       ...
					       getHtmlTag('input', array('type'=>'hidden', 'name'=>'we_edit' . $typetmp . '_ID', 'value'=>'<?php print isset($_REQUEST["we_edit'.$typetmp.'_ID"]) ? ($_REQUEST["we_edit'.$typetmp.'_ID"]) : 0; ?>','xml'=>$xml)) .
					       '<?php endif ?>';
				}else{
					$php.='<?php if(!isset($GLOBALS["we_editmode"]) || !$GLOBALS["we_editmode"]): ?>' .
                        getHtmlTag('form', $formAttribs, '', false, true) .
...
*/
				...
				$php = '<?php if(!isset($GLOBALS["we_editmode"]) || !$GLOBALS["we_editmode"]): ?>
				            ...
				            <div class="weHide" style="display: none;">
                                ' . getHtmlTag('input', array('type' => 'hidden', 'name' => 'order', 'value' => '<?php print "'.$order.'"; ?>', 'xml' => $xml)) . '
                                ' . getHtmlTag('input', array('type' => 'hidden', 'name' => 'required', 'value' => '<?php print "'.$required.'"; ?>', 'xml' => $xml)) . '
                                ' . getHtmlTag('input', array('type' => 'hidden', 'name' => 'subject', 'value' => '<?php print "'.$subject.'"; ?>', 'xml' => $xml)) . '
                                ' . getHtmlTag('input', array('type' => 'hidden', 'name' => 'recipient', 'value' => '<?php print $_recipientIdString; ?>', 'xml' => $xml)) . '
                                ' . getHtmlTag('input', array('type' => 'hidden', 'name' => 'mimetype', 'value' => '<?php print "'.$mimetype.'"; ?>', 'xml' => $xml)) . '
                                ' . getHtmlTag('input', array('type' => 'hidden', 'name' => 'from', 'value' => '<?php print "'.$from.'"; ?>', 'xml' => $xml)) . '
                                ...
                                ' . getHtmlTag('input', array('type' => 'hidden', 'name' => 'charset', 'value' => '<?php print "'.$charset.'"; ?>', 'xml' => $xml)) . '
                                ' . getHtmlTag('input', array('type' => 'hidden', 'name' => 'confirm_mail', 'value' => '<?php print "'.$confirmmail.'"; ?>', 'xml' => $xml)) . '
                                ...
                                ' . getHtmlTag('input', array('type' => 'hidden', 'name' => 'forcefrom', 'value' => $forcefrom, 'xml' => $xml)) . '
...
##########################################################################################
	function parseCaptchaTag($tag,$code,$attribs=""){
		...
		// clean attribs
		$attribs = removeAttribs($attribs, array('path', 'maxlength', 'type', 'font', 'fontpath', 'fontsize', 'fontcolor', 'angle', 'subset', 'case', 'skip', 'align', 'valign', 'bgcolor', 'transparent', 'style', 'stylecolor', 'stylenumber'));
