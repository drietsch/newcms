...
function htmlFormElementTable($element, $text, $textalign = "left", $textclass = "defaultfont", $col2 = "", $col3 = "", $col4 = "", $col5 = "", $col6 = "" , $abstand = 1) {
	...
	return '<table cellpadding="0" cellspacing="0" border="0">'.($text ? '<tr><td class="'.trim($textclass).'" align="'.trim($textalign).'" colspan="'.$colspan.'">'.$text.'</td></tr>' : '').($abstand ? ('<tr><td colspan="'.$colspan.'">'.getPixel(2,$abstand).'</td></tr>') : '').'<tr>'.$elemOut.($col2 ? $col2out : "").($col3 ? $col3out : "").($col4 ? $col4out : "").($col5 ? $col5out : "").($col6 ? $col6out : "").'</tr></table>';
}
...
function targetBox($name, $size, $width = "", $id = "", $value = "", $onChange = "", $abstand = 8, $selectboxWidth = "", $disabled = false)  {
	...
	$_inputs = array("class" => "weSelect", "name" => "sel_" . $name, "onfocus"=>"change$jsvarname=1;", "onchange" => "if(change$jsvarname) this.form.elements['$name'].value = this.options[this.selectedIndex].text; change$jsvarname=0; this.selectedIndex = 0;" . $onChange, "style" => (($selectboxWidth != "") ? ("width: " . $selectboxWidth . "px;") : ""));

	if ($disabled) {
		$_inputs = array_merge($_inputs, array("disabled" => "true"));
	}

	$_target_box = new we_htmlSelect($_inputs, 0);
	$_target_box->addOptions(5, array("", "_top", "_parent", "_self", "_blank"), array("", "_top", "_parent", "_self", "_blank"));

	$_table = new we_htmlTable(array("cellpadding" => 0, "cellspacing" => 0, "border" => 0), 1, 3);

	$_inputs = array("name" => $name, "class" => "defaultfont");

	if ($width) {
		$_inputs = array_merge($_inputs, array("style" => "width: " . $width . "px;"));
	}

	if ($id) {
		$_inputs = array_merge($_inputs, array("id" => $id));
	}

	if ($value) {
		$_inputs = array_merge($_inputs, array("value" => htmlspecialchars($value)));
	}

	if ($onChange) {
		$_inputs = array_merge($_inputs, array("onchange" => $onChange));
	}

	$_table->setCol(0, 0, array("class" => "defaultfont"), htmlTextInput($name, $size, $value, "", (!empty($onChange)? 'onchange="'.$onChange.'"' : ''), "text", $width, 0, "", $disabled));

	$_table->setCol(0, 1, null, getPixel($abstand, 1));

	$_table->setCol(0, 2, array("class" => "defaultfont"), $_target_box->getHtmlCode());
	$js = <<<HTS
...
function htmlDialogLayout($content,$headline,$buttons="",$width="100%",$marginLeft="30",$height="", $overflow="auto"){
	...
	$parts = array(array("html"=>$content,"headline"=>"","space"=>0));
...
function htmlDialogBorder3Row($content,$class="middlefont",$bgColor="") {
	$anz = sizeof($content);
	$out = '<td style="border-bottom: 1px solid silver;background-image:url('.IMAGE_DIR.'box/shaddowBox3_l.gif);">'.getPixel(8,isset($content[0]["height"]) ? $content[0]["height"] : 1).'</td>';

		for ($f=0; $f<$anz; $f++) {
			$bgcol = $bgColor ? $bgColor : ((isset($content[$f]["bgcolor"]) && $content[$f]["bgcolor"]) ? $content[$f]["bgcolor"]  : "white");
			$out .= '<td class="'.$class.'" style="padding:2px 5px 2px 5px;'.(($f!=0) ? "border-left:1px solid silver;" : "").'border-bottom: 1px solid silver;background-color:'.$bgcol.';" '.((   isset($content[$f]["align"]))  ?'align="'.$content[$f]["align"].'"':"").' '.(  (isset($content[$f]["height"])) ?'height="'.$content[$f]["height"].'"':"").'>'.(( isset($content[$f]["dat"]) &&  $content[$f]["dat"])?$content[$f]["dat"]:"&nbsp;").'</td>';
		}
		$out .= '
					<td style="border-bottom: 1px solid silver;background-image:url('.IMAGE_DIR.'box/shaddowBox3_r.gif);">'.getPixel(8,isset($content[0]["height"]) ? $content[0]["height"] : 1).'</td>
...
function htmlDialogBorder3($w,$h,$content,$headline,$class="middlefont",$bgColor="",$buttons="",$id="",$style=""){  //content && headline are arrays
	...
	$out = '<table'.($id ? ' id="'.$id.'"' : '').($style ? ' style="'.$style.'"' : '').' border="0" cellpadding="0" cellspacing="0" width="' . $w . '">
		<tr>
		<td width="8" style="background-image:url('.IMAGE_DIR.'box/box_header_ol2.gif);">'.getPixel(8,21).'</td>';
		// HEADLINE
	for ($f=0; $f<$anz; $f++) {
	...
	if($buttons){
		$attribs = array("border" => "0", "cellpadding" => "0", "cellspacing" => "0");
...
function gifButton($name,$href,$language="Deutsch",$alt="",$width="",$height="",$onClick="",$bname="",$target="",$disabled=false){
	...
	$img = '<img src="'.IMAGE_DIR.'buttons/'.$name.($disabled ? "_d" : "").
	($language ? '_' : '').$language.'.gif"'.
	($width ? ' width="'.$width.'"' : '').
	($height ? ' height="'.$height.'"' : '').
	($bname ? ' name="'.$bname.'"' : '').
	' border="0" alt="'.$alt.'">';

	if($disabled) return $img;
	if($href){
...
function getExtensionPopup($name,$selected,$extensions,$width="",$attribs=""){
	...
	if((isset($extensions))&&(sizeof($extensions)>1)){
		...
		htmlTextInput($name,5,$selected,"",$attribs,"text",$width/2,"0","top").
		'</td><td><select class="weSelect" name="wetmp_'.$name.'" size=1'.($width ? ' style="width: '.($width/2).'px"' : '').' onChange="if(typeof(_EditorFrame) != \'undefined\'){_EditorFrame.setEditorIsHot(true);}if(this.options[this.selectedIndex].text){this.form.elements[\''.$name.'\'].value=this.options[this.selectedIndex].text;};this.selectedIndex=0"><option>';
...
function getDateInput2($name,$time="",$setHot=false,$format="",$onchange="",$class="weSelect",$xml="",$minyear="",$maxyear=""){
	...
	if(($format == "") || $_dayPos > -1){
		...
		for($i=1;$i<=31;$i++){
		    if($time && $day == $i){
		        $_atts2 = array('selected' => 'selected');
		    } else {
		        $_atts2 = array();
		    }
			$days .= getHtmlTag('option',array_merge($_attsOption,$_atts2 ),sprintf("%02d",$i));
		}
		$daySelect = getHtmlTag('select', array_merge($_attsSelect, array('name'=>sprintf($name,"_day"),'id'=>sprintf($name,"_day"))),$days, true) . '&nbsp;';
	}else{
		$daySelect = getHtmlTag('input',array_merge($_attsHidden, array('type'=>'hidden','name'=>sprintf($name,"_day"),'id'=>sprintf($name,"_day"),'value' =>$day)));
		$_showDay = false;
	}
	...
	if(($format == "") || $_monthPos > -1){
		...
		$monthSelect = getHtmlTag('select',array_merge($_attsSelect, array('name'=>sprintf($name,"_month"),'id'=>sprintf($name,"_month"))), $months, true) . '&nbsp;';
	}else{
		$monthSelect = getHtmlTag('input',array_merge($_attsHidden, array('type'=>'hidden','name'=>sprintf($name,"_month"),'id'=>sprintf($name,"_month"),'value' => $month)));
		$_showMonth = false;
	}
	...
	if(($format == "") || $_yearPos > -1){
		...
		$yearSelect = getHtmlTag('select',array_merge($_attsSelect, array('name'=>sprintf($name,"_year"),'id'=>sprintf($name,"_year"))), $years, true) . '&nbsp;';
	}else{
		$yearSelect = getHtmlTag('input',array_merge($_attsHidden, array('type'=>'hidden','name'=>sprintf($name,"_year"),'id'=>sprintf($name,"_year"),'value' => $year)));
		$_showYear = false;
	}
	...
	if(($format == "") || $_hourPos > -1){
		...
		$hourSelect = getHtmlTag('select',array_merge($_attsSelect, array('name'=>sprintf($name,"_hour"),'id'=>sprintf($name,"_hour"))), $hours, true) . '&nbsp;';
	}else{
		$hourSelect = getHtmlTag('input',array_merge($_attsHidden, array('type'=>'hidden','name'=>sprintf($name,"_hour"),'id'=>sprintf($name,"_hour"),'value' => $hour)));
		$_showHour = false;
	}
	...
	if(($format == "") || $_minutePos > -1){
		...
		$minSelect = getHtmlTag('select',array_merge($_attsSelect, array('name'=>sprintf($name,"_minute"),'id'=>sprintf($name,"_minute"))), $minutes, true) . '&nbsp;';
	}else{
		$minSelect = getHtmlTag('input',array_merge($_attsHidden, array('type'=>'hidden','name'=>sprintf($name,"_minute"),'id'=>sprintf($name,"_minute"),'value' => $minute)));
		$_showMinute = false;
	}

	$_datePosArray = array(		($_dayPos == -1) ? "d" : $_dayPos=>$daySelect,
								($_monthPos == -1) ? "m" : $_monthPos=>$monthSelect,
								($_yearPos == -1) ? "y" : $_yearPos=>$yearSelect
							);

	$_timePosArray = array(		($_hourPos == -1) ? "h" : $_hourPos=>$hourSelect,
								($_minutePos == -1) ? "i" : $_minutePos=>$minSelect
							);
...
function getHtmlTop($title = "webEdition (c) living-e AG", $charset = "", $useMessageBox=true) {
	...
	$_meta_expires = we_htmlElement::htmlMeta(array("http-equiv" => "expires", "content" => 0));
	$_meta_no_cache = we_htmlElement::htmlMeta(array("http-equiv" => "pragma", "content" => "no-cache"));
	...
	$_meta_imagetoolbar_type = we_htmlElement::htmlMeta(array("http-equiv" => "imagetoolbar", "content" => "no"));
	$_meta_generator = we_htmlElement::htmlMeta(array("name" => "generator", "content" => "webEdition Version " . WE_VERSION));
	$_showMessage = $useMessageBox
						?
...
function htmlYesNoCancelDialog($text = "", $img = "", $yes = "", $no = "", $cancel = "", $yesHandler = "", $noHandler = "", $cancelHandler = "", $script = "") {
	...
	$content = new we_htmlTable(array("cellpadding" => 10, "cellspacing" => 0, "border" => 0), 1, ($img != "" ? 2 : 1));

	if ($img != "") {
		if (file_exists($_SERVER["DOCUMENT_ROOT"] . $img)) {
			$size = getimagesize($_SERVER["DOCUMENT_ROOT"] . $img);
			$content->setCol(0, 0, array("valign" => "top"), we_htmlElement::htmlImg(array("src" => $img, "border" => 0, "width" => $size[0], "height" => $size[1])));
		}
	}

	$content->setCol(0, ($img != "" ? 1 : 0), array("class" => "defaultfont"), $text);
	...
	return htmlDialogLayout($out,"",$we_button->position_yes_no_cancel($yesButton, $noButton, $cancelButton),"99%","0");
...
function htmlSelect($name,$values,$size=1,$selectedIndex="",$multiple=false,$attribs="",$compare="value",$width="",$cls="defaultfont", $htmlspecialchars=true){
	...
	foreach($values as $value=>$text) {
		if ($text == '<!--we_optgroup-->') {
			...
			$ret .= '<option value="'.($htmlspecialchars ? htmlspecialchars($value) : $value).'"'.(in_array((($compare == "value") ? $value : $text),$selIndex) ? " selected" : "").'>'. ($htmlspecialchars ? htmlspecialchars($text) : $text)."</option>\n";
		}
	}
	if ($optgroup) {
		$ret .= '</optgroup>' . "\n";
...
function htmlAlertAttentionBox($text,$type=0,$width=0,$useHtmlSpecialChars = true,$clip=0){
	...
	return $js . '<div style="background-color:#dddddd;padding:5px;'.($width ? ' width:'.$width.'px;' : '').'"><table border="0" cellpadding="2" width="100%">'.
			'<tr>'.($icon ? '<td style="padding-right:10px;" valign="top"><img src="'.IMAGE_DIR.$icon.'_small.gif" width="20" height="22"></td>' : '').
			'<td class="middlefont" '.($clip>0 ? 'id="'.$unique.'"' : '').'>'.$text.'</td>' .
			($clip>0 ? '<td valign="top" align="right" id="btn_'.$unique.'"><a href="javascript:clip_'.$unique.'();"><img src="'.IMAGE_DIR.'/button/btn_direction_right.gif" border="0"></a><td>' : '') .
			'</tr></table></div>';
}
