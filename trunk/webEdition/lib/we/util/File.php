...
class we_util_File
{
	...
	public static function loadLine($filename,$offset=0,$rsize=8192,$iscompressed=0){
			...
			if($filename=='') return false;
			if(self::hasURL($filename)) return false;
	...
	public static function saveTemp($content,$filename="",$flags="wb"){
			if($filename=="") $filename=self::getUniqueId();
			$filename = TMP_DIR."/".$filename;
			if(self::save($filename,$content)) return $filename;
			else return false;
	}
	...
	public static function mkpath($path){
		...
		if(self::hasURL($path)) return false;
		if($path!=""){
	...
	public static function hasCompression($comp){
		if($comp=="gzip") return self::hasGzip();
		if($comp=="zip") return self::hasZip();
		if($comp=="bzip") return self::hasBzip();
		return false;
	}
	...
	public static function getCompression($filename){
		...
		foreach($compressions as $val){
   			if(eregi(".".self::getZExtension($val),basename($filename))) return $val;
		}
		return "none";
	...
	public static function compress($file,$compression="gzip",$destination="",$remove=true,$writemode="wb"){
		...
		if(!self::hasCompression($compression)) return false;
