...
if($_REQUEST["we_cmd"][0] == "do_move" || $_REQUEST["we_cmd"][0] == "move_single_document"){
	...
	if(isset($_REQUEST["sel"]) && $_REQUEST["sel"] && isset($_REQUEST["we_target"])){
		...
		if($retVal == -1){	//	not allowed to move document
			...
			$script .= we_message_reporting::getShowMessageCall( sprintf($l_alert["noRightsToMove"],id_to_path($selectedItems[$i],$table)), WE_MESSAGE_ERROR);

		}else if($retVal){	//	move files !
			...
			if($_SESSION["we_mode"] == "normal"){	//	different messages in normal or seeMode
				if(sizeof($notMovedItems)){
					...
					foreach($notMovedItems as $item){
						$_SESSION["move_files_nok"][]=array("icon"=>$item['Icon'],"path"=>$item['Path']);
					}
		...
		$script .= "top.toggleBusy(0);\n" . we_message_reporting::getShowMessageCall( $l_alert["nothing_to_move"], WE_MESSAGE_ERROR) . "\n";
	}
...
if($_SESSION["we_mode"] == "seem"){
	...
	if($retVal){	//	document moved -> go to seeMode startPage
		$_js = we_message_reporting::getShowMessageCall( $l_alert['move_single']['return_to_start'], WE_MESSAGE_NOTICE) . ";top.we_cmd('start_multi_editor');";
	} else {
...
if(top.treeData.table != "<?php print $table; ?>"){
	top.treeData.table = "<?php print $table; ?>";
	we_cmd("load","<?php print $table; ?>");
}else{
	we_cmd("load","<?php print $table; ?>");
	top.drawTree();
}
...
function press_ok_move() {
	...
	if(!sel){
		top.toggleBusy(0);
		<?php print we_message_reporting::getShowMessageCall( $l_alert['nothing_to_move'], WE_MESSAGE_ERROR) ?>
		return;
	}
	...
	if(acStatusType.toLowerCase() == 'object') {
		if(acStatus.running) {
			...
			<?php print we_message_reporting::getShowMessageCall( $l_we_class["notValidFolder"], WE_MESSAGE_ERROR) ?>
			return;
		}
	}
	...
	var _move_table = "<?php print $table; ?>";
	var _move_ids = "," + sel;
	...
	if ( _open_move_editors.length ) {
		...
		} else if (defined("OBJECT_TABLE") && $table == OBJECT_FILES_TABLE) {
			$_type = $GLOBALS["l_global"]["objects"];

		}
		?>

		if ( confirm("<?php printf($l_alert["move_exit_open_docs_question"], $_type, $_type); ?>" + _openDocs_Str + "\n<?php print $l_alert["move_exit_open_docs_continue"]; ?>") ) {
			...
			we_cmd('do_move','','<?php print $table; ?>');
		}

	} else {

		if(confirm('<?php print $l_alert["move"]; ?>')) {
			we_cmd('do_move','','<?php print $table; ?>');
		}
	}
}
...
$yuiSuggest->setSelectButton($we_button->create_button("select", "javascript:we_cmd('openDirselector',document.we_form.elements['$idname'].value,'$table','top.rframe.bframe.treeheader.document.we_form.elements.".$idname.".value','top.rframe.bframe.treeheader.document.we_form.elements.".$textname.".value','','".session_id()."',0)"),10);
...
$_buttons = $we_button->position_yes_no_cancel(	$we_button->create_button("ok", "javascript:press_ok_move();"),
												"",
												$we_button->create_button("quit_move", "javascript:we_cmd('exit_move','','$table')"),10,"left"
										  );
...
<?php
print '</head><body class="weTreeHeaderMove">
<form name="we_form" method="post" onsubmit="return false">
<div style="width:460px;">
<h1 class="big" style="padding:0;margin:0;">'.htmlspecialchars($l_newFile["title_move"]). '</h1>
