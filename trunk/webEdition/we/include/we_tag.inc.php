...
function we_tag($name,$attribs,$content=""){
	...
	$attribs = removeAttribs($attribs, array('cachelifetime'));
	$OtherCacheActive = (isset($GLOBALS["weTagListviewCacheActive"]) && $GLOBALS["weTagListviewCacheActive"]) || (isset($GLOBALS["weTagBlockCache"]) && $GLOBALS["weTagBlockCache"] >= 1);
	$toolinc = '';

	if (function_exists($fn)) {
		// do noting
	} else if (file_exists($_SERVER["DOCUMENT_ROOT"]."/webEdition/we/include/we_tags/"."we_tag_$name.inc.php")) {
		include_once($_SERVER["DOCUMENT_ROOT"]."/webEdition/we/include/we_tags/"."we_tag_$name.inc.php");

	} else if($fn == "we_tag_noCache") {

	} else if (file_exists($_SERVER['DOCUMENT_ROOT'].'/webEdition/we/include/we_tags/custom_tags/'."we_tag_$name.inc.php")) {
	...
	// Use Document Cache
	if($CacheType == 'document' && (!isset($GLOBALS['weNoCache']) || !$GLOBALS['weNoCache']) && (!isset($GLOBALS['weCacheOutput']) || !$GLOBALS['weCacheOutput'])) {
		...
		if($GLOBALS["we_doc"]->CacheLifeTime > 0 && get_class($weTagCache) != "weTagBlockCache") {
			...
			// Tag is cacheable
			} else if($weTagCache->isCacheable()) {
	...
	} else if($CacheType == 'full' && $weTagCache->lifeTime > 0) {
		$foo = $fn($attribs, $content);
...
function makeEmptyTable($in){
	...
	for($i=0;$i<sizeof($result);$i++){
		...
		if(eregi("< ?td",$tag) || eregi("< ?/ ?td",$tag) || eregi("< ?tr",$tag) || eregi("< ?/ ?tr",$tag) || eregi("< ?table",$tag) || eregi("< ?/ ?table",$tag) || eregi("< ?tbody",$tag) || eregi("< ?/ ?tbody",$tag)){
			$out .= $tag;
		}
...
function cutText($text,$max=0){
	...
	}else if(eregi('^(.+)(<)(a|em|strong|b|i|u|br|div|span)([^>]*)$',$text,$regs)){
		$text = $regs[1];
	}
	return $text."...";
}
		...
		default:
			...
			if($doc){
				if($property){
				    ...
				    if ($fieldType=="checkbox_feld" && $issetElemNameDat && $doc->elements[$name]["dat"] == 0) return false;
...
function we_isVarNotEmpty($attribs){
	...
	if(!we_isVarSet($match,$type,$docAttr,$property,$formname)) return false;
...
function we_isNotEmpty($attribs){
	...
	switch($type){
		...
		case "href":
			...
			$int = ($doc->getElement($match."_we_jkhdsf_int") == "") ? 0 : $doc->getElement($match."_we_jkhdsf_int");
		...
		default:
			...
			if(isset($doc)) {
			    ...
			    if( (substr($doc->getElement($match),0,2) == "a:" )){   //  only unserialize, when $match cluld be an array
                    ...
                    if( is_array($arr = @unserialize($doc->getElement($match)))) {
	...
	switch($type){
		case "calendar":
			if(isset($GLOBALS["lv"]->calendar_struct)){
				if($GLOBALS["lv"]->calendar_struct["date"]<0)  return false;
				if(count($GLOBALS["lv"]->calendar_struct["storage"])<1)  return false;
				if($match=="day"){
				...
				if(isset($sd) && isset($ed)){
					foreach ($GLOBALS["lv"]->calendar_struct["storage"] as $entry){
						if($sd<$entry && $ed>$entry) return true;
					}
				}
				return false;
			}
...
function we_tag_a($attribs,$content){
	...
	$foo = attributFehltError($attribs,"id","a");if($foo) return $foo;

	// get attributes
	$id = we_getTagAttribute("id",$attribs); if($id=="self"){ $id=$GLOBALS["WE_MAIN_DOC"]->ID; }
	...
	if($edit == "shop") {
		...
		if ( isset($GLOBALS["lv"]) && get_class($GLOBALS["lv"]) == 'shop' ) {
			...
			$idd = ( (isset($GLOBALS["lv"]) && isset($GLOBALS["lv"]->IDs[$foo])) && $GLOBALS["lv"]->IDs[$foo]!="") ? $GLOBALS["lv"]->IDs[$foo] : ( (isset($GLOBALS["lv"]->classID))? $GLOBALS["lv"]->DB_WE->Record["OF_ID"] :((isset($GLOBALS["we_obj"]->ID))? $GLOBALS["we_obj"]->ID : $GLOBALS["WE_MAIN_DOC"]->ID));
			$type = ( isset($GLOBALS["lv"]) && isset($GLOBALS["lv"]->IDs[$foo]) && $GLOBALS["lv"]->IDs[$foo]!="") ? ((isset($GLOBALS["lv"]->classID) || isset($GLOBALS["lv"]->Record["OF_ID"]) )?"o":"w") : ((isset($GLOBALS["lv"]->classID)) ?"o":((isset($GLOBALS["we_obj"]->ID))?"o":"w")) ;
		}
		...
		if(isset($_REQUEST["we_transaction"]) && isset($_SESSION["we_data"][$_REQUEST["we_transaction"]]["0"]["ClassName"]) && $_SESSION["we_data"][$_REQUEST["we_transaction"]]["0"]["ClassName"] == "we_objectFile"){
			$type = "o";
		}
		...
		if($delarticle){ // delarticle
			...
			if ( isset($GLOBALS["lv"]) && get_class($GLOBALS["lv"]) == 'shop' ) {
				...
				$idd = (isset($GLOBALS["lv"]->IDs[$foo]) && $GLOBALS["lv"]->IDs[$foo]!="") ? $GLOBALS["lv"]->IDs[$foo] : ((isset($GLOBALS["lv"]->classID))? $GLOBALS["lv"]->DB_WE->Record["OF_ID"] :((isset($GLOBALS["we_obj"]->ID))? $GLOBALS["we_obj"]->ID : $GLOBALS["WE_MAIN_DOC"]->ID));
				$type = ( isset($GLOBALS["lv"]) && isset($GLOBALS["lv"]->IDs[$foo]) && $GLOBALS["lv"]->IDs[$foo]!="") ? ((isset($GLOBALS["lv"]->classID) || isset($GLOBALS["lv"]->Record["OF_ID"]) )?"o":"w") : ((isset($GLOBALS["lv"]->classID)) ?"o":((isset($GLOBALS["we_obj"]->ID))?"o":"w")) ;
			}
			//	preview mode in seem
			if(isset($_REQUEST["we_transaction"]) && isset($_SESSION["we_data"][$_REQUEST["we_transaction"]]["0"]["ClassName"]) && $_SESSION["we_data"][$_REQUEST["we_transaction"]]["0"]["ClassName"] == "we_objectFile"){
		...
		} else if($delshop){ // emptyshop

			$foo = attributFehltError($attribs,"shopname","a");if($foo) return $foo;
	...
	if($return){
		$urladd = ($urladd ? $urladd."&" : '?')."we_returnpage=".rawurlencode($_SERVER["PHP_SELF"]."?".$_SERVER["QUERY_STRING"]);
	}
	...
	$attribs = removeAttribs($attribs, array("id","shop","amount","delshop","delarticle","shopname","return","edit","type","button","hrefonly","confirm","editself","delete"));

	if($button){	//	show button

		$attribs["type"]  = "button";
		$attribs["value"] = htmlspecialchars($content);
		$attribs["onclick"] = ($target ? ("var wind=window.open('','$target');wind") : "self") . ".document.location='$url" . htmlspecialchars($urladd) . "';";
...
function we_tag_author($attribs,$content){
	...
	$foo = getHash("SELECT Username,First,Second FROM ".USER_TABLE." WHERE ID='".($creator ? $doc->CreatorID : $doc->ModifierID)."'",new DB_WE());
...
function we_tag_back($attribs,$content){
	if(isset($GLOBALS["_we_voting_list"])) return $GLOBALS["_we_voting_list"]->getBackLink($attribs);
	else return $GLOBALS["lv"]->getBackLink($attribs);
}
...
function we_tag_block($attribs,$content){
	...
	$foo = attributFehltError($attribs,"name","block");if($foo) return $foo;
	include_once($_SERVER["DOCUMENT_ROOT"]."/webEdition/we/include/"."we_tagParser.inc.php");
	...
	if(strpos($content, "<we:object") === false && strpos($content, "<we:metadata") === false){	//	no we:object is used
		...
		$tp->parseTags($tags,$content,'<we_:_ref>', array('we:field','we:ifFieldNotEmpty','we:ifFieldEmpty'));
	}
	$out = "";
	...
	if($list){
		...
		if($listlen != 0){
			...
			for($i=0;$i<$listlen;$i++){
				...
				if(strpos($foo, 'position') || strpos($foo, 'ifPosition') || strpos($foo, 'ifNotPosition')){	//	set information for ifPosition

					$foo =	'<?php $GLOBALS[\'we_position\'][\'block\'][\'' . $name . '\'] = array(\'position\' => ' . ($i+1) . ', \'size\'=>' . $listlen . '); ?>' .
							$foo .
							'<?php unset($GLOBALS[\'we_position\'][\'block\'][\'' . $name . '\']); ?>';
				}
				...
				if($we_editmode){
					...
					$upbut       = $we_button->create_button("image:btn_direction_up", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(true);we_cmd('up_entry_at_list','$name','$i')");
					$upbutDis    = $we_button->create_button("image:btn_direction_up", "",true, 21, 22, "", "", true);
					$downbut     = $we_button->create_button("image:btn_direction_down", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(true);we_cmd('down_entry_at_list','$name','$i')");
					$downbutDis  = $we_button->create_button("image:btn_direction_down", "", true, 21, 22, "", "", true);
					if($showselect && $show > 0){
						$plusbut     = $we_button->create_button("image:btn_add_listelement", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(true);we_cmd('insert_entry_at_list','$name','$i',document.we_form.elements['".$tmpname."_".$i."'].options[document.we_form.elements['".$tmpname."_".$i."'].selectedIndex].text)", true, 100, 22, "", "", ($show>0?false:true));
					}else{
						$plusbut     = $we_button->create_button("image:btn_add_listelement", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(true);we_cmd('insert_entry_at_list','$name','$i',1)", true, 100, 22, "", "", ($show>0?false:true));
					}
					$trashbut    = $we_button->create_button("image:btn_function_trash", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(true);we_cmd('delete_list','$name','$i','$names',1)");
					$buts = "";
					...
					if(!$isInListview){
						...
						if($showselect){
							...
							$buts =
								$we_button->create_button_table(array(	$plusbut,
																		$selectb,
																		(($i>0) ? $upbut : $upbutDis),
	...
	if($we_editmode){
		...
		if($show > 0) {
			...
			if($showselect){
				$plusbut = $we_button->create_button("image:btn_add_listelement", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(true);we_cmd('add_entry_to_list','$name',document.we_form.elements['".$tmpname."_00'].options[document.we_form.elements['".$tmpname."_00'].selectedIndex].text)", true, 100, 22, "", "", ($show>0?false:true));
				$plusbut = $we_button->create_button_table(array($plusbut,$selectb));
			}else{
				$plusbut = $we_button->create_button("image:btn_add_listelement", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(true);we_cmd('add_entry_to_list','$name',1)", true, 100, 22, "", "", ($show>0?false:true));
			}
			...
			$out .= (!$isInListview) ? ('<input type="hidden" name="we_'.$GLOBALS["we_doc"]->Name.'_list['.$name.']" value="'.htmlentities($list).'"><input type="hidden" name="we_'.$GLOBALS["we_doc"]->Name.'_list['.$name.'#content]" value="'.htmlspecialchars($content).'">'.$plusbut) : '';
		}
...
function we_tag_category($attribs,$content){
	...
	if ($id) {
		$category = str_replace("\\,",",",we_getCatsFromIDs($id,
						$delimiter,
						$showpath,
						$GLOBALS["DB_WE"],
						$rootdir,$field));
	...
	if($isInListview){
		...
		$category = $catIDs ? str_replace("\\,",",",we_getCatsFromIDs($catIDs,
						$delimiter,
						$showpath,
						$GLOBALS["DB_WE"],
						$rootdir,$field)): "";
		...
		$doc=we_getDocForTag($docAttr,false);
		$category = str_replace("\\,",",",we_getCatsFromDoc($doc,
						$delimiter,
						$showpath,
						$GLOBALS["DB_WE"],
						$rootdir,$field ));
...
function we_tag_categorySelect($attribs,$content){
	...
	if($isuserinput && $GLOBALS["WE_FORM"]){
		$objekt = isset($GLOBALS["we_object"][$GLOBALS["WE_FORM"]]) ? $GLOBALS["we_object"][$GLOBALS["WE_FORM"]] : (isset($GLOBALS["we_document"][$GLOBALS["WE_FORM"]]) ? $GLOBALS["we_document"][$GLOBALS["WE_FORM"]] : (isset($GLOBALS["we_doc"]) ? $GLOBALS["we_doc"] : false));
		if($objekt){
	...
	if(!$content){
		...
		while($db->next_record()){
			...
			if($field) {
			    if(in_array($db->f("Path"),$valuesArray)){
			        $content .= getHtmlTag('option', array('value'=>$db->f("Path"), 'selected'=>'selected'), str_repeat($indent, $deep).$field) . "\n";
			    } else {
                    $content .= getHtmlTag('option', array('value'=>$db->f("Path")), str_repeat($indent, $deep).$field) . "\n";
			    }
			}
		}
	}else{
		foreach($valuesArray as $catPaths){
			if(eregi("<option>",$content)){
                $content = eregi_replace('<option>'.quotemeta($catPaths).'( ?[<\n\r\t])','<option selected="selected">'.$catPaths.'\1',$content);
			}
...
function we_tag_conditionAdd($attribs,$content){
	...
	$foo = attributFehltError($attribs,"field","conditionAdd");if($foo) return $foo;
...
function we_tag_css($attribs,$content){
	...
	$foo = attributFehltError($attribs,"id","css");if($foo) return $foo;
...
function we_tag_date($attribs,$content){
	...
	if(strtolower($type) == "js"){
		...
		if(ereg('[^\\]Y',$f) || ereg('^Y',$f))  $js .= "var Y = heute.getYear();Y = (Y < 1900) ? (Y + 1900) : Y;\n";
		if(ereg('[^\\]y',$f) || ereg('^y',$f))  $js .= "var y = heute.getYear();y = (y < 1900) ? (y + 1900) : y;y=y.substring(2,4);\n";;

		if(ereg('[^\\]a',$f) || ereg('^a',$f))  $js .= "var a = (heute.getHours() > 11) ? 'pm' : 'am';\n";
		if(ereg('[^\\]A',$f) || ereg('^A',$f))  $js .= "var A = (heute.getHours() > 11) ? 'PM' : 'AM';\n";
		if(ereg('[^\\]s',$f) || ereg('^s',$f))  $js .= "var s = heute.getSeconds();\n";
		if(ereg('[^\\]m',$f) || ereg('^m',$f))  $js .= "var m = heute.getMonth()+1;m = '00'+m;m=m.substring(m.length-2,m.length);\n";
		if(ereg('[^\\]n',$f) || ereg('^n',$f))  $js .= "var n = heute.getMonth()+1;\n";
		if(ereg('[^\\]d',$f) || ereg('^d',$f))  $js .= "var d = heute.getDate();d = '00'+d;d=d.substring(d.length-2,d.length);\n";
		if(ereg('[^\\]j',$f) || ereg('^j',$f))  $js .= "var j = heute.getDate();\n";
		if(ereg('[^\\]h',$f) || ereg('^h',$f))  $js .= "var h = heute.getHours();if(h > 12){h -= 12;};h = '00'+h;h=h.substring(h.length-2,h.length);\n";
		if(ereg('[^\\]H',$f) || ereg('^H',$f))  $js .= "var H = heute.getHours();H = '00'+H;H=H.substring(H.length-2,H.length);\n";
		if(ereg('[^\\]g',$f) || ereg('^g',$f))  $js .= "var g = heute.getHours();if(g > 12){ g -= 12;};\n";
		if(ereg('[^\\]G',$f) || ereg('^G',$f))  $js .= "var G = heute.getHours();\n";
		if(ereg('[^\\]i',$f) || ereg('^i',$f))  $js .= "var i = heute.getMinutes();i = '00'+i;i=i.substring(i.length-2,i.length);\n";
		if(ereg('[^\\]S',$f) || ereg('^S',$f))  $js .= "var S = getDateS(heute.getDate());\n";

		if(ereg('[^\\]D',$f) || ereg('^D',$f))  $js .= "var D = getDateWord('D',heute);\n";
		if(ereg('[^\\]F',$f) || ereg('^F',$f))  $js .= "var F = getDateWord('F',heute);\n";
		if(ereg('[^\\]l',$f) || ereg('^l',$f))  $js .= "var l = getDateWord('l',heute);\n";
		if(ereg('[^\\]M',$f) || ereg('^M',$f))  $js .= "var M = getDateWord('M',heute);\n";
...
function we_tag_dateSelect($attribs,$content){
	$foo = attributFehltError($attribs,"name","dateSelect");if($foo) return $foo;
	$name = we_getTagAttribute("name",$attribs);
...
//-->
	...
	return getDateInput2("$name%s",(((!isset($_REQUEST[$name])) || $_REQUEST[$name] == -1) ? time() : $_REQUEST[$name]),false,"dmy",$submitonchange ? $checkDate."we_submitForm();" : $checkDate,$class,"",$minyear,$maxyear).$js;
}
...
function we_tag_description($attribs,$content){
	...
	$attribs = removeAttribs($attribs, array('htmlspecialchars'));
	...
	switch($docAttr){
		case "self":
			if($GLOBALS["we_doc"]->DocType) {
				$doctype = f("SELECT DocType FROM ".DOC_TYPES_TABLE." WHERE ID = ".$GLOBALS["we_doc"]->DocType, "DocType", new DB_WE());
			}
			break;
		case "top":
		default:
			if(isset($GLOBALS["WE_MAIN_DOC"])) {
				if($GLOBALS["WE_MAIN_DOC"]->DocType) {
					$doctype = f("SELECT DocType FROM ".DOC_TYPES_TABLE." WHERE ID = ".$GLOBALS["WE_MAIN_DOC"]->DocType, "DocType", new DB_WE());
				}
...
function we_tag_field($attribs,$content){
	...
	if($alt=="we_path") $alt = "WE_PATH";
	if($alt=="we_text") $alt = "WE_TEXT";
	if($name=="we_path") $name = "WE_PATH";
	if($name=="we_text") $name = "WE_TEXT";

	if(isset($attribs["winprops"])){
		unset($attribs["winprops"]);
	}
	$classid = $classid ? $classid : (isset($GLOBALS["lv"]) ? ( isset($GLOBALS["lv"]->object->classID) ? $GLOBALS["lv"]->object->classID : (isset($GLOBALS["lv"]->classID) ? $GLOBALS["lv"]->classID : "")) : (isset($GLOBALS["we_doc"]->TableID) ? $GLOBALS["we_doc"]->TableID : 0));
	$isImageDoc = false;
	...
	switch($type){
		...
		case "binary":
				$t = we_document::getFieldByVal($GLOBALS["lv"]->f($name),
							$type,
							$attribs,
							false,
							...
							$classid,
							'$GLOBALS["lv"]->f');
				$out = $t[0]; $href=(empty($href)?$t[1]:$href);
		break;
	case "link":
		if($GLOBALS["lv"]->ClassName){
		$out = we_document::getFieldByVal($GLOBALS["lv"]->f($name),
							"link",
							$attribs,
							false,
							$GLOBALS["we_doc"]->ParentID,
	...
	case "checkbox":
		...
		if($src && $type == "img"){
			...
			$out = we_document::getFieldByVal(($isImageDoc &&  $type == "img") ? $GLOBALS["lv"]->Record["wedoc_ID"] : $GLOBALS["lv"]->f($name),
							$type,
							$attribs,
							false,
							$GLOBALS["we_doc"]->ParentID,
	...
	case 'shopVat':
		...
		if (defined('SHOP_TABLE')) {
			...
			$normVal = we_document::getFieldByVal($GLOBALS["lv"]->f(WE_SHOP_VAT_FIELD_NAME, 'txt'),
							$type,
							$attribs,
							false,
							$GLOBALS["we_doc"]->ParentID,
	...
	default:
		$normVal = we_document::getFieldByVal($GLOBALS["lv"]->f($name),
							$type,
							$attribs,
							false,
							$GLOBALS["we_doc"]->ParentID,
		...
		if($name && $name != 'we_href'){
			...
			if($normVal==""){
				$altVal = we_document::getFieldByVal($GLOBALS["lv"]->f($alt),
							$type,
							$attribs,
							false,
							$GLOBALS["we_doc"]->ParentID,
	...
	if($hyperlink || $name == 'we_href'){
			...
			if(!$GLOBALS['we_doc']->InWebEdition){	//	we are NOT in webEdition open new window
				...
				if(isset($winpropsArray["left"]) && ($winpropsArray["left"] == -1) && isset($winpropsArray["width"]) && $winpropsArray["width"]){
					$js .= 'if (window.screen) {'.
							...
							'var w = Math.min(screen_width, '.$winpropsArray["width"].');'.
							'}'.
				...
				if(isset($winpropsArray["top"]) && ($winpropsArray["top"] == -1) && isset($winpropsArray["height"]) && $winpropsArray["height"]){
					$js .= 'if (window.screen) {'.
							...
							'var h = Math.min(screen_height, '.$winpropsArray["height"].');'.
							'}'.
		...
		if($href){
			...
			if($id && $isCalendar){
				if(isset($GLOBALS["lv"]->calendar_struct["storage"]) && count($GLOBALS["lv"]->calendar_struct["storage"])){
					$found=false;
					foreach($GLOBALS["lv"]->calendar_struct["storage"] as $date){
						if((($GLOBALS["lv"]->calendar_struct["calendarCount"]>0 || ($GLOBALS["lv"]->calendar_struct["calendar"]=="day" && $GLOBALS["lv"]->calendar_struct["calendarCount"]>=0)) && $GLOBALS["lv"]->calendar_struct["calendarCount"] <= $GLOBALS["lv"]->calendar_struct["numofentries"])
							&& ((int)$date>=(int)$GLOBALS["lv"]->calendar_struct["start_date"] && (int)$date<=(int)$GLOBALS["lv"]->calendar_struct["end_date"])){
							$found=true;
							break;
						}
					}
				 	if($found){
				 		if($GLOBALS["lv"]->calendar_struct["calendar"]=="year") $show="month";
				 		else $show="day";
			    ...
			    $_linkAttribs['href'] = id_to_path($id) . '?' .
                    					...
                    					'we_lv_start_'.$lvname.'='.(($GLOBALS["lv"]->count+$GLOBALS["lv"]->start) - 1).'&amp;we_lv_pend_'.$lvname.'='.($GLOBALS["lv"]->start+$GLOBALS["lv"]->anz).'&amp;we_lv_pstart_'.$lvname.'='.($GLOBALS["lv"]->start);
				...
				if(isset($GLOBALS["lv"]->ClassName) && $GLOBALS["lv"]->ClassName=="we_search_listview" && $GLOBALS["lv"]->f("OID")){
						...
						$_linkAttribs['href'] = $_SERVER["PHP_SELF"].'?we_objectID='.$GLOBALS["lv"]->f("OID").'&amp;pid='.$GLOBALS["lv"]->f("WorkspaceID").$tail;
				...
				}else if(isset($GLOBALS["lv"]->ClassName) && $GLOBALS["lv"]->ClassName=="we_catListview" && we_tag_ifHasChildren(array(),"")){
						$parentidname = we_getTagAttribute('parentidname', $attribs, 'we_parentid');
						$_linkAttribs['href'] = $_SERVER["PHP_SELF"].'?' . $parentidname . '='.$GLOBALS["lv"]->f("ID");
					...
					$showlink = (!isset($GLOBALS["lv"]->ClassName) || $GLOBALS["lv"]->ClassName == "" || $GLOBALS["lv"]->ClassName=="we_listview") ||
						($GLOBALS["lv"]->ClassName=="we_search_listview") ||
						($GLOBALS["lv"]->ClassName=="we_listview_shopVariants") ||
						($GLOBALS["lv"]->ClassName=="we_listview_shoppingCart") ||
						($GLOBALS["lv"]->ClassName=="we_objecttag") ||
	...
	if(	$isImageDoc &&
		isset($_SESSION["we_mode"]) && $_SESSION["we_mode"] == "seem" &&
		$GLOBALS["we_doc"]->InWebEdition && $GLOBALS["we_doc"]->ContentType != "text/weTmpl"
		){
	...
	if(  isset($_SESSION["we_mode"]) && $_SESSION["we_mode"] == "seem" &&
		(isset($GLOBALS["lv"]->ClassName) && $GLOBALS["lv"]->ClassName == "we_listview_object") &&
		 isset($GLOBALS["_we_listview_object_flag"]) && $GLOBALS["_we_listview_object_flag"] &&
		 $GLOBALS["we_doc"]->InWebEdition && $GLOBALS["we_doc"]->ContentType != "text/weTmpl" &&
		 $GLOBALS["lv"]->seeMode &&
		 $seeMode){
...
function we_tag_flashmovie($attribs,$content) {
	...
	$foo = attributFehltError($attribs,"name","flashmovie");if($foo) return $foo;
	$name = we_getTagAttribute("name",$attribs);
	...
	$attribs = removeAttribs($attribs, array('showcontrol','showflash'));
	...
	if ($showcontrol && $we_editmode) {
		...
		$flash_button = $we_button->create_button("image:btn_edit_flash", "javascript:we_cmd('openDocselector','" . $id . "', '".FILE_TABLE."', 'document.forms[\'we_form\'].elements[\'" . $fname . "\'].value', '', 'opener.setScrollTo();opener.top.we_cmd(\'reload_editpage\');opener._EditorFrame.setEditorIsHot(true);', '" . session_id() . "', 0, 'application/x-shockwave-flash', " . (we_hasPerm("CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")", true);
		...
		$out .= $we_button->create_button_table(array($flash_button, $clear_button), 5) . "</td></tr></table>";
	}
	//	When in SEEM - Mode add edit-Button to tag - textarea
	return $out;
}
...
function we_tag_formfield($attribs, $content) {
	...
	$foo = attributFehltError($attribs,"name","formfield");if($foo) return $foo;
	$name = we_getTagAttribute("name",$attribs);
	...
	if(getXmlAttributeValueAsBoolean($xml)){
	    $mandatoryFields = array('textarea_cols', 'textarea_rows');
	}
...
';
		...
		<td class=\"weEditmodeStyle\" width="161"><textarea name="'.$nameprefix . 'ffvalues]" cols="30" rows="5">'.$GLOBALS["we_doc"]->getElement($name, 'ffvalues').'</textarea></td>
	</tr>
	<tr>
		...
		<td class=\"weEditmodeStyle\" width="161"><input type="text" name="'.$nameprefix . 'ffdefault]" size="24" value="'.$GLOBALS["we_doc"]->getElement($name, 'ffdefault').'"></td>
	</tr>
';
		...
		<td class=\"weEditmodeStyle\" width="161"><input type="text" name="'.$nameprefix . 'ffmaxfilesize]" size="24" value="'.$GLOBALS["we_doc"]->getElement($name, 'ffmaxfilesize').'"></td>
	</tr>
';
		}else if($type_sel == 'radio' || $type_sel == 'checkbox') {
			$tbl .= '	<tr>
		...
		<td class=\"weEditmodeStyle\" width="161"><select name="'.$nameprefix . 'ffchecked]" size="1"><option value="0"'.($GLOBALS["we_doc"]->getElement($name, 'ffchecked') ? "" : " selected").'>'.$GLOBALS["l_global"]["no"].'</option><option value="1"'.($GLOBALS["we_doc"]->getElement($name, 'ffchecked') ? " selected" : "").'>'.$GLOBALS["l_global"]["yes"].'</option></select></td>
	</tr>
';
		}
		$tbl .= '</table>
';
	    ...
	    $tagName = '';
	    $tagAtts = array(  'xml' => $xml,
	                       'name' => htmlspecialchars($GLOBALS["we_doc"]->getElement($attribs["name"], 'ffname')));
	    $tagContent = '';
	    ...
	    if ($type_sel == 'textarea') {
	    	...
	    	foreach($foo as $v){
	    	    $_atts = array('value'=>htmlspecialchars($v));
	    	    if($selected == $v){
	    	...
	    	$ret .= getHtmlTag('input', array('type' => 'hidden', 'name' => 'MAX_FILE_SIZE', 'value' => htmlspecialchars($GLOBALS["we_doc"]->getElement($name, 'ffmaxfilesize')), 'xml' => $xml));
	    }
...
function we_tag_hidden($attribs,$content){
	...
	$foo = attributFehltError($attribs,"name","hidden");if($foo) return $foo;
...
function we_tag_href($attribs,$content) {
	...
	$foo = attributFehltError($attribs,"name","href");if($foo) return $foo;
	$name = we_getTagAttribute("name",$attribs);
	$type = we_getTagAttribute("type",$attribs);
	...
	if ($we_editmode) {
		...
		$span = '<span style="color: black;font-size:'.
	...
	if (!$type || $type == "all") {
		...
		if ($we_editmode) {
			if(($directory && $file) || $file){
				$but       = $we_button->create_button("select", "javascript:we_cmd('openDocselector', document.forms[0].elements['$intID_elem_Name'].value, '".FILE_TABLE."', 'document.forms[\\'we_form\\'].elements[\\'$intID_elem_Name\\'].value', 'document.forms[\\'we_form\\'].elements[\\'$intPath_elem_Name\\'].value', 'opener._EditorFrame.setEditorIsHot(true); opener.document.we_form.elements[\'$int_elem_Name\'][0].checked = true;" . (($include || $reload) ? "opener.setScrollTo(); opener.top.we_cmd(\'reload_editpage\');" : "") . "', '" . session_id() . "', '".$rootdirid."', '', " . (we_hasPerm("CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ",'',".($directory ? 1 : 0).");");
				...
				$but       = $we_button->create_button("select", "javascript:we_cmd('openDirselector', document.forms[0].elements['$intID_elem_Name'].value, '".FILE_TABLE."', 'document.forms[\\'we_form\\'].elements[\\'$intID_elem_Name\\'].value', 'document.forms[\\'we_form\\'].elements[\\'$intPath_elem_Name\\'].value', 'opener._EditorFrame.setEditorIsHot(true); opener.document.we_form.elements[\'$int_elem_Name\'][0].checked = true;" . (($include || $reload) ? "opener.setScrollTo(); opener.top.we_cmd(\'reload_editpage\');" : "") . "', '" . session_id() . "', '".$rootdirid."');");
				...
				<table border="0" cellpadding="0" cellspacing="2" background="' . IMAGE_DIR . 'backgrounds/aquaBackground.gif" style="border: solid #006DB8 1px;">
					<tr>
						<td class="weEditmodeStyle">
							' . we_forms::radiobutton(1, $int, $int_elem_Name, $span .$GLOBALS["l_tags"]["int_href"] . ":</span>") . '</td>
						<td class="weEditmodeStyle">
					...
					<tr>
						<td class="weEditmodeStyle">
							' . we_forms::radiobutton(0, !$int, $int_elem_Name, $span . $GLOBALS["l_tags"]["ext_href"] . ":</span>") . '</td>
						<td class="weEditmodeStyle">
		...
		if ($we_editmode) {
			if(($directory && $file) || $file){
				$but       = $we_button->create_button("select", "javascript:we_cmd('openDocselector', document.forms[0].elements['$intID_elem_Name'].value, '".FILE_TABLE."', 'document.forms[\\'we_form\\'].elements[\\'$intID_elem_Name\\'].value', 'document.forms[\\'we_form\\'].elements[\\'$intPath_elem_Name\\'].value', 'opener._EditorFrame.setEditorIsHot(true); " . (($include || $reload) ? "opener.setScrollTo(); opener.top.we_cmd(\'reload_editpage\');" : "") . "', '" . session_id() . "', '".$rootdirid."', '', " . (we_hasPerm("CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ",'',".($directory ? 1 : 0).");");
			}else{
				$but       = $we_button->create_button("select", "javascript:we_cmd('openDirselector', document.forms[0].elements['$intID_elem_Name'].value, '".FILE_TABLE."', 'document.forms[\\'we_form\\'].elements[\\'$intID_elem_Name\\'].value', 'document.forms[\\'we_form\\'].elements[\\'$intPath_elem_Name\\'].value', 'opener._EditorFrame.setEditorIsHot(true); " . (($include || $reload) ? "opener.setScrollTo(); opener.top.we_cmd(\'reload_editpage\');" : "") . "', '" . session_id() . "', '".$rootdirid."');");
			}
...
function we_tag_sidebar($attribs, $content) {
	...
	if(we_tag_ifNotSidebar($attribs, $content) && we_tag_ifEditmode($attribs, $content)) {
		...
		removeAttribs($attribs, array('id', 'file', 'url', 'width', 'href'));
...
function we_tag_ifBack($attribs,$content){
	if(isset($GLOBALS['_we_voting_list'])) return $GLOBALS['_we_voting_list']->hasPrevPage();
...
function we_tag_ifCaptcha($attribs,$content){
	...
	} else if (empty($formname) && isset($_REQUEST['we_ui_we_global_form'][$name])) {
		return Captcha::check($_REQUEST['we_ui_we_global_form'][$name]);
	} else if(empty($formname) && isset($_REQUEST[$name])) {
...
function we_tag_ifCat($attribs,$content){
	...
	if(strlen($categories) == 0 &&  strlen($category) == 0){
		$foo = attributFehltError($attribs,"categories","ifCat");if($foo){print($foo); return "";}
	}
...
function we_tag_ifHasChildren($attribs,$content){
	...
	if (isset($GLOBALS["lv"])) {
		if (abs($GLOBALS["lv"]->f("ID")) > 0) {
			return abs(f("SELECT COUNT(ID) AS ID FROM " . CATEGORY_TABLE . " WHERE ParentID='".abs($GLOBALS["lv"]->f("ID"))."'", "ID", new DB_WE())) > 0;
		}
	}
	return false;
}
...
function we_tag_ifClient($attribs,$content){
	...
	if($browser){
		...
		if (!$foo_br && $_browserOfClient == "firefox" && in_array("mozilla",$bro)) {
			$foo_br = true;
		} else if (!$foo_br && $_browserOfClient == "appleWebKit" && in_array("safari",$bro)) {
			$foo_br = true;
		}
	}else{
		$foo_br = true;
...
function we_tag_ifDoctype($attribs,$content){
	...
	$foo = attributFehltError($attribs,"doctypes","ifDoctype");if($foo){print($foo); return "";}
...
function we_tag_ifEmpty($attribs,$content){
	$foo = attributFehltError($attribs,"match","ifEmpty");if($foo){print($foo); return "";}
	if(isset($GLOBALS["we_editmode"]) && $GLOBALS["we_editmode"]){
	    return true;
	}
...
function we_tag_ifEqual($attribs,$content){
	global $we_editmode;
	$foo = attributFehltError($attribs,"name","ifEqual");if($foo){print($foo); return "";}
	$name = we_getTagAttribute("name",$attribs);
	$eqname = we_getTagAttribute("eqname",$attribs);
	$value = we_getTagAttribute("value",$attribs);

	if(!$eqname){
		$foo = attributFehltError($attribs,"value","ifEqual");if($foo){print($foo); return "";}
		return ($GLOBALS["we_doc"]->getElement($name) == $value);
	}

	$foo = attributFehltError($attribs,"eqname","ifEqual");if($foo){print($foo); return "";}
...
function we_tag_ifFieldNotEmpty($attribs,$content){
	...
	$foo = attributFehltError($attribs,"match","ifFieldNotEmpty");if($foo){print($foo); return "";}
...
function we_tag_ifField($attribs,$content){
	...
	$foo = attributFehltError($attribs,"name","ifField");if($foo){print($foo); return "";}
	$foo = attributFehltError($attribs,"match","ifField",true);if($foo){print($foo); return "";}
	$foo = attributFehltError($attribs,"type","ifField",true);if($foo){print($foo); return "";}
...
function we_tag_ifIsDomain($attribs,$content){
	global $we_editmode;
	$foo = attributFehltError($attribs,"domain","ifIsDomain");if($foo){print($foo); return "";}
...
function we_tag_ifIsNotDomain($attribs,$content){
	global $we_editmode;
	$foo = attributFehltError($attribs,"domain","ifIsNotDomain");if($foo){print($foo); return "";}
	$domain = we_getTagAttribute("domain",$attribs);
	return $we_editmode || (!($domain == $_SERVER["SERVER_NAME"]));
}
...
function we_tag_ifNext($attribs,$content){
	if(isset($GLOBALS['_we_voting_list'])) return $GLOBALS['_we_voting_list']->hasNextPage();
	$useparent = we_getTagAttribute("useparent",$attribs,'',true);
	return $GLOBALS["lv"]->hasNextPage($useparent);
}

function we_tag_ifNoJavaScript($attribs,$content){
	$foo = attributFehltError($attribs,"id","ifNoJavaScript");if($foo){print($foo); return "";}
	$id = we_getTagAttribute("id",$attribs);
	$row = getHash("SELECT Path,IsFolder,IsDynamic FROM ".FILE_TABLE." WHERE ID=$id",new DB_WE);
	$url = $row["Path"].($row["IsFolder"] ? "/" : "");
...
function we_tag_ifNotEmpty($attribs,$content){
	$foo = attributFehltError($attribs,"match","ifNotEmpty");if($foo){print($foo); return "";}
	if(isset($GLOBALS["we_editmode"]) && $GLOBALS["we_editmode"]){
	    return true;
	}
	return we_isNotEmpty($attribs);
...
function we_tag_ifNotEqual($attribs,$content){
	global $we_editmode;
	$foo = attributFehltError($attribs,"name","ifNotEqual");if($foo){print($foo); return "";}
	...
	$foo = attributFehltError($attribs,"eqname","ifNotEqual");if($foo){print($foo); return "";}
	return ($GLOBALS["we_doc"]->getElement($name) != $GLOBALS["WE_MAIN_DOC"]->getElement($eqname));
}
...
function we_tag_ifNotVar($attribs,$content){
	$foo = attributFehltError($attribs,"name","ifNotVar");if($foo){print($foo); return "";}
	$foo = attributFehltError($attribs,"match","ifNotVar",true);if($foo){print($foo); return "";}
	return !we_tag_ifVar($attribs,$content);
}

function we_tag_ifNotVarSet($attribs,$content){
	$foo = attributFehltError($attribs,"name","ifNotVarSet");if($foo){print($foo); return "";}
...
function we_tag_ifRegisteredUserCanChange($attribs,$content) {
	...
	if($admin) {
		if(isset($_SESSION["webuser"][$admin]) && $_SESSION["webuser"][$admin] ) return true;
	}
...
function we_tag_ifSearch($attribs,$content){
	...
	if($set){
		...
		return isset($_REQUEST["we_lv_search_".$name]) && strlen(str_replace("\"","",str_replace("\\\"","", (isset($_REQUEST["we_lv_search_".$name]) ? trim($_REQUEST["we_lv_search_".$name]) : "")  )));
	}
}
...
function we_tag_ifSelf($attribs,$content){
	...
	switch($type){
		case "listview":
			...
			}else if($GLOBALS["lv"]->ClassName == "we_search_listview"){
				return in_array($GLOBALS["lv"]->DB_WE->f("WE_ID"), $ids);
			}else if($GLOBALS["lv"]->ClassName == "we_listview_shopVariants"){
...
function we_tag_ifUserInputEmpty($attribs,$content){
	$foo = attributFehltError($attribs,"match","ifUserInputEmpty");if($foo){print($foo); return "";}
	return !we_isUserInputNotEmpty($attribs);
}

function we_tag_ifUserInputNotEmpty($attribs,$content){
	$foo = attributFehltError($attribs,"match","ifUserInputNotEmpty");if($foo){print($foo); return "";}
	return we_isUserInputNotEmpty($attribs);
}

function we_tag_ifVar($attribs,$content){
	$foo = attributFehltError($attribs,"name","ifVar");if($foo){print($foo); return "";}
	$foo = attributFehltError($attribs,"match","ifVar",true);if($foo){print($foo); return "";}
...
function we_tag_ifVarEmpty($attribs,$content){
	$foo = attributFehltError($attribs,"match","ifVarEmpty");if($foo){print($foo); return "";}
	return !we_isVarNotEmpty($attribs);
}

function we_tag_ifVarNotEmpty($attribs,$content){
	$foo = attributFehltError($attribs,"match","ifVarNotEmpty");if($foo){print($foo); return "";}
	return we_isVarNotEmpty($attribs);
}

function we_tag_ifVarSet($attribs,$content){

	$foo = attributFehltError($attribs,"name","ifVarSet");if($foo){print($foo); return "";}
...
function we_tag_img($attribs,$content) {
	...
	$foo = attributFehltError($attribs,"name","img");if($foo) return $foo;
    ...
    // images can now have custom attribs ...
	if( !(isset($_REQUEST['we_cmd'][0] ) && $_REQUEST['we_cmd'][0] == 'reload_editpage' && (isset($_REQUEST['we_cmd'][1]) && $name == $_REQUEST['we_cmd'][1]) && isset($_REQUEST['we_cmd'][2]) && $_REQUEST['we_cmd'][2] == 'change_image') && isset($GLOBALS['we_doc']->elements[$altField])){   // if no other image is selected.
	...
	if ($showcontrol && $we_editmode) {
		...
		if($id == ""){	// disable edit_image_button
			...
			$_editButton = $we_button->create_button("image:btn_edit_image", "javascript:top.doClickDirect($id,'image/*', '" . FILE_TABLE . "'  )");
		}
		$out .= $we_button->create_button_table(array( $_editButton, $we_button->create_button("image:btn_select_image", "javascript:we_cmd('openDocselector', '" . ($id!=""?$id:$startid) . "', '".FILE_TABLE."', 'document.forms[\\'we_form\\'].elements[\\'" . $fname . "\\'].value', '', 'opener.setScrollTo(); opener.top.we_cmd(\\'reload_editpage\\',\\'" . $name . "\\',\\'change_image\\'); opener.top.hot = 1;', '" . session_id() . "', ".$parentid.", 'image/*', " . (we_hasPerm("CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")", true), $we_button->create_button("image:btn_function_trash", "javascript:we_cmd('remove_image', '" . $name . "')", true)), 5) . "</td></tr></table>";
	}
	return $out;
}
...
function we_tag_input($attribs,$content) {
	...
	$foo = attributFehltError($attribs,"name","input");if($foo) return $foo;
	...
	$val = htmlspecialchars(isset($GLOBALS["we_doc"]->elements[$name]["dat"]) ? $GLOBALS["we_doc"]->getElement($name) : $value);

	if ($type == "date") {
		if ($we_editmode) {
			$d =abs($GLOBALS["we_doc"]->getElement($name));
			return getDateInput2("we_".$GLOBALS["we_doc"]->Name."_date[".$name."]",($d ? $d : time()),true,$format);
		} else {
		...
		if ($we_editmode) {
			...
			return '<input onchange="_EditorFrame.setEditorIsHot(true);" type="text" name="we_'.$GLOBALS["we_doc"]->Name.'_txt['.$name.']" value="'.$val.'"'.($attr ? " $attr" : "").'>'."&nbsp;" . (isset($sel) ? $sel : "");
		} else {
			return ($GLOBALS["we_doc"]->getElement($name));
		}
	} else if ($type == "select") {
	...
	<tr>
			...
			<td class="weEditmodeStyle">'.$we_button->create_button('image:spellcheck','javascript:we_cmd("spellcheck","we_'.$GLOBALS["we_doc"]->Name.'_txt['.$name.']")').'</td>
	</tr>
</table>';
			}else {
...
function we_tag_js($attribs,$content){
	...
	$foo = attributFehltError($attribs,"id","js");if($foo) return $foo;
	$id = we_getTagAttribute("id",$attribs);
	$row = getHash("SELECT Path,IsFolder,IsDynamic FROM ".FILE_TABLE." WHERE ID=$id",new DB_WE);
...
function we_tag_link($attribs,$content) {
	...
	$attribs = removeAttribs($attribs, array('text','id','imageid'));
	...
	if (!$we_editmode) {
		...
		if (is_array($link)) {
			...
			if ($startTag) {
				return $we_button->create_button_table(array($startTag . $content . "</a>", $editbut, $delbut), 5);
			} else {
				return $we_button->create_button_table(array($content, $editbut, $delbut), 5);
			}
		}
	}
	return "";
...
function we_tag_linkToSeeMode($attribs,$content) {
	...
	$foo = attributFehltError($attribs,"value","linkToSeeMode");if($foo) return $foo;
	...
	else if (isset($GLOBALS['we_obj']) || $oid) { // use object if possible
	...
	if (isset($_SESSION["webuser"]) && isset($_SESSION["webuser"]) && $_SESSION["webuser"]["registered"] && !isset($_REQUEST["we_transaction"])) {
		if ($permission == "" || isset($_SESSION["webuser"][$permission]) && $_SESSION["webuser"][$permission]) { // Has webUser the right permissions??
			...
			$tmpDB->query("SELECT ID FROM " . USER_TABLE . " WHERE username=\"" . $_SESSION["webuser"]["Username"] . "\" AND passwd=\"" . md5($_SESSION["webuser"]["Password"]). "\";");

			if ($tmpDB->num_rows() == 1) { // customer is also a user
				$retStr = getHtmlTag(   'form',
				                        array('method'=>'post', 'name'=>'startSeeMode_'.$type.'_'.$id, 'target'=>'_parent', 'action'=>'/webEdition/loginToSuperEasyEditMode.php'),
                                        getHtmlTag('input', array('type' => 'hidden','name' => 'username', 'value' => $_SESSION["webuser"]["Username"], 'xml' => $xml)) .
				                        getHtmlTag('input', array('type' => 'hidden','name' => 'passwd', 'value' => md5($_SESSION["webuser"]["Password"]), 'xml' => $xml)) .
				                        ...
				                        getHtmlTag('input', array('type' => 'hidden','name' => 'path', 'value' => WE_SERVER_REQUEST_URI, 'xml' => $xml))
				                    ) .
                          getHtmlTag('a', array('href'=>'javascript:document.forms[\'startSeeMode_'.$type.'_'.$id.'\'].submit();', 'xml' => $xml), $value);
...
function we_tag_linklist($attribs,$content){
	...
	$foo = attributFehltError($attribs,"name","linklist");if($foo) return $foo;
	$isInListview = isset($GLOBALS["lv"]);

	if($isInListview){
		$linklist = $GLOBALS["lv"]->f($name);
	}else if(isset($GLOBALS["we_doc"])){
		$linklist = $GLOBALS["we_doc"]->getElement($name);
	}
	$ll = new we_linklist($linklist);
	$ll->name = $name;

	$out = $ll->getHTML((isset($GLOBALS["we_editmode"]) && $GLOBALS["we_editmode"] && (!$isInListview)),$attribs,$content,$GLOBALS["we_doc"]->Name);
	return $out;
}
...
function we_tag_list($attribs,$content){
	...
	if($we_editmode)	$we_button = new we_button();

	$foo = attributFehltError($attribs,"name","list");if($foo) return $foo;
	$name = we_getTagAttribute("name",$attribs);
	...
	if($list){
		...
		for($i=0;$i<$listlen;$i++){
			...
			if($we_editmode){
				...
				$downbut    = $we_button->create_button("image:btn_direction_down", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(true);we_cmd('down_entry_at_list','$name','$i');");
				$downbutDis = $we_button->create_button("image:btn_direction_down", "", true, 21, 22, "", "", true);
				$plusbut    = $we_button->create_button("image:btn_add_listelement", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(true);we_cmd('insert_entry_at_list','$name','$i');");
				$trashbut   = $we_button->create_button("image:btn_function_trash", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(true);we_cmd('delete_list','$name','$i','$names');");


				if(!$isInListview){

					$buts = $we_button->create_button_table(array(	$plusbut,
																(($i>0) ? $upbut : $upbutDis),
																(($i<($listlen-1)) ? $downbut : $downbutDis),
																$trashbut),5);
				}
	...
	if($we_editmode){
		$plusbut = $we_button->create_button("image:btn_add_listelement", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(true);we_cmd('add_entry_to_list','$name')");
	...
	$out .= ((!$isInListview) && $we_editmode) ? ('<input type="hidden" name="we_'.$GLOBALS["we_doc"]->Name.'_list['.$name.']" value="'.htmlspecialchars($list).'"><input type="hidden" name="we_'.$GLOBALS["we_doc"]->Name.'_list['.$name.'#content]" value="'.htmlspecialchars($content).'">'.$plusbut) : '';
	//	When in SEEM - Mode add edit-Button to tag - textarea
	return $out;
}
...
function we_tag_listdir($attribs,$content){
	...
	$db->query("SELECT ID,Text,IsFolder,Path FROM ".FILE_TABLE." WHERE ((Published > 0 AND IsSearchable = 1) OR (IsFolder = 1)) AND ParentID='$dirID'");
	...
	while($db->next_record()){
		...
		if($db->f("IsFolder")){
			$db2->query("SELECT ID FROM ".FILE_TABLE." WHERE ParentID='".$db->f("ID")."' AND IsFolder = 0 AND ($q) AND (Published > 0 AND IsSearchable = 1)");
			if($db2->next_record()){
				if($sort){
					$db3->query("SELECT ".CONTENT_TABLE.".Dat as Dat FROM ".LINK_TABLE.",".CONTENT_TABLE." WHERE ".LINK_TABLE.".DID='".$db2->f("ID")."' AND ".LINK_TABLE.".Name='$sort' AND ".CONTENT_TABLE.".ID = ".LINK_TABLE.".CID");
					if($db3->next_record()){
						$sortfield = $db3->f("Dat");
					}else{
						$sortfield = $db->f("Text");
				...
				if($dirfield){
					$db3->query("SELECT ".CONTENT_TABLE.".Dat as Dat FROM ".LINK_TABLE.",".CONTENT_TABLE." WHERE ".LINK_TABLE.".DID='".$db2->f("ID")."' AND ".LINK_TABLE.".Name='$dirfield' AND ".CONTENT_TABLE.".ID = ".LINK_TABLE.".CID");
					if($db3->next_record()){
						$namefield = $db3->f("Dat");
					}else{
						$namefield = $db->f("Text");
			...
			if($sort){
				$db2->query("SELECT ".CONTENT_TABLE.".Dat as Dat FROM ".LINK_TABLE.",".CONTENT_TABLE." WHERE ".LINK_TABLE.".DID='".$db->f("ID")."' AND ".LINK_TABLE.".Name='$sort' AND ".CONTENT_TABLE.".ID = ".LINK_TABLE.".CID");
				if($db2->next_record()){
					$sortfield = $db2->f("Dat");
				}else{
					$sortfield = $db->f("Text");
			...
			if($name){
				$db2->query("SELECT ".CONTENT_TABLE.".Dat as Dat FROM ".LINK_TABLE.",".CONTENT_TABLE." WHERE ".LINK_TABLE.".DID='".$db->f("ID")."' AND ".LINK_TABLE.".Name='$name' AND ".CONTENT_TABLE.".ID = ".LINK_TABLE.".CID");
				if($db2->next_record()){
					$namefield = $db2->f("Dat");
				}else{
					$namefield = $db->f("Text");
	...
	foreach($files as $i=>$v){
		...
		$foo = ereg_replace('<we:ifSelf[^>]*>','<?php if("'.$GLOBALS["WE_MAIN_DOC"]->ID.'" == "'.$id.'"): ?>',$foo);
		$foo = ereg_replace('</we:ifSelf[^>]*>','<?php endif ?>',$foo);
		$foo = ereg_replace('<we:ifNotSelf[^>]*>','<?php if("'.$GLOBALS["WE_MAIN_DOC"]->ID.'" != "'.$id.'"): ?>',$foo);
		$foo = ereg_replace('</we:ifNotSelf[^>]*>','<?php endif ?>',$foo);
		$foo = ereg_replace('</we:else[^>]*>','<?php else: ?>',$foo);
		$foo = ereg_replace('<we:else[^/]*/>','<?php else: ?>',$foo);

		//	parse we:ifPosition
		if( strpos($foo, 'setVar') || strpos($foo, 'position') || strpos($foo, 'ifPosition') || strpos($foo, 'ifNotPosition') ){
			$tp = new we_tagParser();
			$tags = $tp->getAllTags($foo);

			$tp->parseTags($tags,$foo);
			$foo =	'<?php $GLOBALS[\'we_position\'][\'listdir\'] = array(\'position\' => ' . ($i+1) . ', \'size\' => ' . sizeof($files) . ', \'field\' => \'' . $field . '\', \'id\' => \'' . $id . '\', \'path\' => \'' . $path . '\'); ?>' .
					$foo .
					'<?php unset($GLOBALS[\'we_position\'][\'listdir\']); ?>';
		}
...
function we_tag_listviewEnd($attribs,$content){
	return $GLOBALS["lv"]->rows ?
		min(($GLOBALS["lv"]->start - abs($GLOBALS["lv"]->offset)) + $GLOBALS["lv"]->rows,($GLOBALS["lv"]->anz_all - abs($GLOBALS["lv"]->offset))) :
		($GLOBALS["lv"]->anz_all - abs($GLOBALS["lv"]->offset));
}

function we_tag_listviewPageNr($attribs,$content){
	return $GLOBALS["lv"]->rows ?
 		(((abs($GLOBALS["lv"]->start) - abs($GLOBALS["lv"]->offset)) / $GLOBALS["lv"]->rows) + 1):
 		1;
}

function we_tag_listviewPages($attribs,$content){
	return $GLOBALS["lv"]->rows ?
 		ceil(((float)$GLOBALS["lv"]->anz_all - abs($GLOBALS["lv"]->offset)) / (float)$GLOBALS["lv"]->rows) :
 		1;
}
...
function we_tag_next($attribs,$content){
	if(isset($GLOBALS["_we_voting_list"])) return $GLOBALS["_we_voting_list"]->getNextLink($attribs);
	else return $GLOBALS["lv"]->getNextLink($attribs);
}
...
function we_tag_options ($attribs, $content) {
	...
	if($classid && $field){
		...
		if(!isset($GLOBALS["WE_OBJECT_DEFARRAY"]["cid_$classid"])){
			$db = new DB_WE();
			$GLOBALS["WE_OBJECT_DEFARRAY"]["cid_$classid"] = unserialize(f("SELECT DefaultValues FROM ".OBJECT_TABLE." WHERE ID='$classid'","DefaultValues",$db));
		}
...
function we_tag_path ($attribs, $content) {
	...
	if(!in_array($doc->Text,$indexArray)){
		if(!$show) $show = $doc->Text;
		$path = $htmlspecialchars ? htmlspecialchars($sep.$show) : $sep.$show;
	}
	while($pID){
		$db->query("SELECT ID,Path FROM ".FILE_TABLE." WHERE ParentID='$pID' AND IsFolder = 0 AND ($q) AND (Published > 0 AND IsSearchable = 1)");
		$db->next_record();
		$fileID = $db->f("ID");
		$filePath = $db->f("Path");
		if($fileID){
			$show = f("SELECT ".CONTENT_TABLE.".Dat as Dat FROM ".LINK_TABLE.",".CONTENT_TABLE." WHERE ".LINK_TABLE.".DID='$fileID' AND ".LINK_TABLE.".Name='$dirfield ' AND ".CONTENT_TABLE.".ID = ".LINK_TABLE.".CID","Dat",$db);
			if(!$show) $show = f("SELECT Text FROM ".FILE_TABLE." WHERE ID='$pID'","Text",$db);
	...
	$filePath = $db->f("Path");
	if($fileID){
		$show = f("SELECT ".CONTENT_TABLE.".Dat as Dat FROM ".LINK_TABLE.",".CONTENT_TABLE." WHERE ".LINK_TABLE.".DID='$fileID' AND ".LINK_TABLE.".Name='$field' AND ".CONTENT_TABLE.".ID = ".LINK_TABLE.".CID","Dat",$db);
		if(!$show) {
			$show = $home;
		}
		$link_pre = '<a href="'.$filePath.'"'.$class.$style.'>';
...
function we_tag_printVersion($attribs,$content){
	$foo = attributFehltError($attribs,"tid","printVersion");if($foo) return $foo;
...
function we_tag_processDateSelect($attribs,$content){
	...
	$GLOBALS[$name] = $_REQUEST[$name] =	mktime(	$endofday ? 23 : 0,
															$endofday ? 59 : 0,
															$endofday ? 59 : 0,
...
function we_tag_quicktime($attribs,$content) {
	...
	$foo = attributFehltError($attribs,"name","quicktime");if($foo) return $foo;
	...
	$attribs = removeAttribs($attribs, array('showcontrol','showquicktime'));
	...
	if ($showcontrol && $we_editmode) {
		include_once($_SERVER["DOCUMENT_ROOT"]."/webEdition/we/include/we_classes/html/we_button.inc.php");
		$we_button = new we_button();
		$quicktime_button = $we_button->create_button("image:btn_edit_quicktime", "javascript:we_cmd('openDocselector','" . $id . "', '".FILE_TABLE."', 'document.forms[\'we_form\'].elements[\'" . $fname . "\'].value', '', 'opener.setScrollTo();opener.top.we_cmd(\'reload_editpage\');opener._EditorFrame.setEditorIsHot(true);', '" . session_id() . "', 0, 'video/quicktime', " . (we_hasPerm("CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")", true);
		$clear_button = $we_button->create_button("image:btn_function_trash", "javascript:we_cmd('remove_image', '" . $name . "')", true);
		...
		$out = "
			<table border=\"0\" cellpadding=\"0\" cellspacing=\"2\" background=\"" . IMAGE_DIR . "backgrounds/aquaBackground.gif\" style=\"border: solid #006DB8 1px;\">
				...
				<tr>
					<td class=\"weEditmodeStyle\" align=\"center\">";
		$out .= $we_button->create_button_table(array($quicktime_button, $clear_button), 5) . "</td></tr></table>";
	}
	//	When in SEEM - Mode add edit-Button to tag - textarea
	return $out;
}
...
function we_tag_registeredUser($attribs,$content){
	...
	if(ereg("^field:(.+)$",$id,$regs)){
		$doc = we_getDocForTag($docAttr);
		$field = $regs[1];
		if(strlen($field)) $id = $doc->getElement($field);
	}
	if($id){
		$db = new DB_WE();
...
function we_tag_returnPage($attribs,$content){
	...
	return isset($_REQUEST["we_returnpage"]) ? (getXmlAttributeValueAsBoolean($xml) ? htmlspecialchars($_REQUEST["we_returnpage"]) : $_REQUEST["we_returnpage"]) : "";
}
...
function we_tag_search($attribs,$content){
	...
	$searchValue = htmlspecialchars(str_replace("\"","",str_replace("\\\"","", (isset($_REQUEST["we_lv_search_".$name]) ? trim($_REQUEST["we_lv_search_".$name]) : $value)  )));
	if($type == "print"){
		return $searchValue;
	}else{

	    $attsHidden = array('type' => 'hidden', 'xml' => $xml, 'name' => 'we_from_search_'.$name, 'value' => (isset($GLOBALS["we_editmode"]) && $GLOBALS["we_editmode"] ? 0 : 1));

		$hidden = getHtmlTag('input', $attsHidden);
		if($type == "textinput"){

		    $atts = removeAttribs($attribs, array('type','onchange','name','cols','rows'));
		    $atts = array_merge($atts, array('name'=>"we_lv_search_$name",'type'=>'text','value'=>$searchValue,'xml'=>$xml));
			return getHtmlTag('input', $atts) . $hidden;

		}else if($type == "textarea"){

		    $atts = removeAttribs($attribs, array('type','onchange','name','size','maxlength','value'));
		    $atts = array_merge($atts, array('class'=>'defaultfont', 'name'=>"we_lv_search_$name", 'xml' => $xml));
...
function we_tag_select($attribs,$content){
	global $we_editmode,$l_global;
	$foo = attributFehltError($attribs,"name","select");if($foo) return $foo;
	$name = we_getTagAttribute("name",$attribs);
	...
	if($we_editmode){
		...
		if($val){
			...
			if(eregi("<option>",$content)) $content = eregi_replace('<option>'.quotemeta($val)."( ?[<\n\r\t])",'<option selected>'.$val.'\1',$content);
			if(eregi('<option value=[\'"]?.*[\'"]?>',$content))
				$content = eregi_replace('<option value=[\'"]?'.quotemeta($val).'[\'"]?>','<option value="'.$val.'" selected>',$content);
		}
...
function we_tag_sendMail($attribs,$content){
	...
	$foo = attributFehltError($attribs,"recipient","sendMail");if($foo) return $foo;
	$foo = attributFehltError($attribs,"from","sendMail");if($foo) return $foo;
	...
	if(!$GLOBALS["we_doc"]->InWebEdition){
		...
		if (!empty($id)) {
			...
			if ($mimetype != "text/html") {
				$we_mailcontent = strip_tags(str_replace("&nbsp;"," ",str_replace("<br />","\n",str_replace("<br>","\n",$codes))));
			} else {
			...
			for ($l=0;$l < sizeof($to);$l++) {
		    	...
		    	if($we_recipient){
		    		$m = new we_mailer($we_recipient, $subject, $we_mailcontent, $from, $reply, ($mimetype == "text/html"), $charset);
					$m->send();
		    	}
			}
		}
		...
		if( isset($_REQUEST["we_webUser_logout"]) && $_REQUEST["we_webUser_logout"] ){
				...
				if(!isset($GLOBALS["we_editmode"]) || !$GLOBALS["we_editmode"]){
					...
					if(isset($_REQUEST["s"]["Username"]) && isset($_REQUEST["s"]["Password"]) && !(isset($_REQUEST["s"]["ID"]) ) ){
						$u = getHash('SELECT * from '.CUSTOMER_TABLE.' WHERE Username="'.$_REQUEST['s']["Username"].'"',$GLOBALS["DB_WE"]);
						if (isset($u["Password"]) &&  $u["LoginDenied"] != 1) {
							if($_REQUEST['s']["Username"] == $u["Username"] && $_REQUEST['s']["Password"] == $u["Password"]){
								...
								$_SESSION["webuser"] = array("registered"=>false,"loginfailed"=>true);
							}
...
function we_tag_setVar($attribs,$content){
	$foo = attributFehltError($attribs,"nameto","setVar");if($foo) return $foo;
	$foo = attributFehltError($attribs,"to","setVar");if($foo) return $foo;
	...
	if(isset($attribs["value"])){
		...
		switch($from){
			...
			case "top":
				if($propertyFrom){
					eval('$valueFrom = isset($GLOBALS["WE_MAIN_DOC"]->'.$nameFrom.') ? $GLOBALS["WE_MAIN_DOC"]->'.$nameFrom.' : "";');
				}else{
					if ($typeFrom == "href") {
						$valueFrom = isset($GLOBALS["WE_MAIN_DOC"]->elements[$nameFrom . '_we_jkhdsf_int']) ? $GLOBALS["WE_MAIN_DOC"]->getField(array("name"=>$nameFrom),$typeFrom,true) : "";
					} else {
						$valueFrom = isset($GLOBALS["WE_MAIN_DOC"]->elements[$nameFrom]) ? $GLOBALS["WE_MAIN_DOC"]->getField(array("name"=>$nameFrom),$typeFrom,true) : "";
					}
				}
				break;
			case "self":
				if($propertyFrom){
					eval('$valueFrom = isset($GLOBALS["we_doc"]->'.$nameFrom.') ? $GLOBALS["we_doc"]->'.$nameFrom.' : "";');
				}else{
					if ($typeFrom == "href") {
						$valueFrom = isset($GLOBALS["we_doc"]->elements[$nameFrom . '_we_jkhdsf_int']) ? $GLOBALS["we_doc"]->getField(array("name"=>$nameFrom),$typeFrom,true) : "";
					} else {
						$valueFrom = isset($GLOBALS["we_doc"]->elements[$nameFrom]) ? $GLOBALS["we_doc"]->getField(array("name"=>$nameFrom),$typeFrom,true) : "";
					}
				}
				break;
			case "object":
			case "document":
				if($propertyFrom){
					eval('$valueFrom = isset($GLOBALS["we_'.$from.'"][$formnameFrom]->'.$nameFrom.') ? $GLOBALS["we_'.$from.'"][$formnameFrom]->'.$nameFrom.' : "";');
				}else{
            ...
            case "listview":
                ...
                $valueFrom = we_tag_field(array('name'=>$nameFrom,'type'=>$typeFrom),"");
                break;
			case "block":
	...
	switch($to){
		...
		case "document":
			if($propertyTo){
				if(isset($GLOBALS["we_$to"][$formnameTo])) eval('$GLOBALS["we_$to"][$formnameTo]->'.$nameTo.' = $valueFrom;');
			}else{
				if(isset($GLOBALS["we_$to"][$formnameTo])) $GLOBALS["we_$to"][$formnameTo]->setElement($nameTo,$valueFrom);
			}
			break;
		case "sessionfield":
			if(isset($_SESSION["webuser"][$nameTo])) $_SESSION["webuser"][$nameTo] = $valueFrom;
	}
...
function we_tag_textarea($attribs,$content){
	...
	$foo = attributFehltError($attribs,"name","textarea");if($foo) return $foo;
	...
	if($we_editmode){
		return we_forms::weTextarea($fieldname,$value,$attribs,$autobr,$autobrName,$showAutobr,$GLOBALS["we_doc"]->getHttpPath(),false,false,getXmlAttributeValueAsBoolean($xml),$removeFirstParagraph,'',($spellcheck=='true'));
	}else{
...
<!--
_my_stat_write(\''.$websitename.'\',\''.$trackerurl.'\''.($trackname ? (",'".addslashes($trackname)."'") : "").');
//-->
</script>
<noscript>
<img width="1" height="1"  alt="" src="'.$trackerurl.'/connector.php?'.$websitename.'&amp;mode=NOSCRIPT'.($trackname ? ("&amp;trackname=".rawurlencode($trackname)) : "").'" />
</noscript>
<!-- pageLogger Code END -->
';
	} else if ($type == 'robot') {
...
function we_tag_url($attribs,$content){
	$foo = attributFehltError($attribs,"id","url");if($foo) return $foo;
	static $urls=array();
	$id = we_getTagAttribute("id",$attribs);
...
function we_tag_userInput($attribs,$content){
	$foo = attributFehltError($attribs, "name", "userInput");if($foo) return $foo;
	...
	$fieldname = $property ? ("we_ui_" . (isset($GLOBALS["WE_FORM"]) ? $GLOBALS["WE_FORM"] : "") . "_" . $name) : ("we_ui_" . (isset($GLOBALS["WE_FORM"]) ? $GLOBALS["WE_FORM"] : "") . "[" . $name . "]");

	$objekt = (
				isset($GLOBALS["WE_FORM"]) ?
					(isset($GLOBALS["we_object"][$GLOBALS["WE_FORM"]]) ?
						...
						(isset($GLOBALS["we_document"][$GLOBALS["WE_FORM"]])
							...
							(isset($GLOBALS["we_doc"]) ? $GLOBALS["we_doc"] : false)))
	...
	$content = we_document::getFieldByVal($orgVal,
											$type,
											$attribs,
											true,
											$object_pid,
	...
	if(!$editable && $type !== "img"){
		$_hidden = getHtmlTag('input', array('type'=>'hidden', 'name'=>$fieldname, 'value'=>htmlspecialchars($orgVal),'xml'=>$xml));
		return (($type != "hidden") ? $content : "").$_hidden;
	}else{
		switch($type){
			case "img":

				$_imgDataId = isset($_REQUEST['WE_UI_IMG_DATA_ID_' . $name]) ? $_REQUEST['WE_UI_IMG_DATA_ID_' . $name] : md5(uniqid(rand()));
				...
				if ($editable) {
					...
					$foo = attributFehltError($attribs, "parentid", "userInput");if($foo) return $foo;
					...
					$_SESSION[$_imgDataId]["width"] = we_getTagAttribute("width",$attribs,0);
					...
					if (isset($_SESSION[$_imgDataId]["serverPath"])) {
						...
						return '<img src="'.$src.'" alt="" width="'.$_SESSION[$_imgDataId]["imgwidth"].'" height="'.$_SESSION[$_imgDataId]["imgheight"].'" />'.
							$hidden;
					} else if (isset($_SESSION[$_imgDataId]["id"]) && $_SESSION[$_imgDataId]["id"]){
			...
			case "textarea":
				...
				if($pure){
				    $atts = removeAttribs($attribs, array('wysiwyg','commands','pure','type','value','checked','autobr','name','values','hidden','editable','format','property','size','maxlength','width','height','fontnames','bgcolor'));
					...
					$showAutobr = isset($attribs["autobr"]);
					$charset = we_getTagAttribute("charset",$attribs,"iso-8859-1");
					return we_forms::weTextarea($fieldname,$content,$attribs,$autobr,"autobr",$showAutobr,$GLOBALS["we_doc"]->getHttpPath(),false,false,getXmlAttributeValueAsBoolean($xml),$removeFirstParagraph,$charset,false,true);
				}
			case "checkbox":
			    $atts = removeAttribs($attribs, array('wysiwyg','commands','pure','type','value','checked','autobr','name','values','hidden','editable','format','property','cols','rows','width','height','bgcolor','fontnames'));
				if((!$isset) && $checked){
                    $content = 1;
				}
			...
			case "date":
				...
				if($hidden){
					...
					return getDateInput2("we_ui_" . (isset($GLOBALS["WE_FORM"]) ? $GLOBALS["WE_FORM"] : "") . "[we_date_".$name."]",($orgVal ? $orgVal : time()),false,$format, '', '', $xml);
				}
				break;
			case "select";
			    $options = '';
			    $atts = removeAttribs($attribs, array('wysiwyg','commands','pure','type','value','checked','autobr','name','values','hidden','editable','format','property','rows','cols','fontnames','bgcolor','width','height','maxlength'));
				if($values){
					...
					foreach($values as $txt){
					    ...
					    if($txt == $orgVal){
					        $attsOption = array('selected' => 'selected');
					    } else {
					        $attsOption = array();
					    }
					    $options .= getHtmlTag('option', $attsOption, trim($txt), true) . "\n";
					}
				}else if($objekt &&  isset($objekt->DefArray["meta_".$name])){
					foreach($objekt->DefArray["meta_".$name]["meta"] as $key=>$val){

					    if($orgVal == $key){
					        $atts2 = array('value'=>$key,'selected' => 'selected');
					    } else {
					        $atts2 = array('value'=>$key);
					    }
				...
				break;
			case "radio";
                $atts = removeAttribs($attribs, array('wysiwyg','commands','pure','type','value','checked','autobr','name','values','hidden','editable','format','property','rows','cols','width','height','bgcolor','fontnames'));
				if(!$isset){
			...
			case "hidden";
				...
				return getHtmlTag('input', $attsHidden);
			case "choice";
			    $atts = removeAttribs($attribs, array('wysiwyg','commands','pure','type','value','checked','autobr','name','values','hidden','editable','format','property','cols','rows','width','height','bgcolor','fontnames','maxlength'));
                $mode = we_getTagAttribute("mode",$attribs);
...
function we_tag_var($attribs,$content){
	$foo = attributFehltError($attribs,"name","var");if($foo) return $foo;
	$docAttr = we_getTagAttribute("doc",$attribs);
	$name = we_getTagAttribute("name",$attribs);
	$type = we_getTagAttribute("type",$attribs);
    	    	...
    	    	if(isset($doc->DefArray) && is_array($doc->DefArray)) {
	    	        foreach ($doc->DefArray as $_glob_key => $_val){
	    	            ...
	    	            if (substr($_glob_key,0,7)=="object_"){
	    	                ...
	    	                $normVal = we_document::getFieldByVal($doc->getElement($name),
	    							$type,
	    							$attribs,
	    							false,
	    							$GLOBALS["we_doc"]->ParentID,
...
function we_tag_write($attribs,$content){
	...
	if($type=="object"){
		$foo = attributFehltError($attribs,"classid","write");if($foo) return $foo;
	}else{
		$foo = attributFehltError($attribs,"doctype","write");if($foo) return $foo;
	}

	$name = we_getTagAttribute("formname",$attribs,((isset($GLOBALS["WE_FORM"]) && $GLOBALS["WE_FORM"] ) ? $GLOBALS["WE_FORM"] : "we_global_form"));
	...
	if(isset($_REQUEST["edit_$type"]) &&  $_REQUEST["edit_$type"]){
		...
		if($ok){
			...
			if($isAdmin || ($GLOBALS["we_$type"][$name]->ID==0)  || $isOwner || $forceedit){
				...
				if ($protected) {
					if (!isset($_SESSION["webuser"]["ID"])) return;
				...
				if (defined("OBJECT_FILES_TABLE") && $type=="object" && $GLOBALS["we_$type"][$name]->Text=="") {
					$GLOBALS["we_$type"][$name]->Text = 1 + abs(f("SELECT max(ID) as ID FROM " . OBJECT_FILES_TABLE ,"ID",new DB_WE()));
				}
				...
				if($mail){
					...
					if($type=="object"){
						$classname = f("SELECT Text FROM ".OBJECT_TABLE." WHERE ID='".$classid."'","Text",$GLOBALS["DB_WE"]);
						if($triggerid){
							$port = (defined("HTTP_PORT")) ? (":".HTTP_PORT) : "";
							$mailtext = sprintf($GLOBALS["l_global"]["std_mailtext_newObj"],$path,$classname)."\n"."http://".$GLOBALS["SERVER_NAME"].$port.id_to_path($triggerid)."?we_objectID=".$GLOBALS["we_object"][$name]->ID;
						}else{
...
function we_tag_checkForm($attribs, $content) {
    ...
    if($onError){
        $jsOnError = '
            if(self.' . $onError . '){
            	...
            	' . we_message_reporting::getShowMessageCall($content, WE_MESSAGE_FRONTEND) . '
            }
        ';
    } else {
    ...
    if($jsIncludePath){
        ...
        if(is_numeric($jsIncludePath)){
            $jsTag = we_tag_js(array('id'=>$jsIncludePath, 'xml'=>$xml));
            if($jsTag){
...
function we_tag_xmlfeed($attribs, $content){
	...
	$foo = attributFehltError($attribs,"name","xmlfeed");if($foo) return $foo;
	$foo = attributFehltError($attribs,"url","xmlfeed");if($foo) return $foo;

	$name = we_getTagAttribute("name",$attribs);
	$url = we_getTagAttribute("url",$attribs);

	if(isset($attribs["refresh"]) && is_numeric($attribs["refresh"])) $refresh=$attribs["refresh"]*60;
	else $refresh=0;

	if(!isset($GLOBALS["xmlfeeds"])) $GLOBALS["xmlfeeds"]=array();
