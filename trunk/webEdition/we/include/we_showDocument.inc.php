...
if(isset($_REQUEST['cmd']) && $_REQUEST['cmd']!="ResetVersion") {
	if(isset($FROM_WE_SHOW_DOC)  &&  $FROM_WE_SHOW_DOC) { // when called showDoc.php
		...
		else if($we_doc->IsDynamic && (!$publ) && (!$tmplID)) { // if the document is a dynamic php-doc and is not published we make a redirect to dummy page that does not exist, so the user gets a 404-Errpr Page from the server
