...
/**
 * /webEdition/we/include/we_widgets/mod/pad.inc.php
 * 
 * Displays the note list and the dialog for edit or add notes
 * 
 * @package Widget
 * @subpackage Notepad
 */
...
// validity settings
$sctValid = htmlSelect("sct_valid",array($l_cockpit['always'],$l_cockpit['from_date'],$l_cockpit['period']),1,$l_cockpit['always'],
	false,'style="width:100px;" onChange="toggleTblValidity()"','value',100,'middlefont');
$oTblValidity = new we_htmlTable(array("cellpadding"=>"0","cellspacing"=>"0","border"=>"0", "id"=>"oTblValidity"),1,3);
...
$oTblPeriod = new we_htmlTable(array("width"=>"100%","cellpadding"=>"0","cellspacing"=>"0","border"=>"0"),1,2);
$oTblPeriod->setCol(0,0,array("class"=>"middlefont"),$sctValid);
$oTblPeriod->setCol(0,1,array("align"=>"right"),$oTblValidity->getHTMLCode());

// Edit note prio settings
$rdoPrio[0] = we_forms::radiobutton($value=0,$checked=0,$name="rdo_prio",$text=$l_cockpit['high'],$uniqid=true,
	$class="middlefont",$onClick="",$disabled=false,$description="",$type=0,$onMouseUp="");
$rdoPrio[1] = we_forms::radiobutton($value=1,$checked=0,$name="rdo_prio",$text=$l_cockpit['medium'],$uniqid=true,
	$class="middlefont",$onClick="",$disabled=false,$description="",$type=0,$onMouseUp="");
$rdoPrio[2] = we_forms::radiobutton($value=2,$checked=1,$name="rdo_prio",$text=$l_cockpit['low'],$uniqid=true,
	$class="middlefont",$onClick="",$disabled=false,$description="",$type=0,$onMouseUp="");
$oTblPrio = new we_htmlTable(array("cellpadding"=>"0","cellspacing"=>"0","border"=>"0"),1,8);
$oTblPrio->setCol(0,0,null,$rdoPrio[0]);
$oTblPrio->setCol(0,1,null,we_htmlElement::htmlImg(array("src"=>IMAGE_DIR."pd/prio_high.gif","width"=>13,"height"=>14,"style"=>"margin-left:5px")));
$oTblPrio->setCol(0,2,null,getPixel(15,1));
$oTblPrio->setCol(0,3,null,$rdoPrio[1]);
$oTblPrio->setCol(0,4,null,we_htmlElement::htmlImg(array("src"=>IMAGE_DIR."pd/prio_medium.gif","width"=>13,"height"=>14,"style"=>"margin-left:5px")));
$oTblPrio->setCol(0,5,null,getPixel(15,1));
$oTblPrio->setCol(0,6,null,$rdoPrio[2]);
$oTblPrio->setCol(0,7,null,we_htmlElement::htmlImg(array("src"=>IMAGE_DIR."pd/prio_low.gif","width"=>13,"height"=>14,"style"=>"margin-left:5px")));
...
// Edit note dialog
$oTblProps = new we_htmlTable(array("width"=>"100%","cellpadding"=>"0","cellspacing"=>"0","border"=>"0"),9,2);
$oTblProps->setCol(0,0,array("class"=>"middlefont"),$l_cockpit['valid'].'&nbsp;');
$oTblProps->setCol(0,1,array("colspan"=>2,"align"=>"right"),$oTblPeriod->getHTMLCode());
$oTblProps->setCol(1,0,null,getPixel(1,8));
$oTblProps->setCol(2,0,array("class"=>"middlefont"),$l_cockpit['prio']);
$oTblProps->setCol(2,1,null,$oTblPrio->getHTMLCode());
$oTblProps->setCol(3,0,null,getPixel(1,8));
$oTblProps->setCol(4,0,array("class"=>"middlefont"),$l_cockpit['title']);
$oTblProps->setCol(4,1,null,htmlTextInput($name="props_title",$size=255,$value="",$maxlength=255,$attribs="",$type="text",$width="100%",$height=0));
$oTblProps->setCol(5,0,null,getPixel(1,8));
$oTblProps->setCol(6,0,array("class"=>"middlefont", "valign"=>"top"),$l_cockpit['note']);
$oTblProps->setCol(6,1,null,we_htmlElement::htmlTextArea(array('name'=>'props_text','id'=>'previewCode','style'=>'width:100%;height:60px;','class'=>'wetextinput','onblur'=>'this.className=\'wetextinput\';','onfocus'=>'this.className=\'wetextinputselected\''),""));
$oTblProps->setCol(7,0,null,getPixel(1,8));
$oTblProps->setCol(8,0,array("colspan"=>3),$buttons);

// Button: add note
$oTblBtnProps = new we_htmlTable(array("width"=>"100%","cellpadding"=>"0","cellspacing"=>"0","border"=>"0"),1,1);
$oTblBtnProps->setCol(0,0,array("align"=>"right"),$we_button->create_button("image:btn_add_note","javascript:displayNote();",false,-1,-1));

// Table with the note list
$oPad = new we_htmlTable(array("width"=>"100%","cellpadding"=>"0","cellspacing"=>"0","border"=>"0","style"=>"table-layout:fixed;"),3,3);
$oPad->setCol(0,0,array("width"=>"6"),we_htmlElement::htmlImg(array("src"=>IMAGE_DIR."pd/pad_corner_lt.gif","width"=>6,"height"=>4)));
$oPad->setCol(0,1,array("class"=>"cl_notes"),"");
$oPad->setCol(0,2,array("width"=>"6"),we_htmlElement::htmlImg(array("src"=>IMAGE_DIR."pd/pad_corner_rt.gif","width"=>6,"height"=>4)));
$oPad->setCol(1,0,array("colspan"=>3,"class"=>"cl_notes"),we_htmlElement::htmlDiv(array("id"=>"notices"),getNoteList($_sql,$bDate,$bDisplay)));
$oPad->setCol(2,0,array("width"=>"6"),we_htmlElement::htmlImg(array("src"=>IMAGE_DIR."pd/pad_corner_lb.gif","width"=>6,"height"=>6)));
$oPad->setCol(2,1,array("class"=>"cl_notes"),"");
$oPad->setCol(2,2,array("width"=>"6"),we_htmlElement::htmlImg(array("src"=>IMAGE_DIR."pd/pad_corner_rb.gif","width"=>6,"height"=>6)));

$_notepad = $oPad->getHTMLCode().
	we_htmlElement::htmlDiv(array("id"=>"props"),$oTblProps->getHTMLCode()).
	we_htmlElement::htmlDiv(array("id"=>"view"),$oTblBtnProps->getHTMLCode());
...
print we_htmlElement::htmlHtml(
	we_htmlElement::htmlHead(
		...
		STYLESHEET.
		we_htmlElement::cssElement(getCSS()).
		...
		we_htmlElement::jsElement((($_REQUEST["we_cmd"][6]=="pad/pad")? "
			var _sObjId='".$_sObjId."';
					...
					if(hot){
						...
						if(q_curr['Validity'] == 'period') {
							...
							if(weValidFrom>weValidUntil) {
								" . we_message_reporting::getShowMessageCall($l_cockpit['until_befor_from'], WE_MESSAGE_NOTICE) . "
								return false;
							}
						}
						if(q_curr['Title']=='') {
							" . we_message_reporting::getShowMessageCall($l_cockpit['title_empty'], WE_MESSAGE_NOTICE) . "
							return false;
						}
						var q_ID=gel(_id+'_ID').value;
						parent.rpc(_ttlB64Esc.concat(','+_sInitProps),(q_ID+';'+escape(csv)),'update','',_ttlB64Esc,_sObjId,'pad/pad');
					}else{
						" . we_message_reporting::getShowMessageCall($l_cockpit['note_not_modified'], WE_MESSAGE_NOTICE) . "
					}
				}else{
					if(hot){
						// insert note
						if(q_curr['Validity'] == 'period') {
							...
							if(weValidFrom>weValidUntil) {
								...
								" . we_message_reporting::getShowMessageCall($l_cockpit['date_empty'], WE_MESSAGE_NOTICE) . "
								return false;
							}
	...
	we_htmlElement::htmlBody(array(
			...
			we_htmlElement::htmlDiv(array("id"=>"pad"),
			$_notepad.we_htmlElement::htmlHidden(array("name"=>"mark","value"=>"")).we_htmlElement::jsElement("calendarSetup();"))
		)
	)
);
