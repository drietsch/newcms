...
switch ($iDate) {
	case 1: $timestamp = mktime(0,0,0,date('m'),date('d'),date('Y')); break;
	case 2: $iTime = time()-(7*24*60*60); $timestamp = mktime(date('H',$iTime),date('i',$iTime),date('s',$iTime),date('m',$iTime),date('d',$iTime),date('Y',$iTime)); break;
	case 3: $iTime = time()-(30*24*60*60); $timestamp = mktime(date('H',$iTime),date('i',$iTime),date('s',$iTime),date('m',$iTime),date('d',$iTime),date('Y',$iTime)); break;
	case 4: $iTime = time()-(365*24*60*60); $timestamp = mktime(date('H',$iTime),date('i',$iTime),date('s',$iTime),date('m',$iTime),date('d',$iTime),date('Y',$iTime)); break;
}
$iNumItems = $_REQUEST["we_cmd"][2];
switch($iNumItems){
	case  0: $iMaxItems = 200; break;
	case 11: $iMaxItems = 15; break;
	case 12: $iMaxItems = 20; break;
	case 13: $iMaxItems = 25; break;
	case 14: $iMaxItems = 50; break;
	default: $iMaxItems = $iNumItems;
}
...
$i = $j = $k = 0;
while($j < $iMaxItems){
	$_query = "SELECT * FROM ".HISTORY_TABLE.(!empty($_where)? (' WHERE '.((count($_users_where)>0)? 'UserName IN ('.implode(',',$_users_where) . ') AND ' : '').'DocumentTable IN('.implode(',',$_where).')') : '').(($iDate)? ' AND ModDate >'.$timestamp : '').' ORDER BY ModDate DESC LIMIT '.($k * $_count)." , ".$_count.";";
	$k++;
	$DB_WE->query($_query);
	$_db = new DB_WE();
	...
	while($DB_WE->next_record()){
		...
		$_hash = getHash("SELECT ID,Path,Icon,Text,ContentType,ModDate,CreatorID,Owners,RestrictOwners FROM ".$_table." WHERE ID = '".$DB_WE->f("DID")."'".(!empty($_paths)? (' AND ('.implode(' OR ',$_paths).')'): '').";",$_db);	
		if(!empty($_hash)) {
			$_show = true;
			...
			if($_show){
				if($i+1 <= $iMaxItems){
					...
					$j++;
					$lastModified .= '<tr>';
					$lastModified .= '<td width="20" height="20" valign="middle" nowrap><img src="'.ICON_DIR.$_hash['Icon'].'">'.getpixel(4,1).'</td>';
					$lastModified .= '<td valign="middle" class="middlefont">';
					$lastModified .= '<a href="'.
						'javascript:top.weEditorFrameController.openDocument(\''.$_table.'\',\''.$_hash['ID'].'\',\''.$_hash['ContentType'].'\');"'.
						' title="'.$_hash['Path'].'" style="color:#000000;text-decoration:none;">'.$_hash['Path']."</a></td>";
					if ($bMfdBy) $lastModified .= '<td>'.getpixel(5,1).'</td><td class="middlefont" nowrap>'.$DB_WE->f("UserName").(($bDateLastMfd)? ',' : '').'</td>';
					if ($bDateLastMfd) $lastModified .= '<td>'.getpixel(5,1).'</td><td class="middlefont" nowrap>'.date($GLOBALS["l_global"]["date_format"],$_hash['ModDate']).'</td>';
					$lastModified .= "</tr>\n";
				} else{
					break;
				}
...
print we_htmlElement::htmlHtml(
	...
	we_htmlElement::htmlBody(array(
		...
		),we_htmlElement::htmlDiv(array("id"=>"mfd"),$lastModified)
	)
);
