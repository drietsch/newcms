...
$showAC = false;
$yuiSuggest->setAutocompleteField("yuiAcInputDoc","yuiAcContainerDoc",FILE_TABLE,"folder,text/webEdition,text/html","docSelector",14,1,"yuiAcLayerDoc",array("yuiAcIdDoc"),1,"296px");	
...
function getHTMLDirSelector($_selType) {
	...
	$_button_doc = $we_button->create_button("select", "javascript:we_cmd('openDirselector',document.we_form.elements['FolderID'].value,'".FILE_TABLE."','document.we_form.elements[\\'FolderID\\'].value','document.we_form.elements[\\'FolderPath\\'].value','','".session_id()."','$rootDirID')");
	$_button_obj = defined('OBJECT_TABLE') ? $we_button->create_button("select","javascript:we_cmd('openDirselector',document.we_form.elements['FolderID'].value,'".OBJECT_FILES_TABLE."','document.we_form.elements[\\'FolderID\\'].value','document.we_form.elements[\\'FolderPath\\'].value','','".session_id()."','$rootDirID')") : '';

	$_buttons = '<div id="docFolder" style="display: '.(!$_selType? "inline" : "none").'">' .
		$_button_doc .
		"</div>" .
		'<div id="objFolder" style="display: '.($_selType? "inline" : "none").'">' .
		$_button_obj .
		"</div>";
	$_path = id_to_path($folderID,(!$_selType? FILE_TABLE : (defined("OBJECT_FILES_TABLE")? OBJECT_FILES_TABLE : "")));

	return we_htmlElement::htmlDiv(array("style"=>"margin-top:10px;"),
		htmlFormElementTable("<div id=\"yuiAcLayerDoc\" class=\"yuiAcLayer\">".$yuiSuggest->getErrorMarkPlaceHolder("yuiAcErrorMarkDoc").htmlTextInput("FolderPath",58,$_path,"",'onchange="" id="yuiAcInputDoc"',"text",(420-120),0)."<div id=\"yuiAcContainerDoc\"></div></div>",
			$l_cockpit["dir"],
			"left",
			"defaultfont",
			getPixel(300,20),
			we_htmlElement::htmlHidden(array("name"=>"FolderID","value"=>$folderID,"id"=>"yuiAcIdDoc")),
			getPixel(20,4),
			$_buttons
		)
	);
...
$docTypes = array(0=>$l_cockpit["no_entry"]);
$q = getDoctypeQuery($DB_WE);
$DB_WE->query("SELECT ID,DocType FROM ".DOC_TYPES_TABLE." $q");
while($DB_WE->next_record()) {
	$docTypes[$DB_WE->f("ID")] = $DB_WE->f("DocType");
}
$doctypeElement = htmlFormElementTable(htmlSelect("DocTypeID",$docTypes,1,0,false,'onChange="" style="width:420px; border: #AAAAAA solid 1px;"','value'),$l_cockpit["doctype"]);

$cls = new we_htmlSelect(array("name"=>"classID","size"=>"1","class"=>"defaultfont","style"=>"width:420px; border: #AAAAAA solid 1px"));
$optid = 0;
...
if($ac) {
	...
	while($DB_WE->next_record()) {
		...
		if ($DB_WE->f("ID") == -1) $cls->selectOption($DB_WE->f("ID"));
	}
}

function getHTMLCategory() {
	$we_button = new we_button();
	$addbut = $we_button->create_button("add", "javascript:we_cmd('openCatselector','','".CATEGORY_TABLE."','','','fillIDs();opener.addCat(top.allPaths);')",false,100,22,"","",(!we_hasPerm("EDIT_KATEGORIE")));
	$del_but = addslashes(we_htmlElement::htmlImg(array('src'=>IMAGE_DIR.'/button/btn_function_trash.gif','onclick'=>'javascript:#####placeHolder#####;top.mark();','style'=>'cursor: pointer; width: 27px;-moz-user-select: none;')));

	$js = we_htmlElement::jsElement('',array('src'=>JS_DIR.'utils/multi_edit.js?'.WE_VERSION));
	...
	$table->setColContent(2,0,we_htmlElement::htmlDiv(array('id'=>'categories','class'=>'blockWrapper','style'=>'width:420px;height:60px;border:#AAAAAA solid 1px;')));
...
$_seltype = array('doctype'=>$l_cockpit['documents']);
...
$divStatic = we_htmlElement::htmlDiv(array("id"=>"static","style"=>($_selection? "display:block;" : "display:none;")),
	we_htmlElement::htmlDiv(array("id"=>"treeContainer"),$tree->getHTMLMultiExplorer(420,180)).
	"<iframe name=\"cmd\" src=\"about:blank\" style=\"visibility:hidden; width: 0px; height: 0px;\"></iframe>"
);
...
$divDynamic = we_htmlElement::htmlDiv(array("id"=>"dynamic","style"=>(!$_selection? "display:block;" : "display:none;")),
	getHTMLDirSelector($_selType).
	getPixel(1,5).we_htmlElement::htmlBr().
	((!$_selType)? $doctypeElement : htmlFormElementTable($cls->getHTMLCode(),$l_cockpit["class"])).
	getPixel(1,5).we_htmlElement::htmlBr().getHTMLCategory());

$divContent = we_htmlElement::htmlDiv(array("style"=>"display:block;"),
	htmlSelect("Selection",array("dynamic"=>$l_cockpit["dyn_selection"],"static"=>$l_cockpit['stat_selection']),1,($_selection? "static" : "dynamic"),false,'style="width:420px;border:#AAAAAA solid 1px;" onChange="closeAllSelection();we_submit();"','value') .
	we_htmlElement::htmlBr().
	htmlSelect("headerSwitch",$captions,1,(!$_selType? FILE_TABLE : OBJECT_FILES_TABLE),false,'style="width:420px;border:#AAAAAA solid 1px;margin-top:10px;" onChange="setHead(this.value);we_submit();"','value',420).
	$divStatic.$divDynamic.
	getPixel(1,5).we_htmlElement::htmlBr().
	htmlFormElementTable(htmlTextInput($name="title",$size=55,$value=$_title,$maxlength=255,$attribs="",$type="text",$width=420,$height=0),$l_cockpit['title'],"left","defaultfont")
);

array_push($parts,array("headline"=>"","html"=>$divContent,"space"=>0));
...
$sTblWidget = we_multiIconBox::getHTML("mdcProps","100%",$parts,30,$buttons,-1,"","","",$l_cockpit['my_documents']);

print we_htmlElement::htmlHtml(
	we_htmlElement::htmlHead(
		we_htmlElement::htmlTitle($l_cockpit['my_documents']).
	...
	we_htmlElement::htmlBody(array(
		"class" => "weDialogBody",
		"onload" => "init();startInit();"
		),we_htmlElement::htmlForm("",
			we_htmlElement::htmlHidden(array("name"=>"table","value"=>"")).
			we_htmlElement::htmlHidden(array("name"=>"FolderID","value"=>0)).
			we_htmlElement::htmlHidden(array("name"=>"CategoriesControl","value"=>(isset($_REQUEST['CategoriesCount'])? $_REQUEST['CategoriesCount'] : 0))).
