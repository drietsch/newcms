...
class we_linklist{
	...
	function getLink($nr){
		...
		if($this->getType($nr) == "int"){
			$link = $this->getUrl($nr);
		}else if($this->getType($nr) == "ext"){
			$link = $this->getHref($nr);
		}else if($this->getType($nr) == "mail"){
			$link = $this->getHref($nr);
		}else if($this->getType($nr) == "obj"){
			$link = getHrefForObject($this->getObjID($nr),$GLOBALS["WE_MAIN_DOC"]->ParentID,$GLOBALS["WE_MAIN_DOC"]->Path,$this->db);
	...
	function getLinktag($nr,$link="",$tagAttr=""){
		...
		if(isset($jswinAttribs) && is_array($jswinAttribs) && isset($jswinAttribs["jswin"]) ){    //popUp
			...
			}else if($jswinAttribs["jsposx"]!="" || $jswinAttribs["jsposy"]!=""){
			...
			$lattribs = removeAttribs($lattribs, array('name','target','href','onClick','onclick'));
	...
	function getString(){
		if(sizeof($this->listArray) == 0) return "";
	...
	function  correctContent($content){
			...
			while($starttag &&  (!($ipos === false))){
				...
				if((!($starttagpos === false)) && ($starttagpos < $endtagpos)){
					$ipos  = $starttagpos;
					$starttag++;
				}else if(!($endtagpos === false)){
					$starttag--;
					$ipos = $endtagpos;
				}else if(!($starttagpos === false)){
					$ipos =  $starttagpos;
				}
			}
	...
	function getHTML($editmode,$attribs,$content,$docName){
		...
		$tp->parseTags($tags,$content,'<we_:_linklistRef>',array('we:ifVar'));
			...
			if(abs($i) || $i == "0"){
				...
				if($linkcontent){
					...
					if($editmode){
						...
						$plusbut  = $we_button->create_button("image:btn_add_link", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(1);we_cmd('insert_link_at_linklist','" . $attribs["name"] . "','" . $i . "')", true, 100, 22, "", "", $disabled);
						$upbut    = $we_button->create_button("image:btn_direction_up", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(1);we_cmd('up_link_at_list','" . $attribs["name"] . "','" . $i . "')", true, -1, -1, "", "", !($i > 0));
						$downbut  = $we_button->create_button("image:btn_direction_down", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(1);we_cmd('down_link_at_list','" . $attribs["name"] . "','" . $i . "')", true, -1, -1, "", "", !($i < (sizeof($this->listArray) - 1)));
						$editbut  = $we_button->create_button("image:btn_edit_link", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(1);we_cmd('edit_linklist','" . $attribs["name"] . "','" . $i . "')", true);
						$trashbut = $we_button->create_button("image:btn_function_trash", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(1);we_cmd('delete_linklist','" . $attribs["name"] . "','" . $i . "','" . $names . "')", true);
						$buts     = $we_button->create_button_table(array($plusbut, $upbut, $downbut, $editbut, $trashbut), 5);
					}
					...
					if( strpos($foo, 'position') || strpos($foo, 'ifPosition') || strpos($foo, 'ifNotPosition')){
						$foo = '<?php $GLOBALS[\'we_position\'][\'linklist\'][\'' . $this->name . '\'] = array(\'size\'=> ' . sizeof($this->listArray) . ',\'position\'=>' . ($i+1) . '); ?>' . $foo . '<?php unset($GLOBALS[\'we_position\'][\'linklist\'][\'' . $this->name . '\']); ?>';
					}
					//	handle we:ifPosition - if available
					...
					$foo = str_replace('<we_:_ifSelf>','<?php if("'.$GLOBALS["WE_MAIN_DOC"]->Path.'" == "'.$this->getLink($i).'"): ?>',$foo);
					$foo = str_replace('</we_:_ifSelf>','<?php endif ?>',$foo);
					$foo = str_replace('<we_:_ifNotSelf>','<?php if("'.$GLOBALS["WE_MAIN_DOC"]->Path.'" != "'.$this->getLink($i).'"): ?>',$foo);
		...
		if($editmode){
			...
			if(isset($GLOBALS["we_list_inserted"]) && isset($GLOBALS["we_list_inserted"]) && ($GLOBALS["we_list_inserted"] == $attribs["name"])) {
				$out .= '<script language="JavaScript" type="text/javascript">we_cmd(\'edit_linklist\',\''.$attribs["name"].'\',\''.((isset($GLOBALS["we_list_insertedNr"]) && ($GLOBALS["we_list_insertedNr"] != "")) ? $GLOBALS["we_list_insertedNr"] : $this->getMaxListNrID()).'\');</script>';
			}
			...
			if($limit == 0 || ($limit != 0 && $this->length() < $limit)) {
				$plusbut = "<br>" . $we_button->create_button("image:btn_add_link", "javascript:setScrollTo();_EditorFrame.setEditorIsHot(1);we_cmd('add_link_to_linklist','" . $attribs["name"] . "')", true, 100, 22, "", "", $disabled);
				$out .= '<input type="hidden" name="we_'.$docName.'_linklist['.$attribs["name"].']" value="'.htmlspecialchars($this->getString()).'">'.eregi_replace('(.*)<we_:_link[^>/]*/?>(.*)','\1'.$plusbut.'\2'."\n",$clearContent);
			}
		}
	...
	function makeImgTag($nr,$attribs=""){
		$id = $this->getImageID($nr);
		if(!$attribs) $attribs = $this->getImageAttribs($nr);
