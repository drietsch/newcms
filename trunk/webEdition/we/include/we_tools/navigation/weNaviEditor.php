<?php
		...
		$_parts[] = array(
			...
			'html'=> htmlSelect('ParentID',$_dirs,1,$_navi->ParentID,false,($GLOBALS['BROWSER']=="IE" ? '' : 'style="width: '.$_input_size.'px;" ') . 'onChange="queryEntries(this.value)"'),
			'space' => $_space_size,
			'noline' => 1
		);
		...
		$_parts[] = array(
			...
			'html'=>	hidden('Ordn',$_navi->Ordn) . htmlTextInput('OrdnTxt',8,($_navi->Ordn+1),'','onchange="document.we_form.Ordn.value=(document.we_form.OrdnTxt.value-1);"','text',117) . getPixel(6,5) . htmlSelect('OrdnSelect',array('begin'=>$l_navigation['begin'],'end'=>$l_navigation['end']),1,'',false,'onchange="document.we_form.OrdnTxt.value=document.we_form.OrdnSelect.options[document.we_form.OrdnSelect.selectedIndex].text;document.we_form.Ordn.value=this.value;"',"value",317),
			'space' => $_space_size,
			'noline' => 1
		);
		...
		$_body=we_htmlElement::htmlBody(array("class"=>"weDialogBody","onLoad"=>"loaded=1;queryEntries(".$_def.")"),
					we_htmlElement::htmlForm(array("name"=>"we_form","onsubmit"=>"return false"),
						we_multiIconBox::getHTML('','100%',$_parts,30,$buttonsBottom,-1,'','',false,$l_navigation['add_navigation'],"", 311)
					)

		);

		$_head=WE_DEFAULT_HEAD."\n" . STYLESHEET . "\n".
				we_htmlElement::jsElement('',array('src'=>'/webEdition/js/libs/yui/yahoo-min.js')) . "\n".
				we_htmlElement::jsElement('',array('src'=>'/webEdition/js/libs/yui/event-min.js')) . "\n".
				we_htmlElement::jsElement('',array('src'=>'/webEdition/js/libs/yui/connection-min.js')) . "\n".
				we_htmlElement::jsElement($_js);

		print we_htmlElement::htmlHtml(
					we_htmlElement::htmlHead($_head).
					$_body
		);
