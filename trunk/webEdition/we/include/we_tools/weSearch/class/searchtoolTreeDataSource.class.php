  ...
  class searchtoolTreeDataSource extends weToolTreeDataSource
  {
      ...
      function getItemsFromDB($ParentID = 0, $offset = 0, $segment = 500, $elem = 'ID,ParentID,Path,Text,Icon,IsFolder', $addWhere = '', $addOrderBy = '')
      {
        ...
        WHERE ID='$id'", "ParentID", $db);
          ...
          if ($offset && $segment) {
              $this->treeItems[] = array('icon' => 'arrowup.gif', 'id' => 'prev_' . $ParentID, 'parentid' => $ParentID, 'text' => 'display (' . $prevoffset . '-' . $offset . ')', 'contenttype' => 'arrowup', 'table' => $table, 'typ' => 'threedots', 'open' => 0, 'published' => 0, 'disabled' => 0, 'tooltip' => '', 'offset' => $prevoffset);
          }
          
          $where = " WHERE $wsQuery ParentID=$ParentID " . $addWhere;
          
          $db->query("SELECT $elem, LOWER(Text) AS lowtext, abs(Text) as Nr, (Text REGEXP '^[0-9]') as isNr from $table $where ORDER BY isNr DESC,Nr,lowtext,Text " . ($segment ? "LIMIT $offset,$segment;" : ";"));
          ...
          while ($db->next_record()) {
              ...
              elseif (($db->f('Path') == '/Versionen' || $db->f('Path') == '/Versionen/Dokumente' || $db->f('Path') == '/Versionen/Objekte' || $db->f('Path') == '/Versionen/Dokumente/gelöschte Dokumente' || $db->f('Path') == '/Versionen/Objekte/gelöschte Objekte') && !we_hasPerm('SEE_VERSIONS')) {
              }
              else {
          ...
          if ($segment && ($total > $nextoffset)) {
              $this->treeItems[] = array('icon' => 'arrowdown.gif', 'id' => 'next_' . $ParentID, 'parentid' => $ParentID, 'text' => 'display (' . $nextoffset . '-' . ($nextoffset + $segment) . ')', 'contenttype' => 'arrowdown', 'table' => $table, 'typ' => 'threedots', 'open' => 0, 'disabled' => 0, 'tooltip' => '', 'offset' => $nextoffset);
          }
