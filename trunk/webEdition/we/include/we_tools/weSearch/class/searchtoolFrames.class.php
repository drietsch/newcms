...
class searchtoolFrames extends weToolFrames {
	...
	function getHTMLCmd() {
      ...
      ';
		...
		$hiddens = we_htmlElement::htmlHidden ( array ('name' => 'pnt', 'value' => 'cmd' ) ) . we_htmlElement::htmlHidden ( array ('name' => 'cmd', 'value' => 'no_cmd' ) );
		
		$out .= we_htmlElement::htmlBody ( array ('bgcolor' => 'white', 'marginwidth' => '10', 'marginheight' => '10', 'leftmargin' => '10', 'topmargin' => '10' ), we_htmlElement::htmlForm ( array ('name' => 'we_form' ), $hiddens . we_htmlElement::jsElement ( $rootjs . $this->Tree->getJSLoadTree ( $_loader->getItems ( $pid, $offset, $this->Tree->default_segment, '' ) ) ) ) );
		
		if (isset ( $_SESSION ["weSearch"] ["modelidForTree"] )) {
			$out .= we_htmlElement::jsElement ( '' . $this->topFrame . '.treeData.selectnode("' . ($_SESSION ["weSearch"] ["modelidForTree"]) . '");' );
	...
	function getHTMLEditorHeader() {
		...
		if (we_hasPerm ( 'CAN_SEE_DOCUMENTS' )) {
			$we_tabs->addTab ( new we_tab ( '#', $l_weSearch ['documents'], '((' . $this->topFrame . '.activ_tab==1) ? TAB_ACTIVE : TAB_NORMAL)', "setTab('1');", array ("id" => "tab_1", "style" => "display:$displayEntry" ) ) );
		}
		if ($_SESSION ["we_mode"] != "seem" && we_hasPerm ( 'CAN_SEE_TEMPLATES' )) {
			$we_tabs->addTab ( new we_tab ( '#', $l_weSearch ['templates'], '((' . $this->topFrame . '.activ_tab==2) ? TAB_ACTIVE : TAB_NORMAL)', "setTab('2');", array ("id" => "tab_2", "style" => "display:$displayEntry" ) ) );
		}
		$we_tabs->addTab ( new we_tab ( '#', $l_weSearch ['advSearch'], '((' . $this->topFrame . '.activ_tab==3) ? TAB_ACTIVE : TAB_NORMAL)', "setTab('3');", array ("id" => "tab_3", "style" => "display:$displayEntry" ) ) );
		
		//tabs for folders
		$we_tabs->addTab ( new we_tab ( '#', $l_weSearch ['properties'], '((' . $this->topFrame . '.activ_tab==4) ? TAB_ACTIVE : TAB_NORMAL)', "setTab('4');", array ("id" => "tab_4", "style" => "display:$displayFolder" ) ) );
	...
	function getHTMLEditorBody() {
		$body = we_htmlElement::htmlBody ( array ("class" => "weEditorBody", 'onkeypress' => 'javascript:if(event.keyCode==\'13\' || event.keyCode==\'3\') search(true);', 'onLoad' => 'loaded=1;setTimeout(\'init()\',200);', 'onresize' => 'sizeScrollContent();' ), we_htmlElement::jsElement ( '', array ('src' => JS_DIR . 'utils/multi_edit.js?' . WE_VERSION ) ) . we_htmlElement::htmlForm ( array ('name' => 'we_form', 'onsubmit' => 'return false' ), $this->getHTMLProperties () . we_htmlElement::htmlHidden ( array ('name' => 'predefined', 'value' => $this->Model->predefined ) ) . we_htmlElement::htmlHidden ( array ('name' => 'savedSearchName', 'value' => $this->Model->Text ) ) ) );
...
//      }
	...
	function getTab() {
		$cmdid = isset ( $_REQUEST ['cmdid'] ) ? ($_REQUEST ['cmdid']) : "";
		if($cmdid!="") $_REQUEST["searchstartAdvSearch"] = 0;
		if (isset ( $_REQUEST ["tab"] ) && $_REQUEST ["tab"] != "") {
			$tabNr = $_REQUEST ["tab"];
		} 
		elseif ($cmdid!="") {
...
//		}
	...
	function getHTMLEditorFooter() {
		...
		$_but_table = $we_button->create_button_table ( array ($we_button->create_button ( "save", "javascript:we_save();", true, 100, 22, '', '', (! we_hasPerm ( 'EDIT_NAVIGATION' )) ) ), 10, array ('style' => 'margin-left: 15px' ) );
		
		return $this->getHTMLDocument ( we_htmlElement::jsElement ( "", array ("src" => JS_DIR . "attachKeyListener.js" ) ) . we_htmlElement::jsElement ( '
          
          function we_save() {
            ' . $this->topFrame . '.we_cmd("tool_' . $this->toolName . '_save");
          }
          ' ) . we_htmlElement::htmlBody ( array ("bgcolor" => "white", "background" => "/webEdition/images/edit/editfooterback.gif", "marginwidth" => "0", "marginheight" => "0", "leftmargin" => "0", "topmargin" => "0" ), we_htmlElement::htmlForm ( array ( ), $table1->getHtmlCode () . $_but_table ) ) );
	}
	...
	function getHTMLProperties($preselect = '') {
		...
		$hiddens = array ('cmd' => '', 'pnt' => 'edbody', 'tabnr' => $tabNr, 'vernr' => (isset ( $_REQUEST ['vernr'] ) ? $_REQUEST ['vernr'] : 0), 'delayParam' => (isset ( $_REQUEST ['delayParam'] ) ? $_REQUEST ['delayParam'] : '') );
		
		$out .= $this->View->getCommonHiddens ( $hiddens );
		$out .= we_htmlElement::htmlHidden ( array ('name' => 'newone', 'value' => ($this->Model->ID == 0 ? 1 : 0) ) );
		
		$out .= we_htmlElement::htmlDiv ( array ('id' => 'tab1', 'style' => ($tabNr == 1 ? 'display: block;' : 'display: none') ), $this->getHTMLSearchtool ( $this->getHTMLTabDocuments () ) );
		$out .= we_htmlElement::htmlDiv ( array ('id' => 'tab2', 'style' => ($tabNr == 2 ? 'display: block;' : 'display: none') ), $this->getHTMLSearchtool ( $this->getHTMLTabTemplates () ) );
		$out .= we_htmlElement::htmlDiv ( array ('id' => 'tab3', 'style' => ($tabNr == 3 ? 'display: block;' : 'display: none') ), $this->getHTMLSearchtool ( $this->getHTMLTabAdvanced () ) );
		
		$out .= we_htmlElement::htmlDiv ( array ('id' => 'tab4', 'style' => ($tabNr == 4 ? 'display: block;' : 'display: none') ), $this->getHTMLSearchtool ( $this->getHTMLGeneral () ) );
	...
	function getHTMLGeneral() {
		...
		array_push ( $parts, array ('headline' => $l_weSearch ['general'], 'html' => htmlFormElementTable ( htmlTextInput ( 'Text', '', $this->Model->Text, '', 'style="width: ' . $this->_width_size . '" );"', '', '', '', '', $disabled ), $l_weSearch ['dir'] ), 'space' => $this->_space_size, 'noline' => 1 ) );
    ...
    ';
		...
		array_push ( $parts, array ('headline' => $l_weSearch ['suchenIn'], 'html' => $_searchDirChooser_block, 'space' => $this->_space_size ) );
    ...
    ';
		...
		array_push ( $parts, array ('headline' => $l_weSearch ['text'], 'html' => $_searchField_block, 'space' => $this->_space_size ) );
		
		$_searchCheckboxes_block = '
      <div>
      ' . $this->View->getSearchDialogCheckboxes ( $innerSearch ) . '
      </div>
    ';
		
		array_push ( $parts, array ('headline' => $l_weSearch ['optionen'], 'html' => $_searchCheckboxes_block, 'space' => $this->_space_size ) );
    ...
    ';
		...
		array_push ( $parts, array ('headline' => '', 'html' => $_searchResult_block, 'space' => $this->_space_size ) );
    ...
    ';
		...
		array_push ( $parts, array ('headline' => $l_weSearch ['text'], 'html' => $_searchFields_block, 'space' => $this->_space_size ) );
    ...
    ';
		...
		array_push ( $parts, array ('headline' => $l_weSearch ['anzeigen'], 'html' => $_searchCheckboxes_block, 'space' => $this->_space_size ) );
