...
class weCodeWizard {
	...
	function _getSnippetsByDir($SnippetDir, $Depth = 0) {
		...
		while (false !== ($_entry = $_dir->read())) {
			...
			} else if(		!is_dir($this->SnippetPath . $SnippetDir . "/" . $_entry)
						&&	eregi(".xml$", $_entry)) {
				// get the snippet
				$_snippet = weCodeWizardSnippet::initByXmlFile($this->SnippetPath . $SnippetDir . "/" . $_entry);
				$_item = array(
					'type' => 'option',
