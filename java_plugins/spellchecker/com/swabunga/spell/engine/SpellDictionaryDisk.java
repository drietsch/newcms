...
/* Some changes made by satom on Aug 25, 2006 */
...
public class SpellDictionaryDisk extends SpellDictionaryASpell {
	protected final static String DIRECTORY_WORDS = "words"; // visibility changed by satom
	protected final static String DIRECTORY_DB = "db"; // visibility changed by satom
	protected final static String FILE_CONTENTS = "contents"; // visibility changed by satom
	protected final static String FILE_DB = "words.db"; // visibility changed by satom
	protected final static String FILE_INDEX = "words.idx"; // visibility changed by satom

  /* maximum number of words an index entry can represent */
  private final static int INDEX_SIZE_MAX = 200;

  protected File base; // visibility changed by satom
  protected File words; // visibility changed by satom
  protected File db; // visibility changed by satom
  protected Map index; // visibility changed by satom
  /**
  ...
  /* defines the default encoding - added by satom */
   ...
   * @param encoding specifies the character set encoding - added by satom 
  ...
  public SpellDictionaryDisk(File base, File phonetic, boolean block, String enc) throws FileNotFoundException, IOException {
    super(phonetic,enc);
    defEncoding = enc; // added by satom
...
/*
  ...
  public List getWords(String code) {
    ...
    if (posLen != null) {
      try {
        BufferedReader input = new BufferedReader(new InputStreamReader(new FileInputStream(new File(db, FILE_DB)),defEncoding)); // changed by satom
        input.skip(posLen[0]);
        char[] bytes = new char[posLen[1]]; // changed by satom
  ...
  private File buildSortedFile() throws FileNotFoundException, IOException {
    ...
    File[] wordFiles = words.listFiles();
    for (int i = 0; i < wordFiles.length; i++) {
      BufferedReader r = new BufferedReader(new InputStreamReader(new FileInputStream(wordFiles[i]),defEncoding)); // changed by satom
      String word;
    ...
    BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file), defEncoding));// changed by satom
    String prev = null;
  ...
  private void buildCodeDb(File sortedWords) throws FileNotFoundException, IOException {
    ...
    BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(sortedWords),defEncoding));// changed by satom
    String word;
    ...
    BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(db, FILE_DB)), defEncoding));// changed by satom
...
//            if (thisCode.length() > 3) thisCode = thisCode.substring(0, 3);
      ...
      out.write(new String(bytes));// changed by satom
    }
    out.close();
    ...
    BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(db, FILE_INDEX)), defEncoding));// changed by satom
  ...
  protected int[] getStartPosAndLen(String code) { // visibility changed by satom
    while (code.length() > 0) {
  ...
  protected static String[] split(String input, String delimiter) {  // visibility changed by satom
  ...
  protected class CodeWord implements Comparable {
    private String code;
    private String word;
